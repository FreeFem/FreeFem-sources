%&LaTeX  
%   mettre l'example pour calculer les indicateurs d'erreur
%   intalledges
%    -----------
...
Basically \freefempp  is a \index{compiler} compiler,
  the language is typed, polymorphic and reentrant with macro generation\ref{macro}.
...
ubderscore character  \texttt{\_} is not allowed, because
...
\begin{description}
    ...
    \itemtt[hTriangle] size of the current triangle  \index{lenEdge}
    \itemtt[area]  area of the current triangle \index{area}
    \itemtt[cout]  console  \Ostream
    \itemtt[cin]  console  \Istream
    \itemtt[true]    true  \Bool value
    \itemtt[false]   false  \Bool value
    \itemtt[pi]   \Real ~approximation of $\pi$    
\end{description}
...
operator (with right  precedence (as in math)),
...
The transpose operator is texttt{\string'} like  MathLab or SciLab, so the way 
...
\centerline{\texttt{\bf square, border, buildmesh,  movemesh ,
 adaptmesh, readmesh, trunc, triangulate}}

 \index{square}\index{border}\index{buildmesh}\index{movemesh}\index{adaptmesh}
 \index{readmesh}\index{triangulate}
...
plot(Th,wait=1,fill=1,ps="Lshape.eps");// figure \ref{Lshape}
real coef=1;  
...
plot(Th,wait=1,fill=1,ps="movemesh.eps");// figure \ref{movemesh}
\eFF
...
like $u=u$ will redefine $u$ on the new Th and therefore destroy the old Th
if $u$ was the only function using it.
\end{remark}

\subsection{Read and writre mesh}
...
The coordinate of the set of point are set in a file like :
...
\eFF
here the third number by line is use in the command  \texttt{triangulate}.
...
{ ifstream file("xyf");
   ...
   file >> xx >>yy >> fxy[][i];  // to only read third row
}
plot(fxu); // plot the function 
\eFF
...
\end{equation}
    ...
    by hand (false is the default's value).  So 1 or 3 functions given defining 
...
@for (i=0;i<n;i++)  // all degre of freedom
 {
  ...
  plot(Th,Sh1,wait=1,ps="trunc"+i+".eps");  //  plot the mesh  the function's support                  
  u[][i]=0;      // reset 
 }
\eFF
...
like\\
\centerline{\ttCC{@fespace\ IDspace(IDmesh,<IDFE>)} } 
or with $k$ pair of periodic boundary condition

\centerline{\ttCC{@fespace\ IDspace(IDmesh,<IDFE>,periodic=[[la$_1$,sa$_1$],[lb$_1$,sb$_1$],...,[la$_k$,sa$_k$],[lb$_k$,sb$_k$]])}}
...
where a pair of periodic boundary condition
...
\begin{description}
    ...
    \begin{equation}
     P0_{h} = \{ v \in L^2(\Omega) / \forall K \in \mathcal{T}_{h}
        \quad v_{|K} = \alpha_{K }\hbox{constant of } \R \} \label{eq:P0}
     \end{equation}
     ...
     \end{equation}
     \item[P1dc]  piecewise linear  continuous finite element
     \index{P1dc|textbf}\index{fespace!P1dc}
     \begin{equation}
     P1_{h} = \{ v \in L^{2}(\Omega) / \forall K \in \mathcal{T}_{h}
        \quad v_{|K} \in P_{1} \} \label{eq:P1}
     \end{equation}
     \item[P1b]  piecewise linear  continuous finite element plus bubble
     \index{P1b|textbf}\index{fespace!P1b}
     \begin{equation}
     P1_{h} = \left\{ v \in L^{2}(\Omega) / \forall K \in \mathcal{T}_{h}
        \quad v_{|K} \in P_{1} \bigoplus Span\{ \Pi^{2}_{i=0} \lambda^{K}_{i} \} \right\} \label{eq:P1b}
     \end{equation}
     where $\lambda ^{K}_{i}$ is the barycentric coordinate fonction of the traingle $K$
     ...
     \begin{equation}
     P2_{h} = \{ v \in L^{2}(\Omega) / \forall K \in \mathcal{T}_{h}
        \quad v_{|K} \in P_{2} \}
     \end{equation}
...
$$U_{h}=\vecttwo{Uxh}{Uyh}  \quad \mbox{and}\quad V_{h}=\vecttwo{Vxh}{Vyh}$$.
 ...
 (type \texttt{FE}
 functions) are both
 functions from  $\R^{2}$ to $\R^{N}$ and arrays of real.
 ...
 or \texttt{uh},\texttt{[Uxh,Uyh]},one writes
...
\eFF
  ...
  \index{FE  function!n|textbf}\index{[]}\index{n}
...
\eq{ -\Delta v = 1 \hbox{ in } \Omega=\{(x,y)\in ]0,1[^2 \},
...
\plot[height=6cm]{Laplace}{  Isovalues of the solution  }
...
The parameters are FE function, the number $n$ of FE function is even
...
\index{solver!LU}\index{solver!CG}\index{solver!Crout}\index{solver!GMRES}\index{solver!Cholesky}
The named parameters are:
\begin{description}
    \item[solver=]   \texttt{LU}, \texttt{CG},
    \texttt{Crout},\texttt{Cholesky},\texttt{GMRES} ...
    ...
    and for \texttt{GMRES} the matrix is just  sparse.

    \item[eps=]  \index{problem!eps=}  \index{solver!eps=} a real expression. $\varepsilon$  sets the stopping test for
    ...
    \item[init=]   \index{problem!init=}  \index{solver!init=}boolean expression, if it is false or 0 \index{init=}
    the matrix is reconstructed. Note that if the mesh changes the matrix is
    reconstructed too.
    \item[precon=]  \index{problem!precon=}  \index{solver!precon=}name of a function (for example \texttt{P}) to set the precondioner. \index{precon=}
    The prototype for the function \texttt{P} must be
\bFF
    @func @real[@int]  P(@real[@int] & xx) ;
\eFF
      \item[tgv=]  \index{problem!tgv=}  \index{solver!tgv=}  Huge value, for lock boundary conditions
      \item[dimKrylov=]  \index{problem! dimKrylov =}  \index{solver! dimKrylov =}  dimension of the Krylov space.
\end{description}

\subsection{Problem definition}

Below  \texttt{v} is the unknown function and \texttt{vv} is the test function.

  After the "=" sign, one may find sums of:
  \index{int1d}\index{int2d}
...
\begin{itemize}
    ...
    \item  A bilinear form
         \texttt{ int1d(Th)( K*v*vv) },
        \texttt{ int1d(Th,2,5)( K*v*vv) },
        a sparce matrix of type \texttt{matrix}
    \item  A linear form
         \texttt{ int1d(Th)( K*v) },
        \texttt{ int1d(Th,2,5)( K*v) },
        a vector of type  \texttt{real[int]}

    \item  The boundary condition form
    \begin{itemize}
        \item  An "on" form (for Dirichlet ) :\index{on}
     \texttt{ on(1, u = g )}\index{on}
        \item  a linear form on $\Gamma$  (for Neumann )
         \texttt{ int1d(Th))( -f*vv) } or \texttt{ int1d(Th,3))( -f*vv) }
        \item   a bilinear form on $\Gamma$  or $\Gamma_{2}$ (for  Robin )
         \texttt{ int1d(Th))( K*v*vv) } or
         \texttt{ int1d(Th,2))( K*v*vv)}.
...
\subsection{Integrals}
  ...
  \item integrals on the edges of all triangle  \texttt{intalledges}.\index{intalledges}\index{int2d}\index{int1d}
  \end{itemize}
...
u1[] = A^-1*b[]; // solve the linear system \index{solve!linear system}
\eFF
...
\begin{description}
    ...
    \itemtt[fill=]  \index{\index{plot!coef=}}to fill between isovalues.
...
\end{description}
{\bf Warning} 
\texttt{convect}  is a non-local operator; in  the instruction "phi=convec([u1,u2],-dt,phi0)" every values of
phi0 are used to compute phi So \ttCC{phi=convec([u1,u2],-dt,phi0)} won't work.
...
//    label :  2 and 4  are left and right   side with y abcissa 
//             1 and 2  are bottom and upper side with x abcissa
...
An over exemple is in {periodic4.edp} file, with 
...
\subsubsection{NSUzawaCahouetChabart.edp}
 ...
 The idea of the precondioner is that in a periodic domain, all
...
A = at(Xh,Xh,solver=CG);  //  change the matrix \index{matrix!=}
...
$$  E_{ij} = 0.5 ( d_i u_j + d_j u_i ) + \sum_k d_i u_k * d_j*u_k  $$ 



The differential of $J$ is   $$ DJ(u_{1},u_{2})(v_{1},v_{2}) = \int 2 A(E[u_{1},u_{2}],DE[u_{1},u_{2}](v_{1},v_{2})) f'(F2(u_{1},u_{2}))) -  \int_{\Gamma_{p}} Pa  u_{2}  $$ \begin{eqnarray*} D^2Ju_{1},u_{2})((v_{1},v_{2}),(w_{1},w_{2}))  &= & A(E[u_{1},u_{2}],DE[u_{1},u_{2}](v_{1},v_{2})) A(E[u_{1},u_{2}],DE[u_{1},u_{2}](w_{1},w_{2})) f''(F2(u_{1},u_{2}))) \\
 & & +  A(DE[u_{1},u_{2}](v_{1},v_{2}),DE[u_{1},u_{2}](w_{1},w_{2})) f'(F2(u_{1},u_{2}))) \\ & & +  A(DE[u_{1},u_{2}],DDE[u_{1},u_{2}]((v_{1},v_{2}),(w_{1},w_{2}))) f'(F2(u_{1},u_{2}))) \end{eqnarray*}
...
\begin{figure}[hbt]
\includegraphics[width=15cm]{nl-elas}
\caption{The deformated domain}
\end{figure}
...
\itemtt[mpirank]  the number of my curent process in $\{0,..., mpisize-1\}$.
...
\eFF
\end{description}
\section{Schwarz in parallel}
If exemple is just the rewritting of exemple \texttt{schwarz-overlap} 
page \ref{schwarz-overlap.edp}\index{schwarz}
