...
%\DeclareFixedFont{\TitreFont}{\encodingdefault}{pnc}{r}{\shapedefault}{70pt}
 {\Blue{ \TitreFont FreeFem++ \\ \vglue 1cm  Manual}} \\ \vglue 3cm  ~ \\  
      \normalsize  { version 1.40  \Red{(Under construction)} }
 \\ \vglue 0.7cm
...
As its name says, it is public domain software based on the Finite
Element Method. This software runs on all unix OS (with g++ 2.95.2 or
better and X11R6) , on Window95, 98, 2000, NT, XP and   on MacOS 9 and
X.  \\\\
%%% alh? Is it really public domain? (no restriction whatsoever!)
...
element functions (\texttt{func}), arrays of finite element functions
(\texttt{func$[basic type]$}), linear and bilinear operators, sparse
...
\begin{description}
    ...
    the current point (x,y). Integer value. \index{region}
%%%alh (x,y,z) in R^2?
    \item[\texttt{P}] current point ($R^{2}$ value. \index{P}
    ...
    \itemtt[nTonEdge] integer index of the triangle on the current
    edge \index{nTonEdge}
...
%%%alh ``pi'' and ``endl'' should be among constants and not variables?
...
\eFF
constructs a grid $n× m$ in the rectangle $[1.2,1.8]×[0,1]$ and
a grid  $4× 5$ in the unit square $[0,1]^2$.

\begin{note} The label of \index{label}
...
%%% alh? Thxy.eps and xyf.eps not found
...
It is can by usefull we you have a Lagrange multiplier definied on the border.

So the fonction emptymesh remove all the internal point of a mesh expect if the
...
{  //  new stuff 2004 emptymesh (version 1.40) \hfilll
 // -- usefull to build Multiplicator space \hfilll
