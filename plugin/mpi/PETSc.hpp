...
namespace PETSc {
  ...
  class DistributedCSR {
   public:
    ...
    unsigned int _clast;
    DistributedCSR( )
      : _A( ), _petsc( ), _vS( ), _ksp( ), _exchange( ), _num( ), _first( ), _last( ), _cnum( ),
        _cfirst( ), _clast( ) {}
    ~DistributedCSR( ) { dtor( ); }
    void dtor( ) {
      if (_petsc) {
      ...
      if (_vS) {
        for (int i = 0; i < _vS->size( ); ++i) MatDestroy(&(*_vS)[i]);
        delete _vS;
        _vS = nullptr;
      }
  ...
  template< class HpddmType, typename std::enable_if< std::is_same<
                               HpddmType, HpSchwarz< PetscScalar > >::value >::type* = nullptr >
  void globalMapping(HpddmType* const& A, unsigned int*& num, unsigned int& start,
                     unsigned int& end, unsigned int& global, unsigned int* const list) {
    num = new unsigned int[A->getMatrix( )->_n];
    A->template globalMapping< 'C' >(num, num + A->getMatrix( )->_n, start, end, global,
                                     A->getScaling( ), list);
  }
  template< class HpddmType, typename std::enable_if< !std::is_same<
                               HpddmType, HpSchwarz< PetscScalar > >::value >::type* = nullptr >
  void globalMapping(HpddmType* const& A, unsigned int* const& num, unsigned int& start,
                     unsigned int& end, unsigned int& global, unsigned int* const list) {}
  template< class Type, class Tab,
            typename std::enable_if< !std::is_same<
              Tab, PETSc::DistributedCSR< HpSchwarz< PetscScalar > > >::value >::type* = nullptr >
  void setVectorSchur(Type* ptA, KN< Tab >* const& mT, KN< double >* const& pL) {
    ...
    for (int i = 0; i < pL->n; ++i) re[i] = std::abs((*pL)[i]) > 1.0e-12 ? nbSchur++ : 0;
    nbSchur--;
    unsigned int* num;
    ...
    for (int k = 0; k < mT->n; ++k) {
      MatriceMorse< PetscScalar >* mS =
        (mT->operator[](k)).A
          ? static_cast< MatriceMorse< PetscScalar >* >(&(*(mT->operator[](k)).A))
          : nullptr;
      int n = mS ? mS->n : 0;
      ...
      for (int i = 0; i < n; ++i) {
        ...
        std::sort(tmp[row].begin( ), tmp[row].end( ),
      ...
      ia = ja = nullptr;
      c = nullptr;
      HPDDM::MatrixCSR< PetscScalar >* dN = new_HPDDM_MatrixCSR< PetscScalar >(
        mS, true, s, is, js);    //->n, mS->m, mS->nbcoef, s, is, js, mS->symetrique, true);
      bool free = dN ? ptA->_A->HPDDM::template Subdomain< PetscScalar >::distributedCSR(
                         numSchur, start, end, ia, ja, c, dN)
                     : false;
      ...
      if (!ia && !ja && !c) ia = new int[2]( );
      MatMPIAIJSetPreallocationCSR((*ptA->_vS)[k], reinterpret_cast< PetscInt* >(ia),
  ...
  template< class Type, class Tab >
  void setFieldSplitPC(Type* ptA, KSP ksp, KN< double >* const& fields, KN< String >* const& names,
                       KN< Tab >* const& mT, KN< double >* const& pL = nullptr) {
    if (fields) {
      PC pc;
      KSPGetPC(ksp, &pc);
      ...
      for (int i = 0; i < fields->n; ++i) local[i] = std::lround(fields->operator[](i));
      ...
      if (fields->n) {
        if (ptA->_num)
          HPDDM::Subdomain< PetscScalar >::template distributedVec< 0 >(
            ptA->_num, first, last, local - fields->n, local, fields->n);
        else
      ...
      for (unsigned short j = 0; j < nb; ++j) {
        ...
        PCFieldSplitSetIS(
          pc, names && j < names->size( ) ? (*(names->operator[](j))).c_str( ) : NULL, is);
        ISDestroy(&is);
      }
      if (mT && mT->n > 0 &&
          (pL || std::is_same< Tab, PETSc::DistributedCSR< HpSchwarz< PetscScalar > > >::value)) {
        if (ptA->_vS) {
          for (int i = 0; i < ptA->_vS->size( ); ++i) MatDestroy(&(*ptA->_vS)[i]);
          delete ptA->_vS;
          ptA->_vS = nullptr;
        }
  ...
  void setCompositePC(PC pc, const std::vector< Mat >* S) {
    if (S && !S->empty( )) {
      ...
      if (S->size( ) == 1) {
        ...
        std::string str = std::string(prefixIS).substr(
          (prefixPC ? std::string(prefixPC).size( ) : 0) + std::string("fieldsplit_").size( ),
          std::string(prefixIS).size( ) - ((prefixPC ? std::string(prefixPC).size( ) : 0) +
                                           std::string("fieldsplit_").size( ) + 1));
        ...
        for (int i = 0; i < S->size( ); ++i) PCCompositeAddPC(pcS, PCNONE);
        PCSetUp(pcS);
  ...
  bool insertOptions(std::string* const& options) {
    ...
    if (options) {
      ...
      std::string item;
      while (std::getline(ss, item, ' ')) elems.push_back(item);
      int argc = elems.size( ) + 1;
      char** data = new char*[argc];
      data[0] = new char[options->size( ) + argc]( );
      data[1] = data[0] + 1;
      for (int i = 0; i < argc - 1; ++i) {
        if (i > 0) data[i + 1] = data[i] + elems[i - 1].size( ) + 1;
        strcpy(data[i + 1], elems[i].c_str( ));
        if (!fieldsplit) fieldsplit = (elems[i].compare("fieldsplit") == 0);
      }
