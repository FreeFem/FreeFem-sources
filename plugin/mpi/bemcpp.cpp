...
template<class K>
class UserGenerator: public VirtualGenerator<K>{
        Stack stack;
        Data_Funct** data;
public:
        UserGenerator(Stack stk, Data_Funct** d, int N, int M):VirtualGenerator<K>(N,M),stack(stk),data(d)  {}

        K get_coef(const int& i, const int& j)const {
            return RTFunc(data,i,j);
            /*
            ...
            Fem2D::R3 O = (*Th)(i);
            Fem2D::R3 PP = (*Th)(j);
            Fem2D::R3 OP = Fem2D::R3(PP,O);
            double lOP2 = OP.norme2();
            double lOP = sqrt(lOP2);
            //double sc = inti[j]*exp(-(lOP)*df.kappa0)/(lOP2+df.eps);
            double sc = (*gen->tab)[j]*exp(-(lOP)*kappa0)/(lOP2+0.001);
            return sc * kappa0/fourpi;
            */

            /*
            gen->s = i;
            gen->t = j;
            K res = GetAny< K >((*(gen->f))(stack));
...
template<class R,class MMesh, class v_fes1,class v_fes2>
AnyType OpHMatrixUser<R,MMesh,v_fes1,v_fes2>::Op::operator()(Stack stack)  const
{
    typedef typename v_fes1::pfes pfes1;
    typedef typename v_fes2::pfes pfes2;
    typedef typename v_fes1::FESpace FESpace1;
    ...
    *Hmat =0;
    ...
    Fem2D::R3 pp;
    bemtool::R3 p;
    SRdHat pbs(1./(SRdHat::d+1),1./(SRdHat::d+1),1./(SRdHat::d+1));
    TRdHat pbt(1./(TRdHat::d+1),1./(TRdHat::d+1),1./(TRdHat::d+1));
    ...
    VirtualGenerator<R>* generator = new UserGenerator<R>(stack,Pot->data,n,m);
...
static void Init_Bem() {
    ...
    Dcl_TypeandPtr< Data_Funct* >(0, 0,::InitializePtr< Data_Funct*  >, ::DeletePtr< Data_Funct* > );
    zzzfff->Add("DataFuncRT", atype< Data_Funct ** >( ));
    TheOperators->Add("<-", new OneOperator4_<Data_Funct **,Data_Funct **,KN<double>*,pmesh3,double >(init_datafunc)  );
    Global.Add("RTFunc","(",new OneOperator3_<double,Data_Funct **,long, long>(RTFunc));
    Global.Add("AdrRTFunc","(",new OneOperator0<long>(AdrRTFunc));
    ...
    typedef KN< HMatrixVirt<double>** > PHmat;
    Dcl_Type< PHmat * >(0, ::DestroyKNmat< HMatrixVirt<double>** >);

    // init array
    TheOperators->Add("<-", new OneOperator2_< PHmat *, PHmat *, long >(&set_initmat));
    //  A[i]
    atype< PHmat * >( )->Add("[", "", new OneOperator2_< HMatrixVirt<double>***, PHmat *, long >(get_elementp_< HMatrixVirt<double>**, PHmat *, long >));
    map_type_of_map[make_pair(atype< long >( ), atype< HMatrixVirt<double>*** >( ))] = atype< PHmat * >( );
    ...
    TheOperators->Add("<-", new OpHMatrixUser< double, Mesh3, v_fes3, v_fes3 > (1));
    TheOperators->Add("=", new OpHMatrixUser< double, Mesh3, v_fes3, v_fes3 > );
