...
int NLCG(const DJ &dJ, const P &C, KN_< R > &x, const int nbitermax, double &eps, long kprint,
         MPI_Comm *) {
  //  -------------
  assert(&x && &dJ && &C);
  typedef KN< R > Rn;
  ...
  R g2 = (Cg, g);
  if (g2 < 1e-30) {
    if (kprint > 1) cout << "GCNL  g^2 =" << g2 << " < 1.e-30  Nothing to do " << endl;
    return 2;
  }
  if (kprint > 5) cout << " 0 GCNL  g^2 =" << g2 << endl;
  ...
  if (verbosity) cout << " Non convergence of the NL cojugate gradient " << endl;
  return 0;
}
template< class T >
struct MPI_TYPE {};
...
int ConjuguedGradient2(const M &A, const P &C, KN_< R > &x, const KN_< R > &b, const int nbitermax,
                       double &eps, long kprint, MPI_Comm *commworld) {
  ...
  if (g2 < 1e-30) {
    if (kprint <= nbitermax) cout << "GC  g^2 =" << g2 << " < 1.e-30  Nothing to do " << endl;
    return 2;
  }
  if (kprint < 5) cout << " 0 GC  g^2 =" << g2 << endl;
  R reps2 = eps > 0 ? eps * eps * g2 : -eps;    // epsilon relatif
  eps = reps2;
  for (int iter = 0; iter <= nbitermax; iter++) {
    R rop = ro;
  ...
  cout << "CG does'nt converge: " << nbitermax << " ||g||^2 = " << g2 << " reps2= " << reps2
       << endl;
  return 0;
}

template< class Operator, class Vector, class Preconditioner, class Matrix, class Real >
int GMRES_MPI(const Operator &A, Vector &x, const Vector &b, const Preconditioner &M, Matrix &H,
              int &m, int &max_iter, Real &tol, MPI_Comm *commworld, long verbosity) {
  Real resid;
  int i, j = 1, k;
  ...
  while (j <= max_iter) {
    ...
    for (i = 0; i < m && j <= max_iter; i++, j++) {
      ...
      for (k = 0; k < i; k++) ApplyPlaneRotation(H(k, i), H(k + 1, i), cs(k), sn(k));
    ...
    if (verbosity > 4)
      cout << "GMRES: restart" << j << " " << beta << " " << normb << " " << beta / normb << " < "
           << tol << endl;
    if ((resid = beta / normb) < tol) {
      tol = resid;
      max_iter = j;
...
class MPILinearCG : public OneOperator {
 public:
    ...
    virtual AnyType operator( )(Stack stack) const {
      ...
      try {
        ...
        if (bbgmres) {
          AA.addMatMul(
            *bbgmres,
            *bbgmres);    // *bbgmres= AA* *bbgmres; // Ok Ax == b -> not translation of b .
          *bbgmres = -*bbgmres;
          if (verbosity > 1)
        ...
        if (CG) {
          ...
          if (cas < 0) {
            ...
            if (C) {
              ...
              ret = GMRES_MPI(AA, (KN< R > &)x, *bb, MatriceIdentite< R >(n), H, k, nbitermax, eps,
                              commworld, verb);
          }
        }
  ...
  E_F0 *code(const basicAC_F0 &args) const { return new E_LCG(args, cas, CG); }
  ...
  MPILinearCG(int cc, int CGG)
    : OneOperator(atype< long >( ), atype< Polymorphic * >( ), atype< KN< R > * >( ),
                  atype< KN< R > * >( )),
      cas(cc), CG(CGG) {}

  MPILinearCG(int cc, int CGG, int)
...
basicAC_F0::name_and_type MPILinearCG< R >::E_LCG::name_param[] = {
  ...
  {"veps", &typeid(double *)}, {"comm", &typeid(pcommworld)}, {"dimKrylov", &typeid(long)},
