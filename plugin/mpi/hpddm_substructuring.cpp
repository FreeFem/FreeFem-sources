//ff-c++-LIBRARY-dep: cxx11 hpddm [mumps parmetis ptscotch scotch scalapack|umfpack] [mkl|blas] mpi
// pthread mpifc fc ff-c++-cpp-dep:
...
namespace Substructuring {
  class Skeleton_Op : public E_F0mps {
   public:
    ...
    Skeleton_Op(const basicAC_F0& args, Expression param1, Expression param2, Expression param3)
      : interface(param1), restriction(param2), outInterface(param3) {
      args.SetNameParam(n_name_param, name_param, nargs);
    }
    AnyType operator( )(Stack stack) const;
  };
  basicAC_F0::name_and_type Skeleton_Op::name_param[] = {{"communicator", &typeid(pcommworld)},
                                                         {"interface", &typeid(KN< long >*)},
                                                         {"redundancy", &typeid(bool)}};
  class Skeleton : public OneOperator {
   public:
    Skeleton( )
      : OneOperator(atype< long >( ), atype< KN< double >* >( ),
                    atype< KN< Matrice_Creuse< double > >* >( ), atype< KN< KN< long > >* >( )) {}

    E_F0* code(const basicAC_F0& args) const {
      return new Skeleton_Op(args, t[0]->CastTo(args[0]), t[1]->CastTo(args[1]),
                             t[2]->CastTo(args[2]));
    }
  };
  AnyType Skeleton_Op::operator( )(Stack stack) const {
    ...
    KN< Matrice_Creuse< double > >* interpolation =
      GetAny< KN< Matrice_Creuse< double > >* >((*restriction)(stack));
    ...
    if (out->n != 1 + n) out->resize(1 + n);
    for (unsigned short i = 0; i < n; ++i) {
      MatriceMorse< double >* pt =
        static_cast< MatriceMorse< double >* >(&(*interpolation->operator[](i).A));
    ...
    for (unsigned short i = 0; i < neighborAfter; ++i) {
      MatriceMorse< double >* pt =
        static_cast< MatriceMorse< double >* >(&(*interpolation->operator[](i).A));
      // cout << mpirank << " receives from " << arrayNeighbor->operator[](i) << ", " << pt->n <<
      // "." << endl;
      MPI_Irecv(recv[i], pt->n, MPI_UNSIGNED_CHAR, out->operator[](0).operator[](i), 0, *comm,
                rq + n + i);
    }
    for (unsigned short i = neighborAfter; i < n; ++i) {
      ...
      MatriceMorse< double >* pt =
        static_cast< MatriceMorse< double >* >(&(*interpolation->operator[](dest).A));
      ...
      // cout << mpirank << " sends to " << arrayNeighbor->operator[](dest) << ", " << pt->n << "."
      // << endl;
      MPI_Isend(send[dest], pt->n, MPI_UNSIGNED_CHAR, out->operator[](0).operator[](dest), 0, *comm,
                rq + n + dest);
      resOut.resize(nnz);
    }
    for (unsigned short i = 0; i < neighborAfter; ++i) {
      ...
      MatriceMorse< double >* pt =
        static_cast< MatriceMorse< double >* >(&(*interpolation->operator[](index).A));
#ifndef VERSION_MATRICE_CREUSE
      resOut.resize(pt->nbcoef);
#else
      resOut.resize(pt->nnz);
      ...
      for (unsigned int j = 0; j < pt->n; ++j) {
        if (recv[index][j] == '1') {
#ifndef VERSION_MATRICE_CREUSE
          if (pt->lg[j + 1] != pt->lg[j]) resOut[(int)nnz++] = pt->cl[col++];
        } else if (pt->lg[j + 1] != pt->lg[j])
          ++col;
#else
          if (pt->p[j + 1] != pt->p[j]) resOut[(int)nnz++] = pt->j[col++];
    ...
    for (unsigned short i = 0; i < n; ++i) {
      delete[] recv[i];
      delete[] send[i];
      // cout << mpirank << " <=> " << arrayNeighbor->operator[](i) << " : " << out->operator[](i).n
      // << endl;
    }
    delete[] rq;
    ...
    if (interfaceNb) {
      ...
      for (int i = 0; i < in->n; ++i) {
        if (in->operator[](i) != 0.0) vec.emplace_back(i);
      }
      std::sort(vec.begin( ), vec.end( ));
      if (interfaceNb->n != vec.size( )) interfaceNb->resize(vec.size( ));
      for (signed int i = 0; i < vec.size( ); ++i) interfaceNb->operator[](i) = vec[i];
      for (unsigned short i = 0; i < n; ++i) {
        KN< long >& res = out->operator[](1 + i);
        for (signed int j = 0; j < res.n; ++j) {
          std::vector< unsigned int >::const_iterator idx =
            std::lower_bound(vec.cbegin( ), vec.cend( ), (unsigned int)res[j]);
      ...
      if (!redundancy) {
        std::vector< std::pair< unsigned short, unsigned int > >* array =
          new std::vector< std::pair< unsigned short, unsigned int > >[interfaceNb->n];
        for (unsigned short i = 0; i < n; ++i) {
          KN< long >& res = out->operator[](1 + i);
          for (signed int j = 0; j < res.n; ++j) array[res[j]].push_back(std::make_pair(i, res[j]));
        }
        ...
        for (unsigned short i = 0; i < n; ++i) copy[i].reserve(out->operator[](1 + i).n);
        for (unsigned int i = 0; i < interfaceNb->n; ++i) {
          for (std::vector< std::pair< unsigned short, unsigned int > >::const_iterator it =
                 array[i].cbegin( );
               it != array[i].cend( ); ++it) {
  ...
  class initDDM_Op : public E_F0mps {
   public:
    ...
    static basicAC_F0::name_and_type name_param[];
    Expression nargs[n_name_param];
    initDDM_Op(const basicAC_F0& args, Expression param1, Expression param2, Expression param3)
      : A(param1), Mat(param2), R(param3) {
      args.SetNameParam(n_name_param, name_param, nargs);
    }
  ...
  class initDDM : public OneOperator {
   public:
    initDDM( )
      : OneOperator(atype< Type* >( ), atype< Type* >( ), atype< Matrice_Creuse< K >* >( ),
                    atype< KN< KN< long > >* >( )) {}

    E_F0* code(const basicAC_F0& args) const {
      return new initDDM_Op< Type, K >(args, t[0]->CastTo(args[0]), t[1]->CastTo(args[1]),
                                       t[2]->CastTo(args[2]));
    }
  };
  template< class Type, class K >
  AnyType initDDM_Op< Type, K >::operator( )(Stack stack) const {
    ...
    if (ptR) {
      KN_< KN< long > > sub(ptR->n > 0 && ptR->operator[](0).n > 0 ? (*ptR)(FromTo(1, ptR->n - 1))
                                                                   : KN< KN< long > >( ));
      ptA->HPDDM::template Subdomain< K >::initialize(
        new_HPDDM_MatrixCSR< K >(mA), STL< long >(ptR->n > 0 ? ptR->operator[](0) : KN< long >( )),
        sub, nargs[0] ? (MPI_Comm*)GetAny< pcommworld >((*nargs[0])(stack)) : 0);
    }
    FEbaseArrayKn< K >* deflation =
      nargs[1] ? GetAny< FEbaseArrayKn< K >* >((*nargs[1])(stack)) : 0;
  ...
  class attachCoarseOperator_Op : public E_F0mps {
   public:
    ...
    static basicAC_F0::name_and_type name_param[];
    Expression nargs[n_name_param];
    attachCoarseOperator_Op(const basicAC_F0& args, Expression param1, Expression param2)
      : comm(param1), A(param2) {
      args.SetNameParam(n_name_param, name_param, nargs);
    }
  ...
  class attachCoarseOperator : public OneOperator {
   public:
    attachCoarseOperator( )
      : OneOperator(atype< long >( ), atype< pcommworld >( ), atype< Type* >( )) {}

    E_F0* code(const basicAC_F0& args) const {
      return new attachCoarseOperator_Op< Type, K >(args, t[0]->CastTo(args[0]),
                                                    t[1]->CastTo(args[1]));
    }
  };
  template< class Type, class K >
  AnyType attachCoarseOperator_Op< Type, K >::operator( )(Stack stack) const {
    ...
    if (!adaptive) ptA->setDeficiency(nu);
    double t = MPI_Wtime( );
    if (R) {
      if (adaptive) ptA->computeSchurComplement( );
      ptA->callNumfactPreconditioner( );
      if (timing) (*timing)[3] = MPI_Wtime( ) - t;
      if (adaptive) {
        if (opt.set("geneo_nu")) nu = opt["geneo_nu"];
#if defined(MUMPSSUB) || defined(PASTIXSUB) || defined(MKL_PARDISOSUB)
        t = MPI_Wtime( );
        ptA->solveGEVP( );
        if (timing) (*timing)[5] = MPI_Wtime( ) - t;
#else
        cout << "Please change your solver" << endl;
#endif
      }
      if (!R && !ptA->getVectors( )) cout << "Problem !" << endl;
      R->resize(0);
      MPI_Barrier(MPI_COMM_WORLD);
      if (timing) t = MPI_Wtime( );
      ...
      if (timing) (*timing)[4] = MPI_Wtime( ) - t;
      if (pair)
        if (pair->p) {
          int flag;
    ...
    if (timing) (*timing)[2] = MPI_Wtime( ) - t;
    if (ret) delete ret;
    if (pair)
      if (pair->p) {
        if (timing) t = MPI_Wtime( );
        MPI_Wait(&(pair->p->first), MPI_STATUS_IGNORE);
        if (timing) (*timing)[timing->n - 1] = MPI_Wtime( ) - t;
  ...
  class solveDDM_Op : public E_F0mps {
   public:
    ...
    static basicAC_F0::name_and_type name_param[];
    Expression nargs[n_name_param];
    solveDDM_Op(const basicAC_F0& args, Expression param1, Expression param2, Expression param3)
      : A(param1), rhs(param2), x(param3) {
      args.SetNameParam(n_name_param, name_param, nargs);
    }
  ...
  basicAC_F0::name_and_type solveDDM_Op< Type, K >::name_param[] = {
    {"timing", &typeid(KN< double >*)}, {"excluded", &typeid(bool)}, {"ret", &typeid(Pair< K >*)}};
  template< class Type, class K >
  class solveDDM : public OneOperator {
   public:
    solveDDM( )
      : OneOperator(atype< long >( ), atype< Type* >( ), atype< KN< K >* >( ),
                    atype< KN< K >* >( )) {}

    E_F0* code(const basicAC_F0& args) const {
      return new solveDDM_Op< Type, K >(args, t[0]->CastTo(args[0]), t[1]->CastTo(args[1]),
                                        t[2]->CastTo(args[2]));
    }
  };
  template< class Type, class K >
  AnyType solveDDM_Op< Type, K >::operator( )(Stack stack) const {
    ...
    Type* ptA = GetAny< Type* >((*A)(stack));
    if (ptX->n != ptRHS->n) return 0L;
    HPDDM::Option& opt = *HPDDM::Option::get( );
    KN< double >* timing = nargs[0] ? GetAny< KN< double >* >((*nargs[0])(stack)) : 0;
    bool excluded = nargs[1] && GetAny< bool >((*nargs[1])(stack));
    if (excluded) opt["level_2_exclude"];
    double timer = MPI_Wtime( );
    if (mpisize == 1) {
      ptA->computeSchurComplement( );
      ptA->callNumfactPreconditioner( );
      if (timing) (*timing)[3] = MPI_Wtime( ) - timer;
      timer = MPI_Wtime( );
      ptA->callNumfact( );
      if (timing) (*timing)[2] = MPI_Wtime( ) - timer;
    }
    MPI_Barrier(MPI_COMM_WORLD);
    if (!excluded && timing && mpisize > 1) (*timing)[timing->n - 1] += MPI_Wtime( ) - timer;
    timer = MPI_Wtime( );
    int rank;
    MPI_Comm_rank(ptA->getCommunicator( ), &rank);
    if (rank != mpirank || rank != 0) opt.remove("verbosity");
    timer = MPI_Wtime( );
    if (!excluded)
  ...
  class renumber_Op : public E_F0mps {
   public:
    ...
    static basicAC_F0::name_and_type name_param[];
    Expression nargs[n_name_param];
    renumber_Op(const basicAC_F0& args, Expression param1, Expression param2, Expression param3)
      : A(param1), Mat(param2), interface(param3) {
      args.SetNameParam(n_name_param, name_param, nargs);
    }
  ...
  class renumber : public OneOperator {
   public:
    renumber( )
      : OneOperator(atype< long >( ), atype< Type* >( ), atype< Matrice_Creuse< K >* >( ),
                    atype< KN< long >* >( )) {}

    E_F0* code(const basicAC_F0& args) const {
      return new renumber_Op< Type, K >(args, t[0]->CastTo(args[0]), t[1]->CastTo(args[1]),
                                        t[2]->CastTo(args[2]));
    }
  };
  template< class Type, class K >
  AnyType renumber_Op< Type, K >::operator( )(Stack stack) const {
    ...
    FEbaseArrayKn< K >* deflation =
      nargs[0] ? GetAny< FEbaseArrayKn< K >* >((*nargs[0])(stack)) : 0;
    ...
    if (deflation && deflation->N > 0) {
      ...
      *ev = new K[deflation->N * deflation->get(0)->n];
      for (int i = 0; i < deflation->N; ++i) {
        ev[i] = *ev + i * deflation->get(0)->n;
        std::copy(static_cast< K* >(*(deflation->get(i))),
                  static_cast< K* >(*(deflation->get(i))) + deflation->get(i)->n, ev[i]);
      }
      ptA->setVectors(ev);
      ptA->Type::super::super::initialize(deflation->N);
    }
    ...
    MatriceMorse< K >* mA =
      static_cast< MatriceMorse< K >* >(&(*GetAny< Matrice_Creuse< K >* >((*Mat)(stack))->A));
    if (mA) {
  ...
  void add( ) {
    ...
    if (std::is_same< K, HPDDM::underlying_type< K > >::value) {
      static_assert(std::is_same< Type< K, S >, HpBdd< K, S > >::value ||
                      std::is_same< Type< K, S >, HpFetiPrec< K, S > >::value,
    ...
    map_type_of_map[make_pair(atype< Type< HPDDM::underlying_type< K >, U >* >( ),
                              atype< K* >( ))] = atype< Type< K, S >* >( );
    ...
    Global.Add("originalNumbering", "(",
               new OneOperator3_< long, Type< K, S >*, KN< K >*, KN< long >* >(originalNumbering));
    addInv< Type< K, S >, InvSubstructuring, KN< K >, K >( );
    Global.Add("statistics", "(",
               new OneOperator1_< bool, Type< K, S >* >(statistics< Type< K, S > >));
    Global.Add("exchange", "(", new exchangeInOut< Type< K, S >, K >);

    if (!exist_type< Pair< K >* >( )) {
      Dcl_Type< Pair< K >* >(InitP< Pair< K > >, Destroy< Pair< K > >);
      map_type_of_map[make_pair(atype< Pair< HPDDM::underlying_type< K > >* >( ), atype< K* >( ))] =
        atype< Pair< K >* >( );
    }
    aType t;
    int r;
