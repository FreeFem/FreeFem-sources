...
template<>
struct MPI_TYPE< Complex > {
  static MPI_Datatype TYPE( ) { return MPI_DOUBLE_COMPLEX; }
};

static std::string analysis[] = {"AMD",
                                 "",
                                 "AMF",
                                 "SCOTCH",
                                 "PORD",
...
class SolveMUMPS_mpi : public VirtualSolver< int, R > {
 public:
  ...
  SolveMUMPS_mpi(HMat &AA, const Data_Sparse_Solver &ds, Stack stack)
    : A(AA), verb(ds.verb), eps(ds.epsilon), tgv(ds.tgv), cn(0), cs(0), rinfog(ds.rinfo),
      infog(ds.info), matrank(ds.master), distributed(ds.master < 0), strategy(ds.strategy) {
