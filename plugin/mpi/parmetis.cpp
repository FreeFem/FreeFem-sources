...
class ParMETIS_Op : public E_F0mps {
 public:
  ...
  ParMETIS_Op(const basicAC_F0& args, Expression param1, Expression param2, Expression param3)
    : part(param1), pTh(param2), lparts(param3) {
...
basicAC_F0::name_and_type ParMETIS_Op< Type, Mesh >::name_param[] = {
  {"communicator", &typeid(pcommworld)}, {"worker", &typeid(long)}};
template< class Type, class Mesh >
class ParMETIS : public OneOperator {
 public:
  ParMETIS( )
    : OneOperator(atype< long >( ), atype< KN< Type >* >( ), atype< const Mesh* >( ),
                  atype< long >( )) {}
  E_F0* code(const basicAC_F0& args) const {
    return new ParMETIS_Op< Type, Mesh >(args, t[0]->CastTo(args[0]), t[1]->CastTo(args[1]),
                                         t[2]->CastTo(args[2]));
  }
};
template< class Type, class Mesh >
AnyType ParMETIS_Op< Type, Mesh >::operator( )(Stack stack) const {
  ...
  MPI_Comm comm =
    nargs[0] ? *((MPI_Comm*)GetAny< pcommworld >((*nargs[0])(stack))) : MPI_COMM_WORLD;
  ...
  if (rank < worker) {
    ...
    vtxdist[0] = 0;
    for (int i = 1; i < worker; ++i) vtxdist[i] = vtxdist[i - 1] + n / worker;
    vtxdist[worker] = n;
    ...
    for (int i = 0; i < nparts; ++i) tpwgts[i] = 1.0 / static_cast< real_t >(nparts);
    real_t ubvec = 1.05;
    idx_t* part = ptInt + vtxdist[rank];
    ParMETIS_V3_PartKway(vtxdist, xadg, adjncy.data( ), NULL, NULL, &wgtflag, &wgtflag, &ncon,
                         &nparts, tpwgts, &ubvec, &wgtflag, &edgecut, part, &workComm);
    delete[] tpwgts;
    delete[] xadg;
    delete[] vtxdist;
  }
  MPI_Allreduce(MPI_IN_PLACE, ptInt, n, IDX_T, MPI_SUM, comm);
  for (int i = n; i-- > 0;) pt[i] = ptInt[i];
  if (sizeof(idx_t) > sizeof(Type)) delete[] ptInt;
  if (nargs[1] && workComm != MPI_COMM_NULL) MPI_Comm_free(&workComm);
  return 0L;
}
