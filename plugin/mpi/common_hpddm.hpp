...
template< class K >
K* newCopy(bool mfree, K* p, int n) {
  if (!mfree) return p;
  K* q = new K[n];
  copy(p, p + n, q);
  return q;
...
template< class K >
using MatriceMorse = HashMatrix< int, K >;
...
template< class K >
HPDDM::MatrixCSR< K >* new_HPDDM_MatrixCSR(MatriceMorse< K >* mA, bool mfree = false, K* s = 0,
                                           int* is = 0, int* js = 0) {
  if (mA) {
...
template< class K >
HPDDM::MatrixCSR< void >* new_HPDDM_MatrixCSRvoid(MatriceMorse< K >* mA, bool mfree = false,
                                                  int* is = 0, int* js = 0) {
  if (mA) {
    mA->CSR( );
    if (!js) js = mA->j;
...
void set_ff_matrix(MatriceMorse< K >* mA, const HPDDM::MatrixCSR< K >& dA) {
  // void HashMatrix<I,R>::set(I nn,I mm,bool hhalf,size_t nnnz, I *ii, I*jj, R *aa,,int f77,int
  // tcsr)
...
class STL {
 ...
 public:
  ...
  T* begin( ) const { return _it; }
  T* end( ) const { return _it + _size; }
...
template< class Type, class K,
          typename std::enable_if< HPDDM::hpddm_method_id< Type >::value == 1 >::type* = nullptr >
...
template< class Type, class K,
          typename std::enable_if< HPDDM::hpddm_method_id< Type >::value != 0 >::type* = nullptr >
...
template< class Type, class K,
          typename std::enable_if< HPDDM::hpddm_method_id< Type >::value == 0 >::type* = nullptr >
void exchange(Type* const& pA, KN< K >* pin, bool scaled) {
  if (pA) exchange_dispatched(pA->_A, pin, scaled);
}
template< class Type, class K,
          typename std::enable_if< HPDDM::hpddm_method_id< Type >::value != 0 >::type* = nullptr >
...
namespace PETSc {
  ...
  void changeNumbering_func(Type*, KN< K >*, KN< K >*, bool) {
    ffassert(0);
  }    // Modif FH. Missing function Do Day
}    // namespace PETSc
template< class Type, class K,
          typename std::enable_if< HPDDM::hpddm_method_id< Type >::value == 0 >::type* = nullptr >
void exchange_restriction(Type* const& pA, KN< K >* pin, KN< K >* pout,
                          MatriceMorse< double >* mR) {
  if (pA->_exchange && !pA->_exchange[1]) {
    ffassert((!mR && pA->_exchange[0]->getDof( ) == pout->n) ||
             (mR && mR->n == pin->n && mR->m == pout->n));
...
class exchangeIn_Op : public E_F0mps {
 public:
  ...
  exchangeIn_Op< Type, K >(const basicAC_F0& args, Expression param1, Expression param2)
    : A(param1), in(param2) {
...
template< class Type, class K >
basicAC_F0::name_and_type exchangeIn_Op< Type, K >::name_param[] = {{"scaled", &typeid(bool)}};
template< class Type, class K >
...
class exchangeInOut_Op : public E_F0mps {
 public:
  ...
  static basicAC_F0::name_and_type name_param[];
  Expression nargs[n_name_param];
  exchangeInOut_Op< Type, K >(const basicAC_F0& args, Expression param1, Expression param2,
                              Expression param3)
    : A(param1), in(param2), out(param3) {
    args.SetNameParam(n_name_param, name_param, nargs);
  }
...
basicAC_F0::name_and_type exchangeInOut_Op< Type, K >::name_param[] = {
  {"scaled", &typeid(bool)}, {"restriction", &typeid(Matrice_Creuse< double >*)}};
template< class Type, class K >
class exchangeInOut : public OneOperator {
 public:
  exchangeInOut( )
    : OneOperator(atype< long >( ), atype< Type* >( ), atype< KN< K >* >( ), atype< KN< K >* >( )) {
  }

  E_F0* code(const basicAC_F0& args) const {
    return new exchangeInOut_Op< Type, K >(args, t[0]->CastTo(args[0]), t[1]->CastTo(args[1]),
                                           t[2]->CastTo(args[2]));
  }
};
template< class Type, class K >
AnyType exchangeInOut_Op< Type, K >::operator( )(Stack stack) const {
  ...
  const bool scaled = mpisize > 1 && nargs[0] && GetAny< bool >((*nargs[0])(stack));
  Matrice_Creuse< double >* pR =
    nargs[1] ? GetAny< Matrice_Creuse< double >* >((*nargs[1])(stack)) : nullptr;
...
double getOpt(string* const& ss) { return HPDDM::Option::get( )->val(*ss); }
bool isSetOpt(string* const& ss) { return HPDDM::Option::get( )->set(*ss); }
#endif
template< class Type, class K >
...
class distributedDot : public OneOperator {
 public:
    ...
    E_distributedDot(const basicAC_F0& args, int d) : E( ), c(d) {
      ...
      if (c == 1) {
        ...
        for (int i = 0; i < EA->size( ); ++i)
          E.emplace_back(to< KN< double >* >((*EA)[i]), to< KN< K >* >((*Ex)[i]),
                         to< KN< K >* >((*Ey)[i]));
      } else {
        E.reserve(1);
        E.emplace_back(to< KN< double >* >(args[0]), to< KN< K >* >(args[1]),
                       to< KN< K >* >(args[2]));
      }
    }
  ...
  distributedDot( )
    : OneOperator(atype< K >( ), atype< KN< double >* >( ), atype< KN< K >* >( ),
                  atype< KN< K >* >( )),
      c(0) {}
  distributedDot(int)
    : OneOperator(atype< KN_< K > >( ), atype< E_Array >( ), atype< E_Array >( ),
                  atype< E_Array >( )),
      c(1) {}
};
template< class K >
...
AnyType distributedDot< K >::E_distributedDot::operator( )(Stack stack) const {
  ...
  MPI_Allreduce(MPI_IN_PLACE, dot.data( ), dot.size( ), HPDDM::Wrapper< K >::mpi_type( ), MPI_SUM,
  ...
  if (c == 0) {
    ...
    KN< K >& tab = *ptab;
    for (int i = 0; i < dot.size( ); ++i) tab[i] = dot[i];
...
void addArray( ) {
  ...
  atype< KN< Op >* >( )->Add(
    "[", "", new OneOperator2_< Op*, KN< Op >*, long >(get_elementp_< Op, KN< Op >*, long >));
  Dcl_Type< Resize< KN< Op > > >( );
  Add< KN< Op >* >("resize", ".", new OneOperator1< Resize< KN< Op > >, KN< Op >* >(to_Resize));
  Add< Resize< KN< Op > > >("(", "",
                            new OneOperator2_< KN< Op >*, Resize< KN< Op > >, long >(resizeClean));
...
void parallelIO(string*& name, MPI_Comm* const& comm, bool const& append) {
  ...
  *name = file_without_extension + "_" + str[0].str( ) + "_" + str[1].str( ) + "_" + str[2].str( ) +
          "." + extension;
  if (rank == 0) {
    ...
    for (int t = 0; t < T + 1; ++t)
      for (int i = 0; i < size; ++i) {
        ...
        pvd << "             file=\"";
        pvd << base_filename << "_" << std::setw(6) << std::setfill('0') << t << "_" << std::setw(6)
            << std::setfill('0') << i << "_" << str[2].str( ) << "." << std::setw(0) << extension
            << "\"/>\n";
      }
    pvd << "  </Collection>\n";
    pvd << "</VTKFile>\n";
...
static void Init_Common( ) {
  ...
  if (!zzzfff->InMotClef("pair", t, r)) {
    ...
    for (int i = 0; i < argc; ++i) argv[i] = (*((*pkarg)[i].getap( )))->data( );
