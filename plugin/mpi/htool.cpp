...
class MyMatrix : public IMatrix< K > {
 ...
 public:
  MyMatrix(const FESpace* Uh, const FESpace* Vh)
    : IMatrix< K >(Uh->Th.nv, Vh->Th.nv), ThU(Uh->Th), ThV(Vh->Th) {}

  K get_coef(const int& i, const int& j) const {
    return 1. / (0.01 + (ThU.vertices[i] - ThV.vertices[j]).norme2( ));
  }
};
...
class init : public OneOperator {
 public:
  init( )
    : OneOperator(atype< HMatrix< LR, K >** >( ), atype< HMatrix< LR, K >** >( ),
...
template< template< class > class LR, class K >
class assembleHMatrix : public OneOperator {
 public:
  class Op : public E_F0info {
   public:
    ...
    pcommworld arg(int i, Stack stack, pcommworld a) const {
      return nargs[i] ? GetAny< pcommworld >((*nargs[i])(stack)) : a;
    }
  ...
  assembleHMatrix( )
    : OneOperator(atype< const typename assembleHMatrix< LR, K >::Op* >( ), atype< pfes* >( ),
                  atype< pfes* >( )) {}
...
basicAC_F0::name_and_type assembleHMatrix< LR, K >::Op::name_param[] = {
  {"epsilon", &typeid(double)},      {"eta", &typeid(double)},
  {"minclustersize", &typeid(long)}, {"maxblocksize", &typeid(long)},
  {"mintargetdepth", &typeid(long)}, {"minsourcedepth", &typeid(long)},
  {"comm", &typeid(pcommworld)}};

/*
template <template<class> class LR, class K, EquationEnum Eq, BIOpKernelEnum Op, BIOpKernelEnum
local_Op, int dim, typename Discretization> HMatrix<LR,K>* generateBIO() { double kappa = 1;
...
BIO_Generator<BIOpKernel<Eq,Op,dim+1,Discretization,Discretization>,Discretization>
generator_BIO(dof,kappa); HMatrix<LR,K>* H = new HMatrix<LR,K>(generator_BIO,x);
...
AnyType SetHMatrix(Stack stack, Expression emat, Expression einter, int init) {
  ...
  const typename assembleHMatrix< LR, K >::Op* mi(
    dynamic_cast< const typename assembleHMatrix< LR, K >::Op* >(einter));
  ...
  for (int i = 0; i < ThU.nv; i++) p1[i] = {ThU.vertices[i].x, ThU.vertices[i].y, 0};

  for (int i = 0; i < ThV.nv; i++) p2[i] = {ThV.vertices[i].x, ThV.vertices[i].y, 0};
  ...
  *Hmat = new HMatrix< LR, K >(A, p1, p2, -1, comm);
  ...
  //*Hmat =
  //generateBIO<LR,K,EquationEnum::YU,BIOpKernelEnum::HS_OP,BIOpKernelEnum::SL_OP,2,P1_2D>();
...
AnyType ToDense(Stack stack, Expression emat, Expression einter, int init) {
  ...
  for (int i = 0; i < mdense.nb_rows( ); i++)
    for (int j = 0; j < mdense.nb_cols( ); j++) (*M)(i, j) = mdense(i, j);
...
ostream& operator<<(ostream& out, const std::map< std::string, std::string >& infos) {
  for (std::map< std::string, std::string >::const_iterator it = infos.begin( ); it != infos.end( );
       ++it) {
   ...
   public:
    ...
    AnyType operator( )(Stack stack) const {
      ...
      if (!H || !(*H)) {
        ...
        int* buf =
          new int[4 * (mpirank == 0 ? nbdenseg : nbdense) + 5 * (mpirank == 0 ? nblrg : nblr)];
        ...
        MPI_Gatherv(rankworld == 0 ? MPI_IN_PLACE : buf, recvcounts[rankworld], MPI_INT, buf,
                    recvcounts, displs, MPI_INT, 0, (*H)->get_comm( ));
        ...
        MPI_Gatherv(rankworld == 0 ? MPI_IN_PLACE : buflr, recvcounts[rankworld], MPI_INT, buflr,
                    recvcounts, displs, MPI_INT, 0, (*H)->get_comm( ));
        ...
        MPI_Gatherv(rankworld == 0 ? MPI_IN_PLACE : bufcomp, recvcounts[rankworld], MPI_DOUBLE,
                    bufcomp, recvcounts, displs, MPI_DOUBLE, 0, (*H)->get_comm( ));
...
template< template< class > class LR, class K >
basicAC_F0::name_and_type plotHMatrix< LR, K >::Op::name_param[] = {{"wait", &typeid(bool)}};

template< template< class > class LR, class K >
void add(const char* namec) {
  Dcl_Type< HMatrix< LR, K >** >(Initialize< HMatrix< LR, K >* >, Delete< HMatrix< LR, K >* >);
  Dcl_TypeandPtr< HMatrix< LR, K >* >(0, 0, ::InitializePtr< HMatrix< LR, K >* >,
                                      ::DeletePtr< HMatrix< LR, K >* >);
  ...
  TheOperators->Add(
    "=", new OneOperator2_< HMatrix< LR, K >**, HMatrix< LR, K >**,
                            const typename assembleHMatrix< LR, K >::Op*, E_F_StackF0F0 >(
           SetHMatrix< LR, K, 1 >));
  TheOperators->Add(
    "<-", new OneOperator2_< HMatrix< LR, K >**, HMatrix< LR, K >**,
                             const typename assembleHMatrix< LR, K >::Op*, E_F_StackF0F0 >(
  ...
  // atype<HMatrix<LR ,K>**>()->Add("(","",new OneOperator2_<string*, HMatrix<LR ,K>**,
  // string*>(get_infos<LR,K>));
  Add< HMatrix< LR, K >** >(
    "infos", ".",
    new OneOperator1_< std::map< std::string, std::string >*, HMatrix< LR, K >** >(get_infos));

  Dcl_Type< Prod< KN< K >*, LR, K > >( );
  TheOperators->Add(
    "*", new OneOperator2< Prod< KN< K >*, LR, K >, HMatrix< LR, K >**, KN< K >* >(Build));
  TheOperators->Add("=", new OneOperator2< KN< K >*, KN< K >*, Prod< KN< K >*, LR, K > >(
                           Prod< KN< K >*, LR, K >::mv));
  TheOperators->Add("<-", new OneOperator2< KN< K >*, KN< K >*, Prod< KN< K >*, LR, K > >(
                            Prod< KN< K >*, LR, K >::init));
  ...
  // to dense:
  TheOperators->Add("=",
                    new OneOperator2_< KNM< K >*, KNM< K >*, HMatrix< LR, K >**, E_F_StackF0F0 >(
                      ToDense< LR, K, 1 >));
  TheOperators->Add("<-",
                    new OneOperator2_< KNM< K >*, KNM< K >*, HMatrix< LR, K >**, E_F_StackF0F0 >(
                      ToDense< LR, K, 0 >));
...
static void Init_Schwarz( ) {
  ...
  TheOperators->Add("<<",
                    new OneBinaryOperator< PrintPinfos< std::map< std::string, std::string >* > >);
  Add< std::map< std::string, std::string >* >(
    "[", "",
    new OneOperator2_< string*, std::map< std::string, std::string >*, string* >(get_info));
  ...
  map_type_of_map[make_pair(atype< HMatrix< partialACA, double >** >( ), atype< double* >( ))] =
    atype< HMatrix< partialACA, double >** >( );
  map_type_of_map[make_pair(atype< HMatrix< partialACA, double >** >( ), atype< Complex* >( ))] =
    atype< HMatrix< partialACA, std::complex< double > >** >( );
}
