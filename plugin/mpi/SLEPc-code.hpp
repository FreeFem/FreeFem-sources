...
namespace SLEPc {
  ...
  template< class K, typename std::enable_if< std::is_same< K, double >::value ||
                                              !std::is_same< PetscScalar, double >::value >::type* =
                       nullptr >
  void copy(K* pt, PetscInt n, PetscScalar* xr, PetscScalar* xi) {}
  template< class K,
            typename std::enable_if< !std::is_same< K, double >::value &&
                                     std::is_same< PetscScalar, double >::value >::type* = nullptr >
  void copy(K* pt, PetscInt n, PetscScalar* xr, PetscScalar* xi) {
    for (int i = 0; i < n; ++i) pt[i] = K(xr[i], xi[i]);
  }
  template< class K, typename std::enable_if< std::is_same< K, double >::value ||
                                              !std::is_same< PetscScalar, double >::value >::type* =
                       nullptr >
  ...
  template< class K, typename std::enable_if<
                       (std::is_same< PetscScalar, double >::value &&
                        std::is_same< K, std::complex< double > >::value) >::type* = nullptr >
  void distributedVec(unsigned int* num, unsigned int first, unsigned int last, K* const in,
                      PetscScalar* pt, unsigned int n) {}
  template< class K, typename std::enable_if<
                       !(std::is_same< PetscScalar, double >::value &&
                         std::is_same< K, std::complex< double > >::value) >::type* = nullptr >
  void distributedVec(unsigned int* num, unsigned int first, unsigned int last, K* const in,
  ...
  class eigensolver : public OneOperator {
   public:
    ...
    eigensolver(int)
      : OneOperator(atype< long >( ), atype< Type* >( ), atype< Polymorphic* >( )), c(1) {}
  };
  template< class Type, class K >
      ...
      if (ptA->_petsc) {
        ...
        if (ptA->_ksp)
          ...
          KN< Matrice_Creuse< PetscScalar > >* mS =
            nargs[7] ? GetAny< KN< Matrice_Creuse< PetscScalar > >* >((*nargs[7])(stack)) : 0;
        ...
        FEbaseArrayKn< K >* eigenvectors =
          nargs[3] ? GetAny< FEbaseArrayKn< K >* >((*nargs[3])(stack)) : nullptr;
        Vec* basis = nullptr;
        PetscInt n = 0;
        if (eigenvectors) {
          ffassert(!isType);
          if (eigenvectors->N > 0 && eigenvectors->get(0) && eigenvectors->get(0)->n > 0) {
            ...
            for (int i = 0; i < n; ++i) {
              ...
              if (!(std::is_same< PetscScalar, double >::value &&
                    std::is_same< K, std::complex< double > >::value))
                distributedVec(ptA->_num, ptA->_first, ptA->_last,
                               static_cast< K* >(*(eigenvectors->get(i))), pt,
                               eigenvectors->get(i)->n);
              VecRestoreArray(basis[i], &pt);
            }
          }
          eigenvectors->resize(0);
        }
        if (n) EPSSetInitialSpace(eps, n, basis);
        EPSSolve(eps);
        for (int i = 0; i < n; ++i) VecDestroy(&basis[i]);
        delete[] basis;
        PetscInt nconv;
        ...
        if (nconv > 0 && (nargs[2] || nargs[3] || nargs[4])) {
          ...
          if (eigenvalues) eigenvalues->resize(nconv);
          if (eigenvectors && !isType) eigenvectors->resize(nconv);
          if (array)
          ...
          for (PetscInt i = 0; i < nconv; ++i) {
            PetscScalar kr, ki;
            EPSGetEigenpair(eps, i, &kr, &ki, (eigenvectors || array) ? xr : NULL,
                            (eigenvectors || array) && std::is_same< PetscScalar, double >::value &&
                                std::is_same< K, std::complex< double > >::value
                              ? xi
                              : NULL);
            if (eigenvectors || array) {
              ...
              if (!isType && ptA->_A) {
                ...
                cpy = K(0.0);
                HPDDM::Subdomain< K >::template distributedVec< 1 >(
                  ptA->_num, ptA->_first, ptA->_last, static_cast< K* >(cpy), pt, cpy.n, 1);
                ptA->_A->HPDDM::template Subdomain< PetscScalar >::exchange(static_cast< K* >(cpy));
                if (eigenvectors) eigenvectors->set(i, cpy);
                if (array && !codeA) (*array)(':', i) = cpy;
              }
  ...
  static PetscErrorCode MatMult_User(Mat A, Vec x, Vec y) {
    ...
    CHKERRQ(ierr);
    typename SLEPc::eigensolver< Type, K >::MatF_O* mat =
      reinterpret_cast< typename SLEPc::eigensolver< Type, K >::MatF_O* >(user->mat);
  ...
  void addSLEPc( ) {
    Global.Add("EPSSolveComplex", "(",
               new SLEPc::eigensolver< PETSc::DistributedCSR< HpSchwarz< PetscScalar > >,
                                       std::complex< double > >( ));
    Global.Add("EPSSolveComplex", "(",
               new SLEPc::eigensolver< PETSc::DistributedCSR< HpSchwarz< PetscScalar > >,
...
static void Init( ) {
  ...
  if (!zzzfff->InMotClef(mmmm, t, r)) {
    ...
    if (!isInitialized && mpirank == 0)
      std::cout
        << "PetscInitialize has not been called, do not forget to load PETSc before loading SLEPc"
        << std::endl;
    ...
    Global.Add(
      "EPSSolve", "(",
      new SLEPc::eigensolver< PETSc::DistributedCSR< HpSchwarz< PetscScalar > >, PetscScalar >( ));
    Global.Add(
      "EPSSolve", "(",
      new SLEPc::eigensolver< PETSc::DistributedCSR< HpSchwarz< PetscScalar > >, PetscScalar >(1));
    if (verbosity > 1)
      cout << "*** End:: load PETSc & SELPc " << typeid(PetscScalar).name( ) << "\n\n" << endl;
    zzzfff->Add(mmmm, atype< Dmat* >( ));
  } else {
    if (verbosity > 1)
      cout << "*** reload and skip load PETSc & SELPc " << typeid(PetscScalar).name( ) << "\n\n"
           << endl;
  }
}
#else
static void Init( ) { Init_PETSc( ); }
#endif
