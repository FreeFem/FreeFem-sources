...
namespace Schwarz {
   ...
   public:
    ...
    initDDM( )
      : OneOperator(atype< Type* >( ), atype< Type* >( ), atype< Matrice_Creuse< K >* >( ),
                    atype< KN< KN< long > >* >( ), atype< KN< HPDDM::underlying_type< K > >* >( )),
        c(0) {}
    initDDM(int)
      : OneOperator(atype< Type* >( ), atype< Type* >( ), atype< long >( ),
                    atype< KN< KN< long > >* >( ), atype< KN< HPDDM::underlying_type< K > >* >( )),
        c(1) {}
    initDDM(int, int)
      : OneOperator(atype< Type* >( ), atype< Type* >( ), atype< Matrice_Creuse< K >* >( )), c(2) {}
    initDDM(int, int, int)
      : OneOperator(atype< Type* >( ), atype< Type* >( ), atype< long >( )), c(3) {}
  };
  template< class Type, class K >
  basicAC_F0::name_and_type initDDM< Type, K >::E_initDDM::name_param[] = {
    {"communicator", &typeid(pcommworld)}, {"scaled", &typeid(bool)}};
  template< class Type, class K >
  AnyType initDDM< Type, K >::E_initDDM::operator( )(Stack stack) const {
    ...
    if (c == 0 || c == 1) {
      ...
      KN< HPDDM::underlying_type< K > >* ptD =
        GetAny< KN< HPDDM::underlying_type< K > >* >((*D)(stack));
      if (ptR) {
        KN_< KN< long > > sub(ptR->n > 0 && ptR->operator[](0).n > 0 ? (*ptR)(FromTo(1, ptR->n - 1))
                                                                     : KN< KN< long > >( ));
        ptA->HPDDM::template Subdomain< K >::initialize(
          dA, STL< long >(ptR->n > 0 ? ptR->operator[](0) : KN< long >( )), sub,
          nargs[0] ? (MPI_Comm*)GetAny< pcommworld >((*nargs[0])(stack)) : 0);
      }
      if (ptD)
        ptA->initialize(*ptD);
      else
        std::cerr << "Something is really wrong here!" << std::endl;
      if (c == 0 && (!nargs[1] || GetAny< bool >((*nargs[1])(stack)))) ptA->exchange( );
    } else {
      const MPI_Comm& comm = MPI_COMM_SELF;
      ptA->HPDDM::template Subdomain< K >::initialize(
        dA, STL< long >(KN< long >( )), KN< KN< long > >( ), const_cast< MPI_Comm* >(&comm));
    }
    return ptA;
  }
   ...
   public:
    ...
    class E_attachCoarseOperator : public E_F0mps {
     public:
      ...
      static basicAC_F0::name_and_type name_param[];
      Expression nargs[n_name_param];
      E_attachCoarseOperator(const basicAC_F0& args, int d)
        : A(0), comm(0), codeC(0), codeMatC(0), c(d) {
        args.SetNameParam(n_name_param, name_param, nargs);
        ...
        if (c == 1) {
          ...
          if (!codeMatC) codeC = op->Find("(", ArrayOfaType(atype< KN< K >* >( ), false));
        }
      }
    ...
    attachCoarseOperator( )
      : OneOperator(atype< long >( ), atype< pcommworld >( ), atype< Type* >( )), c(0) {}
    attachCoarseOperator(int)
  ...
  AnyType attachCoarseOperator< Type, K >::E_attachCoarseOperator::operator( )(Stack stack) const {
    ...
    if (c == 0) {
      MatriceMorse< K >* mA = nargs[0] ? static_cast< MatriceMorse< K >* >(&(
                                           *GetAny< Matrice_Creuse< K >* >((*nargs[0])(stack))->A))
                                       : 0;
      Pair< K >* pair = nargs[5] ? GetAny< Pair< K >* >((*nargs[5])(stack)) : 0;
      FEbaseArrayKn< K >* deflation =
        nargs[6] ? GetAny< FEbaseArrayKn< K >* >((*nargs[6])(stack)) : 0;
      HPDDM::Option& opt = *HPDDM::Option::get( );
      KN< double >* timing = nargs[4] ? GetAny< KN< double >* >((*nargs[4])(stack)) : 0;
      std::pair< MPI_Request, const K* >* ret = nullptr;
      if (mA) {
        ff_HPDDM_MatrixCSR< K > dA(
          mA);    //->n, mA->m, mA->nbcoef, mA->a, mA->lg, mA->cl, mA->symetrique);
        MatriceMorse< K >* mB = nargs[1]
                                  ? static_cast< MatriceMorse< K >* >(
                                      &(*GetAny< Matrice_Creuse< K >* >((*nargs[1])(stack))->A))
                                  : nullptr;
        MatriceMorse< K >* mP = nargs[2] && opt.any_of("schwarz_method", {1, 2, 4})
                                  ? static_cast< MatriceMorse< K >* >(
                                      &(*GetAny< Matrice_Creuse< K >* >((*nargs[2])(stack))->A))
                                  : nullptr;
        if (dA._n == dA._m && !deflation) {
          ...
          // mP ? new HPDDM::MatrixCSR<K>(mP->n, mP->m, mP->nbcoef, mP->a, mP->lg, mP->cl,
          // mP->symetrique) : nullptr;
          if (mB) {
            //                 HPDDM::MatrixCSR<K> dB(mB->n, mB->m, mB->nbcoef, mB->a, mB->lg,
            //                 mB->cl, mB->symetrique);
  ...
  AnyType solveDDM_Op< Type, K >::operator( )(Stack stack) const {
    ...
    Type* ptA = GetAny< Type* >((*A)(stack));
    if (ptX->n != ptRHS->n || ptRHS->n < ptA->getDof( )) return 0L;
    HPDDM::Option& opt = *HPDDM::Option::get( );
    ...
    MatriceMorse< K >* mA = nargs[3] ? static_cast< MatriceMorse< K >* >(
                                         &(*GetAny< Matrice_Creuse< K >* >((*nargs[3])(stack))->A))
                                     : 0;
    ...
    if (mpisize > 1 && (mA && opt.any_of(prefix + "schwarz_method", {1, 2, 4}))) {
      ...
      if (!alreadyRenumbered) ptA->callNumfact( );
    ...
    if (excluded) opt[prefix + "level_2_exclude"];
    if (pair)
      if (pair->p) {
    ...
    MPI_Comm comm =
      nargs[4] ? *(MPI_Comm*)GetAny< pcommworld >((*nargs[4])(stack)) : MPI_COMM_WORLD;
    int rank;
    ...
    if (!excluded) {
      ...
      bool allocate =
        map.size( ) > 0 && ptA->getBuffer( )[0] == nullptr ? ptA->setBuffer( ) : false;
  ...
  class changeOperator_Op : public E_F0mps {
   public:
    ...
    static basicAC_F0::name_and_type name_param[];
    Expression nargs[n_name_param];
    changeOperator_Op(const basicAC_F0& args, Expression param1, Expression param2)
      : A(param1), mat(param2) {
      args.SetNameParam(n_name_param, name_param, nargs);
    }
  ...
  AnyType changeOperator_Op< Type, K >::operator( )(Stack stack) const {
    MatriceMorse< K >* mN =
      static_cast< MatriceMorse< K >* >(&(*GetAny< Matrice_Creuse< K >* >((*mat)(stack))->A));
    HPDDM::MatrixCSR< K >* dN = new_HPDDM_MatrixCSR< K >(
      mN);    // mN->n, mN->m, mN->nbcoef, mN->a, mN->lg, mN->cl, mN->symetrique);
    Type* ptA = GetAny< Type* >((*A)(stack));
    ptA->setMatrix(dN);
    if (!nargs[0] || GetAny< bool >((*nargs[0])(stack))) ptA->exchange( );
    return 0L;
  }
  ...
  template< class Type, class K >
  basicAC_F0::name_and_type set_Op< Type, K >::name_param[] = {{"sparams", &typeid(string*)},
  ...
  AnyType set_Op< Type, K >::operator( )(Stack stack) const {
    if (nargs[0]) HPDDM::Option::get( )->parse(*(GetAny< string* >((*nargs[0])(stack))));
    if (nargs[1]) {
      Type* ptA = GetAny< Type* >((*A)(stack));
  ...
  class distributedMV_Op : public E_F0mps {
   public:
    ...
    distributedMV_Op< Type, K >(const basicAC_F0& args, Expression param1, Expression param2,
                                Expression param3, Expression param4)
      : A(param1), Mat(param2), in(param3), out(param4) {
  ...
  class distributedMV : public OneOperator {
   public:
    distributedMV( )
      : OneOperator(atype< long >( ), atype< Type* >( ), atype< Matrice_Creuse< K >* >( ),
                    atype< KN< K >* >( ), atype< KN< K >* >( )) {}

    E_F0* code(const basicAC_F0& args) const {
      return new distributedMV_Op< Type, K >(args, t[0]->CastTo(args[0]), t[1]->CastTo(args[1]),
  ...
  class InvSchwarz {
   public:
    ...
    void solve(U out) const {
      if (out->n != u->n || u->n < (*t).getDof( )) return;
      ...
      if (!alreadyRenumbered) (*t).callNumfact( );
#ifndef SUITESPARSESUB
      else
      ...
      if (mpirank != 0) HPDDM::Option::get( )->remove((*t).prefix("verbosity"));
      const auto& map = (*t).getMap( );
      bool allocate =
        map.size( ) > 0 && (*t).getBuffer( )[0] == nullptr ? (*t).setBuffer( ) : false;
   ...
   public:
    ...
    class Operator : public HPDDM::EmptyOperator< R > {
     public:
      ...
      template< bool >
      void apply(const R* const in, R* const out, const unsigned short& mu = 1, R* = nullptr,
                 const unsigned short& = 0) const {
        if (prec.mat)
    ...
    class SchwarzOperator : public HpSchwarz< R, S > {
     public:
      Op& prec;
      SchwarzOperator(HpSchwarz< R, S >* m, Op& p) : prec(p) {
        *reinterpret_cast< HpSchwarz< R, S >* >(this) = *m;
      }
      template< bool >
      void apply(const R* const in, R* const out, const unsigned short& mu = 1, R* = nullptr,
                 const unsigned short& = 0) const {
     ...
     public:
      ...
      E_LCG(const basicAC_F0& args, int d) : Op(0), c(d) {
        ...
        if (c == 0 || c == 2) {
          ...
          ffassert(op);
          A = (c == 0 || c == 1 ? op->Find("(", ArrayOfaType(atype< Kn* >( ), false))
                                : op->Find("(", ArrayOfaType(atype< KNM< R >* >( ), false)));
        } else {
        ...
        if (nargs[0]) {
          ...
          ffassert(op);
          C = (c == 0 || c == 1 ? op->Find("(", ArrayOfaType(atype< Kn* >( ), false))
                                : op->Find("(", ArrayOfaType(atype< KNM< R >* >( ), false)));
        } else
          C = 0;
        if (c == 0 || c == 1) {
      ...
      virtual AnyType operator( )(Stack stack) const {
        ...
        try {
          MPI_Comm comm =
            nargs[3] ? *(MPI_Comm*)GetAny< pcommworld >((*nargs[3])(stack)) : MPI_COMM_WORLD;
          if (nargs[2]) HPDDM::Option::get( )->parse(*(GetAny< string* >((*nargs[2])(stack))));
          if (c == 0 || c == 1) {
            ...
            if (c == 0) {
              ...
              if (nargs[1]) Op.setPrefix(*(GetAny< string* >((*nargs[1])(stack))));
    ...
    IterativeMethod( )
      : OneOperator(atype< long >( ), atype< Polymorphic* >( ), atype< KN< R >* >( ),
        ...
        c(0) {}
    IterativeMethod(int)
      : OneOperator(atype< long >( ), atype< HpSchwarz< R, S >* >( ), atype< KN< R >* >( ),
                    atype< KN< R >* >( )),
        c(1) {}
    IterativeMethod(int, int)
      : OneOperator(atype< long >( ), atype< Polymorphic* >( ), atype< KNM< R >* >( ),
                    atype< KNM< R >* >( )),
        c(2) {}
    IterativeMethod(int, int, int)
      : OneOperator(atype< long >( ), atype< HpSchwarz< R, S >* >( ), atype< KNM< R >* >( ),
                    atype< KNM< R >* >( )),
        c(3) {}
  };
  ...
  void add( ) {
    ...
    TheOperators->Add("=", new OneOperator2_< Type< K, S >*, Type< K, S >*, Type< K, S >* >(
                             Schwarz::changeOperatorSimple< Type< K, S >, K >));
    ...
    Global.Add("destroyRecycling", "(",
               new OneOperator1_< bool, Type< K, S >* >(destroyRecycling< Type< K, S >, K >));
    ...
    Global.Add(
      "globalNumbering", "(",
      new OneOperator2_< long, Type< K, S >*, KN< long >* >(globalNumbering< Type< K, S > >));
