...
class GenericOptimizer
{
	public:
		...
		GenericOptimizer&SetEqualityConstraints () {
			...
			if (econsttol.n == 0) {econsttol.resize(etestv.n); econsttol = 1.e-12;} else {assert(econsttol.n == etestv.n);}
...
template<nlopt::algorithm ALGO> GenericOptimizer &SAOptimizer<ALGO>::SetSubOptimizer (const string &name, bool save) {
	if (!subopt) {
		if (name == "DIRECT") {subopt = new Optimizer<nlopt::GN_DIRECT>(x->n);} else if (name == "DIRECTL") {subopt = new Optimizer<nlopt::GN_DIRECT_L>(x->n);} else if (name == "DIRECTLRand") {subopt = new Optimizer<nlopt::GN_DIRECT_L_RAND>(x->n);} else if (name == "DIRECTNoScal") {subopt = new Optimizer<nlopt::GN_DIRECT_NOSCAL>(x->n);} else if (name == "DIRECTLNoScal") {subopt = new Optimizer<nlopt::GN_DIRECT_L_NOSCAL>(x->n);} else if (name == "DIRECTLRandNoScal") {subopt = new Optimizer<nlopt::GN_DIRECT_L_RAND_NOSCAL>(x->n);} else if (name == "OrigDIRECT") {subopt = new Optimizer<nlopt::GN_ORIG_DIRECT>(x->n);} else if (name == "OrigDIRECTL") {subopt = new Optimizer<nlopt::GN_ORIG_DIRECT_L>(x->n);} else if (name == "StoGO") {subopt = new Optimizer<nlopt::GD_STOGO>(x->n);} else if (name == "StoGORand") {subopt = new Optimizer<nlopt::GD_STOGO_RAND>(x->n);} else if (name == "LBFGS") {subopt = new Optimizer<nlopt::LD_LBFGS>(x->n);} else if (name == "PRAXIS") {subopt = new Optimizer<nlopt::LN_PRAXIS>(x->n);} else if (name == "Var1") {subopt = new Optimizer<nlopt::LD_VAR1>(x->n);} else if (name == "Var2") {subopt = new Optimizer<nlopt::LD_VAR2>(x->n);} else if (name == "TNewton") {subopt = new Optimizer<nlopt::LD_TNEWTON>(x->n);} else if (name == "TNewtonRestart") {subopt = new Optimizer<nlopt::LD_TNEWTON_RESTART>(x->n);} else if (name == "TNewtonPrecond") {subopt = new Optimizer<nlopt::LD_TNEWTON_PRECOND>(x->n);} else if (name == "TNewtonPrecondRestart") {subopt = new Optimizer<nlopt::LD_TNEWTON_PRECOND_RESTART>(x->n);} else if (name == "CRS2") {subopt = new Optimizer<nlopt::GN_CRS2_LM>(x->n);} else if (name == "MMA") {subopt = new Optimizer<nlopt::LD_MMA>(x->n);} else if (name == "COBYLA") {subopt = new Optimizer<nlopt::LN_COBYLA>(x->n);} else if (name == "NEWUOA") {subopt = new Optimizer<nlopt::LN_NEWUOA>(x->n);} else if (name == "NEWUOABound") {subopt = new Optimizer<nlopt::LN_NEWUOA_BOUND>(x->n);} else if (name == "NelderMead") {subopt = new Optimizer<nlopt::LN_NELDERMEAD>(x->n);} else if (name == "Sbplx") {subopt = new Optimizer<nlopt::LN_SBPLX>(x->n);} else if (name == "BOBYQA") {subopt = new Optimizer<nlopt::LN_BOBYQA>(x->n);} else if (name == "ISRES") {subopt = new Optimizer<nlopt::GN_ISRES>(x->n);} else if (name == "SLSQP") {subopt = new Optimizer<nlopt::LD_SLSQP>(x->n);} else {cout << "Warning: unknown or unauthorized optimizer name passed as sub algorithm to " << Info<ALGO>::name << endl;}
	}
			...
			public:
				...
				virtual AnyType operator () (Stack stack)  const {
					...
					if (nargs[17]) {
						...
						if (optim.DF()) {cout << "Warning: in " << optim.Name() << " algorithm - using nGradStored is pointless (no gradient to store in a derivative free context)." << endl;} else if (ALGO == nlopt::LD_SLSQP || ALGO == nlopt::LD_MMA) {cout << "Warning: nGradStored can't be used with " << optim.Name() << ", parameter will be ignored." << endl;}
					}
					...
					if (optim.DF()) {
						...
						if (gradient) {optim.SetObjectiveFunctionGradient(ffcalfunc<Rn>(stack, GradJ, theparam));} else {cout << "Warning: in " << optim.Name() << " algorithm - no objective function gradient has been provided (choose a derivative free algorithm if it is not available)." << endl;}

						if (econst) {
							if (gradeconst) {optim.SetEqualityConstraintGradient(ffcalfunc<Rnm>(stack, EGradEConst, theparam));} else {cout << "Warning: in " << optim.Name() << " algorithm - no equality constraints gradients has been provided." << endl;}
						...
						if (iconst) {
							if (gradiconst) {optim.SetInequalityConstraintGradient(ffcalfunc<Rnm>(stack, EGradIConst, theparam));} else {cout << "Warning: in " << optim.Name() << " algorithm - no inequality constraints gradients has been provided." << endl;}
		...
		class E_NLopt: public E_F0mps
		{
			public:
				...
				virtual AnyType operator () (Stack stack)  const {
					...
					if (nargs[17]) {optim.SetSubOptimizer(*Arg<string *>(17, stack), 0);} else {cout << "Warning: in " << optim.Name() << " algorithm - you have to specify a local optimizer, aboarting optimization (use the subOpt named parameter)." << endl;}
					...
					if (nargs[26]) {
						...
						if (optim.DF()) {cout << "Warning: in " << optim.subopt->Name() << " algorithm - using nGradStored is pointless (no gradient to store in a derivative free context)." << endl;} else if (optim.subopt->Tag() == nlopt::LD_SLSQP || optim.subopt->Tag() == nlopt::LD_MMA) {
					...
					if (optim.subopt) {
						if (optim.subopt->DF()) {
							...
							if (gradient) {optim.SetObjectiveFunctionGradient(ffcalfunc<Rn>(stack, GradJ, theparam));} else {cout << "Warning: in " << optim.Name() << " algorithm - no objective function gradient has been provided (choose a derivative free local search if it is not available)." << endl;}
