...
pRnm readPPM (pRnm const &a, const pstring &imgName) {
	...
	if (verbosity)
		cout << DISP_INFO << " Image size: " << image->sizeX << " x " << image->sizeY << " - Type: " << (int)image->type << endl;
...
static void Load_PPM2RNM () {
	...
	TheOperators->Add("<-", new OneOperator2_<KNM<double>*, KNM<double>*, string*>(&readPPM));
	TheOperators->Add("=", new OneOperator2_<KN<double>*, KN<double>*, KNM<double>*>(seta));
	...
	Global.Add("diffPPM", "(", new OneOperator2_<KNM<double>*, KNM<double>*, KNM<double>*>(&diffPPM));
}
