...
int fvmP1P0 (double q[3][2], double u[2], double c[3], double a[3][3], double where[3]) {	// computes matrix a on a triangle for the Dervieux FVM
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			a[i][j] = 0;
		}
	...
	for (int i = 0; i < 3; i++) {
		...
		if (unL > 0) {a[i][i] += unL; a[ip][i] -= unL;} else {a[i][ip] += unL; a[ip][ip] -= unL;}
