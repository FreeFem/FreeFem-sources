...
ffNLP &ffNLP::BuildMatrixStructures (Level hlvl, Level jlvl, int _mm) {
	if (jlvl != do_nothing && dconstraints) {
		if (jlvl == user_defined) {ffassert(JacStruct.size());} else if ((jlvl == one_evaluation || jlvl == basis_analysis) && dconstraints) {JacStruct.AddMatrix(dconstraints->J(x_start));}
	}

	if (hlvl != do_nothing && hessian) {
		if (hlvl == user_defined) {ffassert(HesStruct.size());} else if (hlvl == one_evaluation || !hessian->NLCHPEnabled()) {
			Rn lms = lambda_start;
			lms = 1.;
...
int ffNLP::FindIndex (const KN<int> &irow, const KN<int> &jcol, int i, int j, int kmin, int kmax) {
	...
	if (abs(kmin - kmax) <= 1) {
		if (ij == ijmin) {return kmin;} else if (ij == ijmax) {return kmax;} else {return -1;}
	} else {
		int knew = (kmin + kmax) / 2;
		Z2 ijnew(irow[knew], jcol[knew]);
		if (ij <= ijnew) {return FindIndex(irow, jcol, i, j, kmin, knew);} else {return FindIndex(irow, jcol, i, j, knew, kmax);}
	}
}
...
bool CheckMatrixVectorPair (const E_Array *mv, bool &order) {
	...
	if (NXOR(t1 == atype<Matrice_Creuse<R> *>(), t2 == atype<Matrice_Creuse<R> *>())) {return false;} else if (NXOR(t1 == atype<Rn *>(), t2 == atype<Rn *>())) {return false;} else {
		...
		class E_Ipopt: public E_F0mps
		{
			...
			public:
				...
				virtual AnyType operator () (Stack stack)  const {
					...
					assert(_optim);
					if (WC && nargs[6]) {_optim->lambda_start = *lag_mul;} else if (WC) {
						_optim->lambda_start.resize(mmm);
						_optim->lambda_start = 1.;
					}

					_optim->sigma_start = 1.;
					if (nargs[21] && nargs[0]) {_optim->lz_start = *l_z;} else if (nargs[0]) {_optim->lz_start.resize(xl.N()); _optim->lz_start = 1.;}

					if (nargs[20] && nargs[1]) {_optim->uz_start = *u_z;} else if (nargs[1]) {_optim->uz_start.resize(xu.N()); _optim->uz_start = 1.;}
					...
					if (nargs[22]) {app->Options()->SetNumericValue("mu_init", GetAny<double>((*nargs[22])(stack)));} else {app->Options()->SetStringValue("mu_strategy", "adaptive");}
...
template<> void FitnessFunctionDatas<no_assumption_f>::operator ()
	...
	if (CompletelyNonLinearConstraints) {ffH = new GeneralSparseMatFunc(stack, Hessian, theparam, objfact, L_m);} else {ffH = new GeneralSparseMatFunc(stack, Hessian, theparam);}
}
