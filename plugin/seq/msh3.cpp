...
int TestElementMesh3(const Mesh3 &Th3) {
  ...
  if (verbosity > 1) {
    cout << "      - hmin =" << hmin << " ,  Bounding Box: " << Pinf << " " << Psup << endl;
  }
  ...
  if (verbosity > 1) {
    cout << "TestSameVertexMesh3 " << hseuil << " size" << Th3.nv << endl;
  }
  ...
  if (verbosity > 1) {
    cout << "NbVertexRecollement " << nv_t << " / "
         << "NbVertex(anc)" << Th3.nv << endl;
  }
  ...
  if (verbosity > 1) {
    cout << "TestSameTetrahedraMesh3 " << hseuil << " size " << Th3.nt << endl;
  }
  ...
  if (verbosity > 1) {
    cout << "NbTetrahedraRecollement " << nt_t << " / "
         << "NbVertex(anc)" << Th3.nt << endl;
  }
  ...
  hseuil = hseuil / 3.;
  if (verbosity > 1) {
    cout << "TestSameTriangleMesh3 " << hseuil << endl;
  }
  ...
  if (verbosity > 1) {
    cout << "NbVertexRecollement " << nbe_t << " / "
         << "NbVertex(anc)" << Th3.nbe << endl;
  }
...
Mesh3 *build_layer(const Mesh &Th2, const int Nmax, const int *tab_Ni, const double *tab_zmin,
  ...
  if (verbosity > 1) {
    cout << "debut :   Som3D_mesh_product_Version_Sommet_mesh_tab( Nmax, tab_Ni, tab_zmin, "
            "tab_zmax, Th2, Th3);   "
         << endl;
  }
...
void Som3D_mesh_product_Version_Sommet_mesh_tab(
  ...
  for (int ii = 0; ii < Th2.neb; ii++) {    // Th2.neb ??
    ...
    int edgebid;
    int ffbid = Th2.BoundaryElement(ii, edgebid);    // ii : number of edge => sortie :: ffbid =
                                                     // numero triangles, edgebid = numero edges
    int j0bid, j1bid;
    Th2.VerticesNumberOfEdge(Th2.t(ffbid), edgebid, j0bid, j1bid);

    // bool ffsens = Th2.SensOfEdge( Th2.t(ffbid), edgebid ); // sens du parcours de la edge
    // correcte ou non
     ...
     * Th2.operator()(K[1]) ); printf("value of vertex edge ( bool sens  ) :: %d--%d \n",  i_ind1,
     * i_ind2 );
     */
    i_ind1 = j0bid;
    i_ind2 = j1bid;
    ...
    for (int jNmax = Nmax - 1; jNmax >= 0; jNmax--) {
      ...
      switch (cas_decoupage) {
        ...
        case 3:
          ...
          // cout << "DiagMax1=" << DiagMax1 << " "<< SommetPrisme[0]<<" " <<SommetPrisme[4] <<
          // endl;
        ...
        case 5:
          ...
          // cout << "DiagMax1=" << DiagMax1 << " "<< SommetPrisme[0]<<" " <<SommetPrisme[5] <<
          // endl;
        ...
        case 6:
          ...
          // cout << "DiagMax1=" << DiagMax1 << " "<< SommetPrisme[1]<<" " <<SommetPrisme[5] <<
          // endl;
...
class listMesh3 {
 public:
  ...
  listMesh3(Stack s, const Mesh3 *th) : lth(Add2StackOfPtr2Free(s, new list< const Mesh3 * >)) {
    lth->push_back(th);
  }

  listMesh3(Stack s, const Mesh3 *tha, const Mesh3 *thb)
    : lth(Add2StackOfPtr2Free(s, new list< const Mesh3 * >)) {
    lth->push_back(tha);
    lth->push_back(thb);
  }

  listMesh3(Stack s, const listMesh3 &l, const Mesh3 *th)
    : lth(Add2StackOfPtr2Free(s, new list< const Mesh3 * >(*l.lth))) {
    lth->push_back(th);
  }
};
// to be modified
Mesh3 *GluMesh3(listMesh3 const &lst) {
  ...
  for (list< const Mesh3 * >::const_iterator i = lth.begin( ); i != lth.end( ); i++) {
    ...
    th0 = &Th3;
    if (verbosity > 1) {
      cout << " determination of hmin : GluMesh3D + " << Th3.nv << " " << Th3.nt << " " << Th3.nbe
           << endl;
    }
    nbt += Th3.nt;
    nbvx += Th3.nv;
...
class listMeshS {
 public:
  ...
  listMeshS(Stack s, const MeshS *th) : lth(Add2StackOfPtr2Free(s, new list< const MeshS * >)) {
    lth->push_back(th);
  }

  listMeshS(Stack s, const MeshS *tha, const MeshS *thb)
    : lth(Add2StackOfPtr2Free(s, new list< const MeshS * >)) {
    lth->push_back(tha);
    lth->push_back(thb);
  }

  listMeshS(Stack s, const listMeshS &l, const MeshS *th)
    : lth(Add2StackOfPtr2Free(s, new list< const MeshS * >(*l.lth))) {
    lth->push_back(th);
  }
};
...
void finalize< Mesh3 >(Mesh3 *(&Th)) {
  if (Th->meshS) {
    if (verbosity > 5) cout << "Build the meshS associated to the mesh3" << endl;
    Th->BuildMeshS( );
  }
}
...
class SetMesh_Op : public E_F0mps {
 ...
 public:
  SetMesh_Op(const basicAC_F0 &args, Expression aa) : a(aa) {
    ...
    if (nargs[0] && nargs[2]) CompileError("uncompatible change(... region= , reftet=  ");
    if (nargs[1] && nargs[3]) CompileError("uncompatible  change(...label= , refface=  ");
  }
...
AnyType SetMesh_Op< MMesh >::operator( )(Stack stack) const {
  ...
  for (int i = 0; i < nrT.N( ); i += 2) mapTref[nrT[i]] = nrT[i + 1];
  int z00 = false;
  ...
  for (int i = 0; i < nbe; i++) {
    ...
    if (!rmf && rm_faces) rmf = !onborder && (l1 == rmlabfaces);
    if (rmf)
      nrmf++;
    else
      (*bb++).set(v, iv, l1);
  ...
  if (nrmf && verbosity > 2)
    cout << "   change  mesh3 : number of removed  internal faces " << nrmf
         << " == " << nbe - (bb - b) << endl;
...
template< class MMesh >
class SetMesh : public OneOperator {
 public:
...
MeshS *MoveMesh2_func(const double &precis_mesh, const Mesh &Th2, const double *tab_XX,
  ...
  if (verbosity > 5) {
    cout << "before movemesh::Vertex  triangle2  border " << Th2.nv << " " << Th2.nt << " "
         << Th2.neb << endl;
  }
  ...
  SamePointElement_Mesh2(precis_mesh, tab_XX, tab_YY, tab_ZZ, Th2, recollement_border,
                         point_confondus_ok, Numero_Som, ind_nv_t, ind_nt_t, ind_nbe_t, label_nt_t,
                         label_nbe_t, nv_t, nt_t, nbe_t);

  if (verbosity > 1) {
    cout << " fin: SamePointElement " << endl;
    cout << "After movemesh::Vertex  triangle  border " << nv_t << " " << nt_t << " " << nbe_t
         << endl;
  }
...
void SamePointElement_Mesh2(const double &precis_mesh, const double *tab_XX, const double *tab_YY,
  ...
  if (recollement_border == 1) {
    ...
    for (int i_elem = 0; i_elem < nt_t; i_elem++) {
      ...
      for (int jj = 0; jj < 3; jj++) iv[jj] = Th2.operator( )(K[jj]);
    ...
    for (int i_elem = 0; i_elem < np; i_elem++) ind_nt_t[i_elem] = ind_nt_t_tmp[i_elem];
  ...
  for (int ii = 0; ii < Th2.neb; ii++) {
    ...
    for (int jj = 0; jj < 2; jj++) iv[jj] = Numero_Som[Th2.operator( )(K[jj])];
  ...
  if (recollement_border == 1) {
    ...
    for (int i_border = 0; i_border < nbe_t; i_border++) {
      int &ii = ind_nbe_t[i_border];
      const Mesh::BorderElement &K(
        Th2.be(ii));    // const Triangle2 & K(Th2.elements[ii]);  // avant Mesh2
      int iv[2];
    ...
    for (int i_border = 0; i_border < np; i_border++) ind_nbe_t[i_border] = ind_nbe_t_tmp[i_border];
...
void BuildBoundMinDist_th3(const double &precis_mesh, const double *tab_XX, const double *tab_YY,
  ...
  if (hmin / longmini_box < 1e7) hmin = hmin * 0.1;    //  in case of bad shape element
  if (verbosity > 5) {
    cout << "    longmini_box" << longmini_box << "    hmin =" << hmin << " longmini_box/hmin "
...
//
//  OH LALA A CHANGE REPIDEMENT ET NE DETRUITE LE COMMENTAIRE QUI VA SUIVRE  F. HECHT CODE EN N¨2
//  MERCI A. Gailliegue qui a trouve..
...
//    }
//
//    if (verbosity > 1) {cout << "      nv_t = " << nv_t << " / " << "nv_t(anc)" << tab_nv <<
//    endl;}
//
...
//                    }
//
//                    ind_np[np] = ii;// min(ii,minimum_np);    // Remarque on donne a np le plus
//                    petit element np++;    // nv_t = nvt+1;
//                }
//            }
//
//            break;
...
//
//                    if (point_multiple == 0) {
//                        ind_np[np] = ii;// min(ii,minimum_np);    // Remarque on donne a np le
//                        plus petit element np++;    // nv_t = nvt+1;
//                    }
//                }
//            }
//
...
class cubeMesh_Op : public E_F0mps {
 ...
 public:
  cubeMesh_Op(const basicAC_F0 &args, Expression nnx, Expression nny, Expression nnz,
              Expression transfo = 0)
    : nx(nnx), ny(nny), nz(nnz), xx(0), yy(0), zz(0) {
    if (verbosity > 1) {
      cout << "construction par cubeMesh_Op" << endl;
    }
...
class cubeMesh : public OneOperator {
 public:
  int xyz;
  cubeMesh( )
    : OneOperator(atype< pmesh3 >( ), atype< long >( ), atype< long >( ), atype< long >( )),
      xyz(0) {}
  ...
  E_F0 *code(const basicAC_F0 &args) const {
    if (xyz) {
      return new cubeMesh_Op(args, t[0]->CastTo(args[0]), t[1]->CastTo(args[1]),
...
AnyType cubeMesh_Op::operator( )(Stack stack) const {
  ...
  for (int it = 0; it < nbt; ++it) {
    ...
    if (ni[i0] == 0 && ni[i1] == 0 && ni[i2] == 0) {
      cout << "A tetrahedra with null volume will be created with triangle " << it << " of 2D Mesh "
           << endl;
  ...
  // cas maillage surfacique simplement // A construire Jacques + donner le numero des edges que
...
AnyType BuildLayeMesh_Op::operator( )(Stack stack) const {
  ...
  double epsz = maxdz * 1e-6;
  if (verbosity > 9999) {
    cout << "BuildLayeMesh_Op:: epsz " << epsz << endl;
  }
  ...
  if (verbosity > 9999) {
    cout << " BuildLayeMesh_Op: ni = " << ni << endl;
  }
  ...
  Mesh3 *Th3 = build_layer(Th, nlayer, ni, zmin, zmax, maptet, maptrimil, maptrizmax, maptrizmin,
                           mapemil, mapezmax, mapezmin);
  // cas maillage surfacique simplement // A construire Jacques + donner le numero des edges que
  // l'on veut pas creer à l'intérieure
  ...
  if (!(xx) && !(yy) && !(zz)) {
     ...
     * build_layer_map_edge( Th, mapemil, mapezmax, mapezmin );
     *
     * Mesh3 *Th3= build_layer(Th, nlayer, ni, zmin, zmax, maptet, maptrimil, maptrizmax,
     * maptrizmin, mapemil, mapezmax, mapezmin);
     */
...
basicAC_F0::name_and_type DeplacementTab_Op::name_param[] = {
  {"deltax", &typeid(KN_< double >)}, {"deltay", &typeid(KN_< double >)},
  {"deltaz", &typeid(KN_< double >)}, {"ptmerge", &typeid(double)},
  {"facemerge", &typeid(long)},       {"boolsurface", &typeid(long)}};
AnyType DeplacementTab_Op::operator( )(Stack stack) const {
  ...
  if (verbosity > 5) {
    cout << "before movemesh: Vertex " << nbv << " Tetrahedra " << nbt << " triangles " << nbe
         << endl;
  }
  ...
  Mesh3 *T_Th3 = Transfo_Mesh3(precis_mesh, Th, txx, tyy, tzz, border_only, recollement_elem,
                               recollement_border, point_confondus_ok, 1);
  T_Th3->BuildGTree( );
...
void GetManifolds(Expression mani, int &nbcmanifold, int *&mani_nbe, Expression *&manifold) {
  if (mani) {
    ...
    for (i = 0; i < n; i++) {
      ...
      cout << "number of manifold = " << n << "manifold i=" << i << "nb BE label=" << mani_nbe[i]
           << endl;
...
class CheckManifoldMesh_Op : public E_F0mps {
 ...
 public:
  CheckManifoldMesh_Op(const basicAC_F0 &args, Expression tth) : eTh(tth) {
    args.SetNameParam(n_name_param, name_param, nargs);
    if (nargs[0]) {
...
AnyType CheckManifoldMesh_Op::operator( )(Stack stack) const {
  ...
  long resultat = 1;
  pTh->BuildBoundaryElementAdj(
    nbmanifold, BeginManifold, TabLabelManifold,
    OrientLabelManifold);    // nbvariete, beginvariete, TabLabelVariete, OrientLabelVariete);
...
struct Op_trunc_meshS : public OneOperator {
  ...
  Op_trunc_meshS( ) : OneOperator(atype< pmeshS >( ), atype< pmeshS >( ), atype< bool >( )){};
};
...
Mesh3 *truncmesh(const Mesh3 &Th, const long &kksplit, int *split, bool kk, const int newbelabel) {
  ...
  if (verbosity > 10) {
    cout << "    -- nvertex  " << nvtrunc << ", nedges = " << nedge << ", nfaces = " << nface
         << " ntet =" << ntsplit << endl
         << "    -- Euler/Poincare constante = " << nvtrunc - nedge + nface - ntsplit << endl;
  }
  ...
  nv = ntnosplit * (nvsub - 4 * ((kksplit + 1) * (kksplit + 2) / 2 - 3 * (kksplit - 1) - 3) -
                    6 * (kksplit - 1) - 4);
  if (verbosity > 100) {
    cout << "       1) nv= " << nv << endl;
  }
  ...
  for (int ibe = 0; ibe < Th.nbe; ibe++) {
    ...
    for (int iv = 0; iv < nv2Dsub; iv++) {
      ...
      vertextrisub[iv].x = (1 - alpha - beta) * Th.vertices[ivv[0]].x +
                           alpha * Th.vertices[ivv[1]].x + beta * Th.vertices[ivv[2]].x;
      vertextrisub[iv].y = (1 - alpha - beta) * Th.vertices[ivv[0]].y +
                           alpha * Th.vertices[ivv[1]].y + beta * Th.vertices[ivv[2]].y;
      vertextrisub[iv].z = (1 - alpha - beta) * Th.vertices[ivv[0]].z +
                           alpha * Th.vertices[ivv[1]].z + beta * Th.vertices[ivv[2]].z;
    }
...
long BuildBoundaryElementAdj(const MeshS &Th, bool check = 0, KN< long > *pborder = 0) {
    ...
    for (int i = 0; i < T::nea; ++i) {
      ...
      if (!p) {
        ...
        if (sens * TheElementAdjacencesLink[p->v] > 0) {
          ...
          if (err < 100 && check) {
            cout << " The edges defined by vertex is " << firstVertex << "-" << secondVertex
                 << ", is oriented in the same direction in element " << k + 1 << " and in element "
                 << 1 + (p->v / T::nea) << endl;
          }
        ...
        if (abs(TheElementAdjacencesLink[p->v]) != 1 + p->v) {
          ...
          if (err3 < 100 && check) {
            cout << " The edges defined by vertex is " << firstVertex << "-" << secondVertex
                 << "belong to the three border elements ::" << 1 + (p->v) / T::nea << ", " << k + 1
                 << " and " << 1 + (abs(TheElementAdjacencesLink[p->v]) - 1) / T::nea << endl;
  ...
  if (err && err3 && check) {
    ExecError(" The surface mesh in no manifold ");
  }
...

  {
    ...
    if (verbosity > 3) {
      cout << " nb curve in boundary of manifold  = " << nc << " " << pborder << endl;
    }
  ...
  if (verbosity) {
    cout << "number of adjacents edges " << nk << " nb border edge :" << nb << " " << nc << endl;
  }
...
long GetBorder(const MeshS *pth, KN< long > *pb) { return BuildBoundaryElementAdj(*pth, 0, pb); }

// template<class Mesh_t>
long ShowBorder(const MeshS *pth) { return BuildBoundaryElementAdj(*pth); }
...
const int simplexes[nbsimplexes][4] = {
  {/* -   0 */ 5, 3, 6, 7}, {/* -   1 */ 5, 2, 6, 7}, {/* -   2 */ 5, 1, 6, 7},
  {/* -   3 */ 5, 0, 6, 7}, {/* -   4 */ 4, 3, 6, 7}, {/* -   5 */ 4, 2, 6, 7},
  {/* -   6 */ 4, 1, 6, 7}, {/* -   7 */ 4, 0, 6, 7}, {/* +   8 */ 1, 3, 6, 7},
  {/* +   9 */ 0, 3, 6, 7}, {/* +  10 */ 1, 2, 6, 7}, {/* +  11 */ 0, 2, 6, 7},
  {/* +  12 */ 3, 4, 5, 7}, {/* +  13 */ 2, 4, 5, 7}, {/* +  14 */ 1, 4, 5, 7},
  {/* +  15 */ 0, 4, 5, 7}, {/* -  16 */ 3, 2, 5, 7}, {/* -  17 */ 3, 0, 5, 7},
  {/* +  18 */ 1, 2, 5, 7}, {/* -  19 */ 1, 0, 5, 7}, {/* -  20 */ 3, 2, 4, 7},
  {/* +  21 */ 1, 3, 4, 7}, {/* +  22 */ 1, 2, 4, 7}, {/* +  23 */ 0, 2, 4, 7},
  {/* -  24 */ 1, 0, 4, 7}, {/* -  25 */ 2, 1, 3, 7}, {/* -  26 */ 2, 0, 3, 7},
  {/* +  27 */ 0, 1, 3, 7}, {/* +  28 */ 0, 1, 2, 7}, {/* +  29 */ 3, 4, 5, 6},
  {/* +  30 */ 2, 4, 5, 6}, {/* +  31 */ 1, 4, 5, 6}, {/* +  32 */ 0, 4, 5, 6},
  {/* -  33 */ 3, 2, 5, 6}, {/* -  34 */ 3, 1, 5, 6}, {/* -  35 */ 3, 0, 5, 6},
  {/* -  36 */ 2, 0, 5, 6}, {/* -  37 */ 1, 0, 5, 6}, {/* -  38 */ 3, 2, 4, 6},
  {/* -  39 */ 3, 0, 4, 6}, {/* +  40 */ 1, 2, 4, 6}, {/* -  41 */ 1, 0, 4, 6},
  {/* -  42 */ 2, 1, 3, 6}, {/* -  43 */ 2, 0, 3, 6}, {/* +  44 */ 0, 1, 3, 6},
  {/* +  45 */ 0, 1, 2, 6}, {/* +  46 */ 1, 3, 4, 5}, {/* +  47 */ 0, 3, 4, 5},
  {/* +  48 */ 1, 2, 4, 5}, {/* +  49 */ 0, 2, 4, 5}, {/* -  50 */ 2, 1, 3, 5},
  {/* -  51 */ 2, 0, 3, 5}, {/* +  52 */ 0, 1, 3, 5}, {/* +  53 */ 0, 1, 2, 5},
  {/* -  54 */ 2, 1, 3, 4}, {/* -  55 */ 2, 0, 3, 4}, {/* +  56 */ 0, 1, 3, 4},
  {/* +  57 */ 0, 1, 2, 4}};
const int nhex_decoupe = 74;
const int hex_decoupe[74][7] = {    // 74
  {/* 1 */ 5, 57, 22, 25, 14, 5, -1},  {/* 2 */ 5, 32, 35, 52, 43, 0, -1},
  {/* 1 */ 6, 57, 54, 46, 29, 38, 0},  {/* 2 */ 6, 57, 54, 46, 12, 20, 5},
  {/* 3 */ 6, 57, 54, 46, 12, 4, 38},  {/* 4 */ 6, 57, 54, 21, 14, 20, 5},
  {/* 5 */ 6, 57, 54, 21, 14, 4, 38},  {/* 6 */ 6, 57, 48, 50, 33, 30, 0},
  {/* 7 */ 6, 57, 48, 50, 16, 13, 5},  {/* 8 */ 6, 57, 48, 50, 16, 1, 30},
  {/* 9 */ 6, 57, 48, 18, 25, 13, 5},  {/* 10 */ 6, 57, 48, 18, 25, 1, 30},
  {/* 11 */ 6, 57, 40, 42, 34, 31, 0}, {/* 12 */ 6, 57, 40, 42, 8, 6, 14},
  {/* 13 */ 6, 57, 40, 42, 8, 2, 31},  {/* 14 */ 6, 57, 40, 10, 25, 6, 14},
  {/* 15 */ 6, 57, 40, 10, 25, 2, 31}, {/* 16 */ 6, 55, 56, 46, 29, 38, 0},
  {/* 17 */ 6, 55, 56, 46, 12, 20, 5}, {/* 18 */ 6, 55, 56, 46, 12, 4, 38},
  {/* 19 */ 6, 55, 56, 21, 14, 20, 5}, {/* 20 */ 6, 55, 56, 21, 14, 4, 38},
  {/* 21 */ 6, 55, 47, 52, 29, 38, 0}, {/* 22 */ 6, 55, 47, 52, 12, 20, 5},
  {/* 23 */ 6, 55, 47, 52, 12, 4, 38}, {/* 24 */ 6, 49, 53, 50, 33, 30, 0},
  {/* 25 */ 6, 49, 53, 50, 16, 13, 5}, {/* 26 */ 6, 49, 53, 50, 16, 1, 30},
  {/* 27 */ 6, 49, 53, 18, 25, 13, 5}, {/* 28 */ 6, 49, 53, 18, 25, 1, 30},
  {/* 29 */ 6, 49, 51, 52, 33, 30, 0}, {/* 30 */ 6, 49, 51, 52, 16, 13, 5},
  {/* 31 */ 6, 49, 51, 52, 16, 1, 30}, {/* 32 */ 6, 41, 45, 42, 34, 31, 0},
  {/* 33 */ 6, 41, 45, 42, 8, 6, 14},  {/* 34 */ 6, 41, 45, 42, 8, 2, 31},
  {/* 35 */ 6, 41, 45, 10, 25, 6, 14}, {/* 36 */ 6, 41, 45, 10, 25, 2, 31},
  {/* 37 */ 6, 41, 44, 43, 34, 31, 0}, {/* 38 */ 6, 41, 44, 43, 8, 6, 14},
  {/* 39 */ 6, 41, 44, 43, 8, 2, 31},  {/* 40 */ 6, 39, 56, 46, 29, 0, 43},
  {/* 41 */ 6, 39, 56, 46, 12, 4, 43}, {/* 42 */ 6, 39, 56, 21, 14, 4, 43},
  {/* 43 */ 6, 39, 47, 52, 29, 0, 43}, {/* 44 */ 6, 39, 47, 52, 12, 4, 43},
  {/* 45 */ 6, 32, 37, 45, 42, 34, 0}, {/* 46 */ 6, 32, 37, 45, 42, 8, 2},
  {/* 47 */ 6, 32, 37, 45, 10, 25, 2}, {/* 48 */ 6, 32, 37, 44, 43, 34, 0},
  {/* 49 */ 6, 32, 37, 44, 43, 8, 2},  {/* 50 */ 6, 32, 36, 53, 50, 33, 0},
  {/* 51 */ 6, 32, 36, 53, 50, 16, 1}, {/* 52 */ 6, 32, 36, 53, 18, 25, 1},
  {/* 53 */ 6, 32, 36, 51, 52, 33, 0}, {/* 54 */ 6, 32, 36, 51, 52, 16, 1},
  {/* 55 */ 6, 32, 3, 19, 28, 11, 25}, {/* 56 */ 6, 32, 3, 19, 27, 26, 11},
  {/* 57 */ 6, 32, 3, 19, 27, 9, 43},  {/* 58 */ 6, 32, 3, 17, 52, 26, 11},
  {/* 59 */ 6, 32, 3, 17, 52, 9, 43},  {/* 60 */ 6, 23, 28, 24, 14, 25, 5},
  {/* 61 */ 6, 23, 28, 19, 15, 25, 5}, {/* 62 */ 6, 23, 26, 27, 24, 14, 5},
  {/* 63 */ 6, 23, 26, 27, 19, 15, 5}, {/* 64 */ 6, 23, 26, 17, 52, 15, 5},
  {/* 65 */ 6, 7, 24, 28, 11, 25, 14}, {/* 66 */ 6, 7, 24, 27, 26, 11, 14},
  {/* 67 */ 6, 7, 24, 27, 9, 43, 14},  {/* 68 */ 6, 7, 15, 19, 28, 11, 25},
  {/* 69 */ 6, 7, 15, 19, 27, 26, 11}, {/* 70 */ 6, 7, 15, 19, 27, 9, 43},
  {/* 71 */ 6, 7, 15, 17, 52, 26, 11}, {/* 72 */ 6, 7, 15, 17, 52, 9, 43}};
...
class Cube_Op : public E_F0mps {
 ...
 public:
  Cube_Op(const basicAC_F0 &args, Expression nx, Expression ny, Expression nz,
          Expression transfo = 0)
...
class Square : public OneOperator {
 public:
  ...
  Square(int)
    : OneOperator(atype< pmeshS >( ), atype< long >( ), atype< long >( ), atype< E_Array >( )),
      cas(1) {}
...
Mesh3 *BuildCube(long nx, long ny, long nz, long region, long *label, long kind,
  ...
  for (int i = 0; i < 64; ++i) {
    ...
    if (d[0] == d[1] && d[2] == d[3] && d[4] == d[5]) {    // stable
      if (debug > 3) {
        cout << " code stable " << i << " n " << kstable << endl;
      }
  ...
  int df[74];    // decoupe des 6  du cube
  {
    ...
    for (int c = 0; c < 2; ++c) {
      for (int b = 0; b < 2; ++b) {
        for (int a = 0; a < 2; ++a) {
          l8[a + 2 * b + 4 * c] = 1 * (a == 0) + 2 * (a == 1) + 4 * (b == 0) + 8 * (b == 1) +
                                  16 * (c == 0) + 32 * (c == 1);
        }
      }
    }
  ...
  int nc = (nx) * (ny) * (nz);
  int nv = (nx + 1) * (ny + 1) * (nz + 1), nt = nc * ntetcube,
      nbe = 4 * (nx * ny + nx * nz + ny * nz);
  int nj = nx + 1;
  int nk = nj * (ny + 1);
  ...
  for (int p = 0, k = 0; k <= nz; ++k) {
    for (int j = 0; j <= ny; ++j) {
      for (int i = 0; i <= nx; ++i, ++p) {
        ...
        vff[p].y = y0 + yd * j;
        vff[p].z = z0 + zd * k;
        vff[p].lab = 1 * (i == 0) + 2 * (i == nx) + 4 * (j == 0) + 8 * (j == ny) + 16 * (k == 0) +
                     32 * (k == nz);
        if (tf) {
          (R3 &)vff[p] = tf->eval(vff[p]);
        }
    ...
    for (int j = 0; j < ny; ++j) {
      for (int i = 0; i < nx; ++i) {
        ...
        for (int d = 0; d < ntetcube; ++d) {
          ...
          if (debug > 4) {
            cout << " tet = " << p << " " << nu[0] << " " << nu[1] << " " << nu[2] << " " << nu[3]
                 << " / " << kt << " " << tff[p].mesure( ) << endl;
          }
...
AnyType Cube_Op::operator( )(Stack stack) const {
  ...
  int renumsurf = 0;
  if (nargs[0]) {
    region = GetAny< long >((*nargs[0])(stack));
  }

  if (nargs[2]) {
    kind = GetAny< long >((*nargs[2])(stack));
  }
...
AnyType Square_Op::operator( )(Stack stack) const {
  ...
  takemesh = 0;
  if (verbosity > 2)
    cout << "  -- SquareMesh_Op input: nv" << Th.nv << "  nt: " << Th.nt << " nbe " << Th.neb
         << endl;
  typedef typename MeshS::Element T;
  typedef typename MeshS::BorderElement B;
  typedef typename MeshS::Vertex V;
  ...
  delete pTh;
  if (verbosity > 2) cout << "  -- SquareMesh_Op: end movemesh23 " << endl;
...
AnyType BuildMeshS_Op::operator( )(Stack stack) const {
  ...
  if (Th.meshS) {
    ...
    if (verbosity > 5)
      cout << "copy the original mesh3... nv= " << nv << " nt= " << nt << " nbe= " << nbe << endl;
...
AnyType Movemesh_Op< Mesh >::operator( )(Stack stack) const {
  ...
  for (int it = 0; it < Th.nt; it++)
    for (int iv = 0; iv < 3; iv++) {
      ...
      if (takemesh[i] == 0) {
        mpp->setP(&Th, it, iv);
        if (xx) vS[i].x = GetAny< double >((*xx)(stack));
        if (yy) vS[i].y = GetAny< double >((*yy)(stack));
        if (zz) vS[i].z = GetAny< double >((*zz)(stack));
 ...
 public:
  ...
  E_F0 *code(const basicAC_F0 &args) const {
    if (cas == 0) {
      ...
      ffassert(a);
      if (a->size( ) != 3) {
        CompileError("movemesh(Th,[X,Y,Z],...) need 3 componates in array ", atype< pmesh >( ));
      }
...
class CheckMesh_Op : public E_F0mps {
 ...
 public:
  CheckMesh_Op(const basicAC_F0 &args, Expression tth) : eTh(tth) {
    args.SetNameParam(n_name_param, name_param, nargs);
  }
...
static void Load_Init( ) {
  ...
  TheOperators->Add(
    "=", new OneBinaryOperator_st< Op3_setmesh< false, pmesh3 *, pmesh3 *, listMesh3 > >);
  TheOperators->Add("<-",
                    new OneBinaryOperator_st< Op3_setmesh< true, pmesh3 *, pmesh3 *, listMesh3 > >);
