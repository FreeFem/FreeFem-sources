...
class PLY_LoadMeshS_Op : public E_F0mps {
public:
    Expression filename;
...
public:
    PLY_LoadMeshS_Op(const basicAC_F0 &args, Expression ffname) : filename(ffname) {
        if (verbosity) {
...
basicAC_F0::name_and_type PLY_LoadMeshS_Op::name_param[] = {
    {"swap", &typeid(bool)}, {"cleanmesh", &typeid(bool)},
    {"removeduplicate", &typeid(bool)}, {"precisvertice", &typeid(double)}};

class PLY_LoadMeshS : public OneOperator {
public:
    PLY_LoadMeshS( ) : OneOperator(atype< pmeshS >( ), atype< string * >( )) {}
    
    E_F0 *code(const basicAC_F0 &args) const {
        return new PLY_LoadMeshS_Op(args, t[0]->CastTo(args[0]));
    }
};

MeshS *PLY_LoadS(const string &filename, bool bigEndian, bool cleanmesh, bool removeduplicate,
    ...
    while (!end_header) {
        ...
        if (!strcmp(buffer, "element")) {
            ...
            {fscanf(fp, "%d", &nt); haveElem=true;}
            else if (!strcmp(buffer, "edge"))
            {fscanf(fp, "%d", &nbe);haveBdElem=true;}
        }
    ...
    // read vertex
    if (verbosity > 3)
        cout << "Reading " << nv << " points, " << nt << " triangles, " << nbe  << " border element " << endl;
    ...
    int label[nt], ivt[3], ivb[2], color[3];
    for (int it = 0; it < nt; ++it) {
        ...
        if (binary) {
            if (indexVertFace && (fread(&numVerts, sizeof(char), 1, fp) != 1)) {
                cout << "error in reading PLY file (triangle)" << endl;
                ExecError("error in reading PLY file (triangle)");
            }
            ...
            if (fread(ivt, sizeof(int), 3, fp) != numVerts) {
                cout << "error in reading PLY file (triangle)" << endl;
                ExecError("error in reading PLY file (triangle)");
            }
            if(verbosity>5)
           ...
           if (havelabElem) {
                ...
                if( fread(&label[it], sizeof(int), 1, fp) != 1) {
                    cout << "error in reading PLY file (lab triangle) " << endl;
                    ExecError("error in reading PLY file (lab triangle)");
                }
                if (swap)
            ...
            if (haveTransparencyVert) {
                ...
                if (indexVertFace && (fscanf(fp, "%d", &numVerts) != 1) ) {
                    ...
                    cout << "error in reading PLY file (numVerts)" << endl;
                    ExecError("error in reading PLY file (numVerts)");
                }
                if(verbosity>5)
    ...
    for (int ibe = 0; ibe < nbe; ++ibe) {
        ...
        fclose(fp);
        MeshS *pTh = new MeshS(nv, nt, 0, vff, tff, 0, cleanmesh, removeduplicate, precisvertice);
        return pTh;
    }
    
    
    
    AnyType PLY_LoadMeshS_Op::operator( )(Stack stack) const {
        ...
        MeshS *Th = PLY_LoadS(*pffname, swap, cleanmesh, removeduplicate, precisvertice);
        Add2StackOfPtr2FreeRC(stack, Th);
        return Th;
    }
    
    
    class PLY_WriteMeshS_Op : public E_F0mps {
    public:
        typedef long Result;
        Expression eTh;
        Expression filename;
    ...
    public:
        PLY_WriteMeshS_Op(const basicAC_F0 &args) {
            
            if (verbosity > 2)
                cout << "Write Mesh in PLY Formats" << endl;
            args.SetNameParam(n_name_param, name_param, nargs);
            if (BCastTo< string * >(args[0]))
                filename = CastTo< string * >(args[0]);
            if (BCastTo< pmeshS >(args[1]))
                eTh = CastTo< pmeshS >(args[1]);
        }
        
        static ArrayOfaType typeargs( ) {
            return ArrayOfaType(atype< string * >( ), atype< pmeshS >( ), true);
        }
        static E_F0 *f(const basicAC_F0 &args) { return new PLY_WriteMeshS_Op(args); }
    ...
    basicAC_F0::name_and_type PLY_WriteMeshS_Op::name_param[] = {
        {"floatmesh", &typeid(bool)}, {"bin", &typeid(bool)}};
    
    
    void PLY_WRITE_MESHS(const string &filename, FILE *fp, const MeshS &Th, bool binary, int datasize,
                         bool bigEndian) {
    ...
    AnyType PLY_WriteMeshS_Op::operator( )(Stack stack) const {
        string *pffname = GetAny< string * >((*filename)(stack));
        MeshS *pThS = GetAny< MeshS * >((*eTh)(stack));
        ffassert(pThS);
        MeshS &ThS = *pThS;
        ...
        PLY_WRITE_MESHS(*pffname, fp, ThS, binary, datasize, swap);
        return (MeshS *)NULL;
    }
    ...
    static void Load_Init( ) {
        ...
        Global.Add("saveply", "(", new OneOperatorCode< PLY_WriteMeshS_Op >);
        Global.Add("plyloadS", "(", new PLY_LoadMeshS);
