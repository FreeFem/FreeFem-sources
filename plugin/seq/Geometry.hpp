...
// bool refinable;
	Vertex const *u;
	Vertex const *v;// Note : routines tend to leave v constant when possible (i.e. except construct and flip)
...
inline ostream_math operator << (ostream_math f, const Edge &e) {
	if (f.format == Mathematica) {f << "{" << R2(*(e.getu())) << "," << R2(*(e.getv())) << "}";} else {
		f.os << e;
	}
...
inline bool Edge::check () const {
	if (u == NULL || v == NULL) {cout << "Edge::check : Invalid extremities";} else if (u == v) {cout << "Edge::check : identical extremities";} else if (next == NULL || next->next == NULL) {cout << "Edge::check : Missing edge connections";} else if (next->next->next != this) {cout << "Edge::check : not a triangle";} else if (next->u != v) {cout << "Edge::check : invalid next edge (next->u!=v)";} else if (sister != NULL && sister->u != v) {cout << "Edge::check : invalid sister edge";}
