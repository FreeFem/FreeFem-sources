...
{
	if (verbosity)
	  cout << "Reading Marmousi22d Model file " << *s << endl;
    a->file = new string(* s);
...
{ 
  if (verbosity && mpirank == 0)
    cout << " load: Marmousi2  " << endl;
    
  Dcl_Type<Marmousi22d *>(InitP<Marmousi22d>,Destroy<Marmousi22d>);
  zzzfff->Add("Marmousi2",atype<Marmousi22d*>());
  TheOperators->Add("<-", new OneOperator2_<Marmousi22d *,Marmousi22d* ,string*>(&init_Marmousi22d));
  atype< Marmousi22d * >()->Add("(","",new OneOperator3_<double,Marmousi22d *,double,double>(Marmousi22d_eval));
}
LOADFUNC(Load_Init)
