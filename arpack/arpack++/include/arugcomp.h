...
class ARluCompGenEig:
  public virtual
    ARCompGenEig<FLOAT, ARumNonSymPencil<arcomplex<FLOAT>, FLOAT >,
                 ARumNonSymPencil<arcomplex<FLOAT>, FLOAT > > {

 private:

 // a) Data structure used to store matrices.

  ARumNonSymPencil<arcomplex<FLOAT>, FLOAT > Pencil;
 ...
 // c.2) Constructors and destructor.
  ...
  ARluCompGenEig(int nevp, ARumNonSymMatrix<arcomplex<FLOAT> >& A,
                 ARumNonSymMatrix<arcomplex<FLOAT> >& B,
...
Copy(const ARluCompGenEig<FLOAT>& other)
{
  ...
  ARCompGenEig<FLOAT, ARumNonSymPencil<arcomplex<FLOAT>, FLOAT >,
               ARumNonSymPencil<arcomplex<FLOAT>, FLOAT> >:: Copy(other);
  Pencil = other.Pencil;
  objOP  = &Pencil;
  objB   = &Pencil;
...
inline void ARluCompGenEig<FLOAT>::SetRegularMode()
{
  ...
  ARStdEig<FLOAT, arcomplex<FLOAT>,
    ...
    SetRegularMode(&Pencil,
                   &ARumNonSymPencil<arcomplex<FLOAT>, FLOAT>::MultInvBAv);
...
SetShiftInvertMode(arcomplex<FLOAT> sigmap)
{
  ...
  ARCompGenEig<FLOAT, ARumNonSymPencil<arcomplex<FLOAT>, FLOAT>,
    ...
    SetShiftInvertMode(sigmap, &Pencil,
                       &ARumNonSymPencil<arcomplex<FLOAT>, FLOAT>::MultInvAsBv);
...
ARluCompGenEig(int nevp, ARumNonSymMatrix<arcomplex<FLOAT> >& A,
               ARumNonSymMatrix<arcomplex<FLOAT> >& B, char* whichp,
...

{
  ...
  DefineParameters(A.ncols(), nevp, &Pencil,
                   ARumNonSymPencil<arcomplex<FLOAT>, FLOAT>::MultInvBAv,
                   &Pencil, ARumNonSymPencil<arcomplex<FLOAT>, FLOAT>::MultBv,
