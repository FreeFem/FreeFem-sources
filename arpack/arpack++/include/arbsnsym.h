...
class ARluNonSymStdEig:
 ...
 // a.2) Constructors and destructor.
  ...
  ARluNonSymStdEig(int nevp, ARbdNonSymMatrix<FLOAT>& A,
...
inline void ARluNonSymStdEig<FLOAT>::SetRegularMode()
{
  ...
  ARStdEig<FLOAT, FLOAT, ARbdNonSymMatrix<FLOAT> >::
    SetRegularMode(objOP, &ARbdNonSymMatrix<FLOAT>::MultMv);
...
inline void ARluNonSymStdEig<FLOAT>::SetShiftInvertMode(FLOAT sigmap)
{
  ...
  ARStdEig<FLOAT, FLOAT, ARbdNonSymMatrix<FLOAT> >::
    SetShiftInvertMode(sigmap, objOP, &ARbdNonSymMatrix<FLOAT>::MultInvv);
...

{
  ...
  NoShift();
  DefineParameters(A.ncols(), nevp, &A, ARbdNonSymMatrix<FLOAT>::MultMv,
...

{
  ...
  DefineParameters(A.ncols(), nevp, &A, &ARbdNonSymMatrix<FLOAT>::MultInvv,
                   whichp, ncvp, tolp, maxitp, residp, ishiftp);
  ChangeShift(sigmap);
