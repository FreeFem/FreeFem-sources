...
const char whatbamg[] = "@(#)LJLL, feb. 20, 2006, Bamg v1.02";
...
#define W_OK 02
#include <io.h>
inline int access(char *fileName, int mode) { return _access(fileName, mode); }
#endif
...
int main(int argc, char **argv) {
  ...
  long long bidon = 2;
  if ((double)bidon != 2.0) {
    cerr << "Compiler bug  Pb of cast of long long to a double => remove the LONGLONG define"
         << endl;
    exit(10);
  }
#endif
  ...
  if (argc < 3) {    // for test on the mac
    if (!access(datafile, R_OK)) {
      MeshIstream data(datafile);
      while (data.cm( ) && datargc < 128) datargv[datargc++] = data.ReadStr( );

      if (argc == 1) datargv[0] = argv[0];    // copy of the program name
      argc = datargc;
      argv = datargv;
    }
  };
  ...
  for (i = 1; i < argc; i++)
    if (!strcmp(argv[i], "-v") && ++i < argc)
      ...
      cout << "" << endl;
      cout << "     -M filename     Set  the metric which is  defined on the background mesh "
           << endl;
      cout << "                     or on the geometry. Metric file." << endl;
      cout << "     -Mbb filename   Set the solution  defined on the background mesh for" << endl;
      cout << "                     metric construction, the solutions was FE P1 defined on "
           << endl;
      ...
      cout << "     -maxsubdiv  (double) Change the metric  such that  the maximal subdivision  "
           << endl;
      ...
      cout << "                     in a mesh is bounded  by ratio (by default no smoothing)."
           << endl;
      cout << "     -hmin (double)  Set the value of the minimal edge size." << endl;
      cout << "     -hminaniso (double)  Set the value of the minimal edge size and save aniso."
           << endl;
      ...
      cout << "     -splitpbedge     split in 2 all internal edges with 2 vertex on boundary"
           << endl;
      ...
      cout << "     -nbv (int)       Set the maximal of  vertices (" << nbvx << " by default)."
           << endl;
      cout << "" << endl;
      cout << "" << endl;
  ...
  if (iso) anisomax = 1;
  if (!(fmetrix || fMbb)) NbSmooth = 0;    // no metric -> no smoothing
  if (!rbb)                                // to set the rbb filename by default
    rbb = fMbb;
  if (!rBB)    // to set the rbb filename by default
    rBB = fMBB;

  if (fMbb && rbb) rbbeqMbb = !strcmp(rbb, fMbb);
  if (fMBB && rBB) rBBeqMBB = !strcmp(rBB, fMBB);
  ...
  if (verbosity)
    if (fgeom && fileout)
      ...
      if (NoMeshReconstruction)
        cout << " Modification of a adpated mesh " << fmeshback << "    with less than " << nbvx
             << " vertices" << endl;
      else
        cout << " Construction of a adpated mesh from the background mesh: " << fmeshback
             << "    with less than " << nbvx << " vertices" << endl;
  ...
  if (fgeom && fileout) {
    ...
    time1 = CPUtime( );
    if (verbosity > 3) cout << " Cpu for read the geometry " << time1 - time0 << "s" << endl;
    Triangles Th(nbvx, Gh);
    if (costheta <= 1) Th.MakeQuadrangles(costheta);
    if (allquad) Th.SplitElement(allquad == 2);

    if (SplitEdgeWith2Boundary) Th.SplitInternalEdgeWithBorderVertices( );
    ...
    time3 = CPUtime( );
    if (verbosity > 1)
      cout << " Cpu for meshing         :" << setw(8) << time2 - time1 << "s, for  Smoothing "
           << time3 - time2 << "s  Nb Vertices/s = " << (Th.nbv) / (time2 - time1) << setw(0)
           << endl;
    ...
    if (verbosity > 0) {
      ...
      if (Th.NbOfQuad) cout << " Nb Quadrilaterals = " << Th.NbOfQuad;
      cout << endl;
    }
    ...
    if (raison) BTh.SmoothMetric(raison);
    ...
    time2 = CPUtime( );
    if (foM) {
      if (verbosity > 2) cout << " -- write Metric  file " << foM << endl;
    ...
    if (fileout) {
      ...
      if (verbosity > 0)
        cout << " Cpu time for meshing          : " << time3 - time2
             << "s  Nb Vertices/s = " << (Th.nbv) / (time3 - time2) << endl;
      if (verbosity > 3) Th.ShowHistogram( );
      if (NbSmooth > 0) Th.SmoothingVertex(NbSmooth, omega);
      if (verbosity > 3 && NbSmooth > 0) Th.ShowHistogram( );
#ifdef DRAWING
      ...
      {
        if (verbosity > 1) {
          if (rbb) cout << " -- interpolation P1  files : " << rbb << " -> " << wbb << endl;
          if (rBB) cout << " -- interpolation P1  files : " << rBB << " -> " << wBB << endl;
        }
        ...
        if (fbb) delete fbb;    // close
        if (fBB) delete fBB;    // close
        if (solbb) delete[] solbb;
        if (solBB) delete[] solBB;
      ...
      if (verbosity > 0) {
        cout << " Cpu time for meshing with io  : " << time4 - time0
             << "s  Nb Vertices/s = " << Th.nbv / (time4 - time0) << endl
             << " Nb vertices = " << Th.nbv;
        if (Th.nbt - Th.NbOutT - Th.NbOfQuad * 2)
          cout << " Nb Triangles = " << (Th.nbt - Th.NbOutT - Th.NbOfQuad * 2);
        if (Th.NbOfQuad) cout << " Nb Quadrilaterals = " << Th.NbOfQuad;
  ...
  // clean the
  for (i = 1; i < datargc; i++) delete[] datargv[i];
