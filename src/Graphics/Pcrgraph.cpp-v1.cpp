...
void fillpoly (int n, float *poly) {
	...
	for (int i = 0; i < n; i++) {
		pt[i].x = scalx(poly[2 * i]);
		pt[i].y = scaly(poly[2 * i + 1]);
	}
...
char Getijc (int &x, int &y) {
	...
	do {
		...
		case WM_CHAR: char1 = (TCHAR)msg.wParam;
			cont = 0;
			break;
	...
	while (i < CmdLen) {
		...
		if (lpszCmdLine[i] == '-') {
			...
			case 's':	// not wait at end of execution
				winf_flg |= winf_NOWAIT;
				++i;
				break;
			case 'b':	// no color
				winf_flg |= winf_NOCOLOR;
				++i;
				break;
			case 'n':
				winf_flg |= winf_NOEDIT;
				++i;
				break;
			case 'h':
				winf_flg |= winf_Usage;
...
BOOL mainFreeFEM () {
	...
	cout << "Welcome to freefem+\t\tStart Program freefem+ v" << Version << endl;
