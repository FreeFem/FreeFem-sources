...
int signep4 (int i0, int i1, int i2, int i3) {	// calcul du signe dans la permutation
	int s = 1;
...
void DefColor (float &r, float &g, float &b,
	...
	else if (grey) {float gg = 0.1 + 0.9 * float(k - 2) / (nb - 3); r = g = b = gg;} else if (nbcolors <= 1) {
...
void OneWindow::DefaultView (int state) {
	...
	if (theplot) {
		...
		else if (rapz0 <= 0) {	// ( zmax-zmin )*rapz0 =  0.3 dxyy
...
case 20 + index: {type dummy; fin >= dummy;} break;
	...
	while (1) {
		...
		if (version == 2)
			...
			case 6: fin >> clean;
				break;
			case 7: fin >> aspectratio;
				uaspectratio = true;
				break;
			case 8: fin >> boundingbox;
				break;
			case 9: fin >> Niso;
				break;
			case 10: fin >> Narrow;
			...
			case 15: fin >> colors;
				break;
			case 16: fin >> drawborder;
				break;
			case 17: fin >> dimpp;
				break;	// ajout fevr 2008  v3.0.6
			case 18: fin >> add;
				break;
			case 19: fin >> keepPV;
				break;
...
void plot (double xx, double yy, const char *cmm, int font) {
	...
	switch (font) {
	case  0: glut_font = GLUT_STROKE_ROMAN;
		break;
	case  1: glut_font = GLUT_STROKE_MONO_ROMAN;
		break;
	case  2: glut_font = GLUT_BITMAP_9_BY_15;
		break;
	case  3: glut_font = GLUT_BITMAP_8_BY_13;
		break;
	case  4: glut_font = GLUT_BITMAP_TIMES_ROMAN_10;
		break;
	case  5: glut_font = GLUT_BITMAP_TIMES_ROMAN_24;
		break;
	case  6: glut_font = GLUT_BITMAP_HELVETICA_10;
		break;
	case  7: glut_font = GLUT_BITMAP_HELVETICA_12;
		break;
	case  8: glut_font = GLUT_BITMAP_HELVETICA_18;
		break;
	}
...
void ThePlot::DrawIsoTfill (const R2 Pt[3], const R ff[3], const R *Viso, int NbIso, R rapz) {
	...
	for (int l = 1; l < NbIso; l++) {	// loop on the level curves
		...
		if (im > 2) {
			...
			// SetColor((xfb+xfh)/2);
			for (int i = 0; i < im; i++)// if((debug > 10)) cout << i << " \t : " << PQ[i].x << " " <<  PQ[i].y << " " << z[i]*rapz << endl;
				glVertex3f(P[i].x, P[i].y, P[i].z);
...
void SpecialKey (int key, int x, int y) {
	...
	if (win) {
		...
		switch (key) {
		case  GLUT_KEY_LEFT:   dx = -1;
			break;
		case  GLUT_KEY_RIGHT:  dx = +1;
			break;
		case  GLUT_KEY_DOWN:   dy = -1;
			break;
		case  GLUT_KEY_UP:     dy = +1;
			break;
		}
