...
int getprog(char *fn, int argc, char **argv) {
  ...
  *fn = '\0';
  ...
  if(argc) {
    for (int i = 1; i < argc; i++) {
      ...
      if (ret == 0 && strcmp(argv[i], "-f") == 0 && i + 1 < argc) {
        ...
        progffglut = argv[++i];
        if (flagnw)
          noffglut = NoGraphicWindow = true;// if -nw => no graphic in anycase
        else
          noffglut = NoGraphicWindow = false;
      }
  ...
  if (progffglut && mpirank == 0) {
    ...
    if (!ThePlotStream) {
    cerr << " Error save file glut " << fileglut << " mode " << MODE_WRITE_BINARY << endl;
    exit(1);
    }
  }
  ...
  if (ret != 1) {
    ...
    cout << " Error parameter: " << ff << " , version: " << StrVersionNumber() << " " << sizeof(void*)*8 << "bits \n";
    ...
    if (noffglut)
      cout << " without default ffglut : " << ffglut << endl;
    else
      cout << " with default ffglut : " << ffglut << endl;
    cout << endl;
    exit(1);
    return ret;
  }
