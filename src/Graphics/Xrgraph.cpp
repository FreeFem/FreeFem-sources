...
void initgraphique () {
	...
	if (!font_info) {
		cout << " erreur font 6x10 and 6x9 not found !\n";
		exit(2);
	}
...
void x11draw3 (int *ptype) {
	...
	switch (type) {
	case 0: {gcvalues.line_style = LineSolid;
		     break;
	}
	case 1: {gcvalues.line_style = LineOnOffDash;
		     break;
	}
	default: {gcvalues.line_style = LineDoubleDash;
		      break;
	}
	}
...
void fillpoly (int n, float *poly) {
	...
	if (n < 10)
		poly0 = polyloc;
	else if (poly0 = (XPoint *)malloc(n * sizeof(XPoint)), !poly) {
