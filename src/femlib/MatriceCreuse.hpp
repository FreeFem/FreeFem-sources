...
template<class R> class StopGC {
	public: virtual bool Stop (int iter, R *, R *) {
			cout << " Stop !!!!!\n";
			return false;
	}
};
...
template<class R>
class MatriceIdentite: public VirtualMatrice<R> {
	public:
...
class SolveGCDiag:   public MatriceMorse<R>::VirtualSolver, public VirtualMatrice<R> {
	...
	public:
		...
		SolveGCDiag (const MatriceMorse<R> &A, int itmax, double epsilon = 1e-6):
			...
			n(A.n), nbitermax(itmax ? itmax : Max(100, n)), eps(epsilon), epsr(0), D1(n) {	// throwassert(A.sym());
			for (int i = 0; i < n; i++)
				D1[i] = 1. / A(i, i);
		}
