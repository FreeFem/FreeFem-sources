...
struct PplusQ {const P &p;
	           const Q &q;
	           PplusQ (const P &pp, const Q &qq): p(pp), q(qq) {}
};

template<class R>
struct  VirtualMatrice {
	public:
		int N, M;
...
#endif
		struct  plusAx {const VirtualMatrice *A;
			            const KN_<R> x;
...
class KN_: public ShapeOfArray {
	...
	public:
		...
		KN_ &operator = (const typename VirtualMatrice<R>::plusAx &Ax) {
			if (Ax.A) {
				ffassert(&Ax.x[0] != &this->operator [] (0));
				*this = R();
				Ax.A->addMatMul(Ax.x, *this);
			}
		...
		KN_ &operator = (const typename VirtualMatrice<R>::solveAxeqb &Ab) {
			if (Ab.A) {
				ffassert(&Ab.b[0] != &this->operator [] (0));
				*this = R();
				Ab.A->Solve(*this, Ab.b);
			}
...
class KN: public KN_<R> {
	public:
		...
		KN &operator = (const SetArray<R> &u) {
			if (this->unset()) this->set(new R[u.size()], u.size(), 0, 0);
		...
		KN &operator = (const_R a) {
			if (this->unset()) this->set(new R[1], 1, 0, 0);
		...
		KN &operator = (const KN_<R> &a) {
			if (this->unset()) this->set(new R[a.N()], a.N());
		...
		KN &operator = (const Add_KN_<R> &u) {
			if (this->unset()) this->set(new R[u.a.N()], u.a.N());
		...
		KN &operator = (const Mul_KNM_KN_<R> &u) {
			if (this->unset()) this->set(new R[u.b.N()], u.b.N());
...
// { if(this->unset()) set(new R[A1x.v.N()],A1x.v.N());KN_<R>::operator=(A1x);return *this;}
		...
		KN &operator = (const typename VirtualMatrice<R>::plusAx &Ax) {
			if (this->unset() && Ax.A && Ax.A->N) this->set(new R[Ax.A->N], Ax.A->N);

			if (Ax.A) KN_<R>::operator = (Ax);

			return *this;
		}

		KN &operator = (const typename VirtualMatrice<R>::solveAxeqb &Ab) {
			if (this->unset()) this->set(new R[Ab.b.N()], Ab.b.N());
		...
		KN &operator = (const typename VirtualMatrice<R>::plusAtx &Ax) {
			if (this->unset() && Ax.A->M) this->set(new R[Ax.A->M], Ax.A->M);
...
class KNM: public KNM_<R> {
	public:
		...
		KNM &operator = (const KNM_<const_R> &u) {
			if (this->unset()) this->init(u.N(), u.M());
		...
		KNM &operator = (const_R a) {
			if (this->unset()) RNM_FATAL_ERROR(" KNM operator=(double)");
		...
		KNM &operator = (const ConjKNM_<R> &u) {
			if (this->unset()) this->init(u.N(), u.M());
