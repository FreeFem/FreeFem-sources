...
#include "HashTable.hpp"
	...
	class GenericElement: public Label {
		...
		public:
			...
			int facePermutation (int i) const {	// def the permutatution of orient the face
				int fo = 0;
				const Vertex *f[3] = {&at(nvface[i][0]), &at(nvface[i][1]), &at(nvface[i][2])};
	...
	class GenericMesh: public RefCounter
	{
		...
		public:
			...
			int ElementAdj (int k, int &j) const {
				...
				return p >= 0 ? p / nea : -1 - j;
			}	// modif FH. to change the code of copule k,kadj on border element..
			...
			for (int ke = 0; ke < nbe; ++ke) {
				...
				if (!p) {
					...
					if (nkk >= 0) {
						...
						if (regk != regkk) {// verif orientation  interior normal
							int sr = 1;
							if (step == 0) {
								if (regk > regkk)
		...
		if (nodearevertices) {
			...
			NbOfDF = 0;
			{
				...
				for (int ieq = 0, keq = 0; keq < nbequibe; ++keq) {
					...
					int dimb = B::RdHat::d;
					if (ndfon[dimb]) {	// simple border element
					}
