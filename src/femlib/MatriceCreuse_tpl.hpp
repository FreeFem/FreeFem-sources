...
void MatriceElementairePleine<R, FES>::call (int k, int ie, int label, void *stack, void *B) {
	...
	if (this->onFace) {
		...
		if (&this->Vh == &this->Uh) {
			...
			if (kk < 0) {	// return ; // on saute ????  bof bof
...
MatriceProfile<R>::~MatriceProfile () {
	if (!this->dummy) {	// cout << " del mat profile " << endl ;
		if (U && (U != L)) delete [] U;
...
KN_<R> &operator /= (KN_<R> &x, const MatriceProfile<R> &a) {
	...
	case FactorizationNO:
		...
		if (a.U && !a.L) {	// matrice triangulaire superieure
		...
		} else if (!a.U && a.L) {	// matrice triangulaire inferieure
...
void MatriceMorse<R>::Build (const FESpace &Uh, const FESpace &Vh, bool sym, bool VF) {
	...
	for (int in1 = 1; in1 <= nbn_u; in1++) {// in1 = in + 1
		kk += pe_u[in1];
...
template<class R>
R MatriceMorse<R>::pscal (const KN_<R> &x, const KN_<R> &y) {	// (x, Ay)
	R sum = R();
	int i, j, k;
