...
// void operator=(const TVertex & v) { x=v.x;y=v.y;lab=v.lab;normal=0;}
	...
	class Tetraedre: public Label {
		...
		public:
			...
			R h () const {
				return Max(Max(lenEdge(0), lenEdge(1), lenEdge(2)),
...
// const Vertex & VerticeOfEdge(int i,int j) const {return  *vertices[(i+1+j)%3];}
	...
	class Mesh: public RefCounter {
		public:
			...
			int GetAllElementAdj (int it, int *tabk) const {// get the tab of all adj triangle to a traingle (max 3)
...
// 2 routines to compute the caracteristic
	...
	void HeapSort (T *c, T1 *c1, T2 *c2, long n) {
		...
		c--;
		c1--;
		c2--;	// on decale de 1 pour que le tableau commence a 1
		if (n <= 1) return;
	...
	inline int numSubTVertex (int N, int i, int j) {// i,j  coordonne barycentre * N dans l'eleùent de reference.
