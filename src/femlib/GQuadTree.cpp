...
namespace EF23 {
	...
	void GTree<Vertex>::Add (Vertex &w) {
		...
		if (!(b = *pb))	// if(verbosity > 5)  cout << "Qbox \n";
	...
	template<class Vertex> GTree<Vertex>::StorageQuadTreeBox::~StorageQuadTreeBox () {	// cout <<  "~StorageQuadTreeBox " << this << " n " << n << " b " << b << endl;
		if (n) delete n;
...
L1:
		...
		if (!vn && b != root) {	// cas particulier on repart du sommet on avais rien trouver
			b = root;
			hb = MaxISize;
			p0 = Zd();
			l = 0;
