...
namespace Fem2D {
	...
	void Mesh::ConsAdjacence () {
		...
		}
		{
			...
			do {// two step
				...
				for (int is = 0; is < nv; is++)
					if (linkg[is] == -2) {	// for all extremity of mortars
						if (linkd[is] != -2) {
						...
						for (p = headT3[is]; p >= 0; p = NextT3[p]) {	// for all nonconformitie around sm
							int k = p / 3;
							int i = p % 3;
							const Triangle &T(triangles[k]);
						...
						for (int jm = 0; jm < km; im = jm++) {	// for all break of vertex
							double theta = (bmortars[jm].alpha - thetai);
							thetai = bmortars[jm].alpha;
							if (theta < eps || theta + eps > 2 * Pi) {	// same angle mod 2 * Pi
								...
								do {// for all
									...
									for (p = headT3[sm]; p >= 0; p = NextT3[p]) {
										...
										if (TonBoundary[k] & AddMortar[j]) {// check the sens and the direction
			...
			for (int i = 0; i < NbMortars; i++) {
				if (mortars[i].nleft == 1) t3 = mortars[i].left[0]; else if (mortars[i].nright == 1) t3 = mortars[i].right[0]; else cout << "   -- Bizarre " << mortars[i].nleft << " " << mortars[i].nright << endl;
	...
	void Mesh::read (ifstream &f, bool bin) {	// read the mesh
		dim = 2;
		ne = 0;
		ntet = 0;
		volume = 0;
...
RESTART:
		...
		while (1) {
			...
			if (!ret) {	// VERIF THE DISTANCE**2 Dicrease
				R2 Pjj(P, K[jj]);
				R dd = (Pjj, Pjj);
				if (dd >= dP) {
					Phat = PPhat;
...
// for the  mortar elements
	...
	Mesh::Mesh (const Mesh &Th, int *split, bool WithMortar, int label) {	// routine complique
		...
		for (int K = 0; K < Th.nt; K++) {	// cout << K << endl;
			Triangle &T(Th[K]);
			R2 A(T[0]);
			R2 B(T[1]);
			R2 C(T[2]);
