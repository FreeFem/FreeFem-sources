...
// const int (* const SetonWhatIsFace(int  onWhatIsFace[4][15] ,const int  nvfaceTet[4][3],const int nvedgeTet[6][2]))[15]
	...
	void Mesh3::read (istream &f) {	// read the mesh
		int i;
		// f >> nv >> nt >> neb ;
		string str;
		int err = 0;
		...
		while (!f.eof()) {
			...
			} else if (str[0] == '#') {	// on mange la ligne
				int c;
...
// new version ...
	int WalkInTetn (const Mesh3 &Th, int it, R3 &Phat, const R3 &U, R &dt, R3 &offset) {
		...
		if (l[0] > -eps && l[1] > -eps && l[2] > -eps && l[3] > -eps) {	// a fini  .... ouf ...
			dt = 0;
			Phat = R3(l + 1);
			nomove = false;
			return -1;
		...
		if (kf >= 0) {	// sortie positive ..
			...
			if (k > 0) {
				...
				if (k == 1) kk = p[3];
					...
					if (l[p[1]] > epsedge) {// sort sur un arete .. varaimant ...
