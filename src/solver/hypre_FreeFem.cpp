...
class hypreParam {
	...
	public: hypreParam (const KN<long> &param_int, const KN<double> &param_double) {
			...
			if (param_int.N() > 0) {
				if ((param_int[0] >= 0) && (param_int[0] <= 9)) solver_id = param_int[0];
		...
		~HypreSolver () {
			if (verbosity) {
				...
				case 0:	// BICGSTAB solver
					...
					switch (param->precond_id) {
					case 0:  HYPRE_BoomerAMGDestroy(precond);
						break;
					case 5:  HYPRE_ParaSailsDestroy(precond);
						break;
					case 6:  HYPRE_SchwarzDestroy(precond);
						break;
					case 4:  HYPRE_ParCSRPilutDestroy(precond);
						break;
					default:   break;
					}
