...
void parm_param (string datafile, KN<long> param_int, KN<double> param_double) {
	...
	for (i = 0; i < 16; i++) param_int[i] = -1;

	for (i = 0; i < 9; i++) param_double[i] = -1.0;
	...
	while (fgets(buf, BUFLEN, fp) != NULL) {
		if (num <= 15) {sscanf(buf, "%d", &in_val); param_int[num] = in_val;} else
		...
		HipsSolver (const MatriceMorse<double> &AA, string datafile, const KN<long> &param_int1, const KN<double> &param_double1, MPI_Comm *mpicommw): data_option(datafile) {
			...
			if ((!data_option.empty()) && ((param_int == NULL) || (param_double == NULL)))
				...
				if ((param_double.N() > 0) || (param_int.N() > 0)) {
					if (proc_id == 0) cout << "    WE SET PARAMETER FROM VECTORS   " << endl; } else if (proc_id == 0) cout << "    WE SET DEFAULT PARAMETERS       " << endl;
			}

			if (param_int.N() > 0) {
				if ((param_int[0] == HIPS_ITERATIVE) || (param_int[0] == HIPS_HYBRID))	// Using strategy. Input is ok
				{HIPS_SetDefaultOptions(id, param_int[0]); ic = param_int[0];} else {HIPS_SetDefaultOptions(id, HIPS_ITERATIVE); ic = HIPS_ITERATIVE;}	// Strategy is not they existing one
			} else {HIPS_SetDefaultOptions(id, HIPS_ITERATIVE); ic = HIPS_ITERATIVE;}	// Default strategy is ITERATIVE

			if (param_double.N() > 0) {
				if (param_double[0] > 0) HIPS_SetOptionREAL(id, HIPS_PREC, param_double[0]);
				else HIPS_SetOptionREAL(id, HIPS_PREC, 1e-09); } else HIPS_SetOptionREAL(id, HIPS_PREC, 1e-09);

			if ((param_int.N() > 1) && (ic == HIPS_ITERATIVE)) {
				if ((param_int[1] == 1) || (param_int[1] == 0)) HIPS_SetOptionINT(id, HIPS_KRYLOV_METHOD, param_int[1]);
				else HIPS_SetOptionINT(id, HIPS_KRYLOV_METHOD, 0); } else if (ic == HIPS_ITERATIVE) HIPS_SetOptionINT(id, HIPS_KRYLOV_METHOD, 0);

			if (param_int.N() > 2) {if (param_int[2] > 0) HIPS_SetOptionINT(id, HIPS_ITMAX, param_int[2]); else HIPS_SetOptionINT(id, HIPS_ITMAX, 1000); } else HIPS_SetOptionINT(id, HIPS_ITMAX, 1000);

			if (param_int.N() > 3) {
				if (param_int[3] > 0) HIPS_SetOptionINT(id, HIPS_KRYLOV_RESTART, param_int[3]);
				else HIPS_SetOptionINT(id, HIPS_KRYLOV_RESTART, 40); } else HIPS_SetOptionINT(id, HIPS_KRYLOV_RESTART, 40);

			if (param_int.N() > 4) {if (param_int[4] > 0) {HIPS_SetOptionINT(id, HIPS_SYMMETRIC, param_int[4]); symm = param_int[4];}} else {HIPS_SetOptionINT(id, HIPS_SYMMETRIC, 1); symm = 1;}

			if (param_int.N() > 5) {
				if ((param_int[5] == 0) || (param_int[5] == 1)) {HIPS_SetOptionINT(id, HIPS_GRAPH_SYM, param_int[5]); sym = param_int[5];} else {HIPS_SetOptionINT(id, HIPS_GRAPH_SYM, 1); sym = 1;}} else {HIPS_SetOptionINT(id, HIPS_GRAPH_SYM, 1); sym = 1;}

			if (param_int.N() > 6) {
				if (param_int[6] > 0) HIPS_SetOptionINT(id, HIPS_PARTITION_TYPE, param_int[6]);
				else HIPS_SetOptionINT(id, HIPS_PARTITION_TYPE, 0); } else HIPS_SetOptionINT(id, HIPS_PARTITION_TYPE, 0);

			if (param_int.N() > 7) {
				if (param_int[7] > 0) HIPS_SetOptionINT(id, HIPS_LOCALLY, param_int[7]);
				else HIPS_SetOptionINT(id, HIPS_LOCALLY, 2); } else HIPS_SetOptionINT(id, HIPS_LOCALLY, 2);

			if (param_int.N() > 8) {
				if (param_int[8] > 0) HIPS_SetOptionINT(id, HIPS_FORTRAN_NUMBERING, param_int[8]);
				else HIPS_SetOptionINT(id, HIPS_FORTRAN_NUMBERING, 0); } else HIPS_SetOptionINT(id, HIPS_FORTRAN_NUMBERING, 0);

			if (param_int.N() > 9) {
				if (param_int[9] > 0) HIPS_SetOptionINT(id, HIPS_SCALE, param_int[9]);
				else {HIPS_SetOptionINT(id, HIPS_SCALE, 1); scale = 1;}} else {HIPS_SetOptionINT(id, HIPS_SCALE, 1); scale = 1;}

			if (param_int.N() > 10) {
				if (param_int[10] > 0) HIPS_SetOptionINT(id, HIPS_REORDER, param_int[10]);
				else HIPS_SetOptionINT(id, HIPS_REORDER, 1); } else HIPS_SetOptionINT(id, HIPS_REORDER, 1);

			if (param_int.N() > 11) {
				if (param_int[11] > 0) HIPS_SetOptionINT(id, HIPS_DOF, param_int[11]);
				else HIPS_SetOptionINT(id, HIPS_DOF, 1); } else HIPS_SetOptionINT(id, HIPS_DOF, 1);

			if (param_int.N() > 12) {
				if (param_int[12] > 0) HIPS_SetOptionINT(id, HIPS_SCALENBR, param_int[12]);
				else HIPS_SetOptionINT(id, HIPS_SCALENBR, 2); } else HIPS_SetOptionINT(id, HIPS_SCALENBR, 2);

			if (param_int.N() > 13) {
				if (param_int[13] > 0) HIPS_SetOptionINT(id, HIPS_VERBOSE, param_int[13]);
				else HIPS_SetOptionINT(id, HIPS_VERBOSE, 5); } else HIPS_SetOptionINT(id, HIPS_VERBOSE, 5);

			if (param_int.N() > 14) {
				if (param_int[14] > 0) HIPS_SetOptionINT(id, HIPS_DOMSIZE, param_int[14]);
			...
			if (param_double.N() > 1) {
				if (param_double[1] > 0.0) HIPS_SetOptionREAL(id, HIPS_DROPTOL0, param_double[1]);
				else HIPS_SetOptionREAL(id, HIPS_DROPTOL0, 0.005); } else HIPS_SetOptionREAL(id, HIPS_DROPTOL0, 0.005);

			if (param_double.N() > 2) {
				if (param_double[2] > 0.0) HIPS_SetOptionREAL(id, HIPS_DROPTOL1, param_double[2]);
				else HIPS_SetOptionREAL(id, HIPS_DROPTOL1, 0.005); } else HIPS_SetOptionREAL(id, HIPS_DROPTOL1, 0.005);

			if (param_double.N() > 3) {
				if (param_double[3] > 0.0) HIPS_SetOptionREAL(id, HIPS_DROPTOLE, param_double[3]);
				else HIPS_SetOptionREAL(id, HIPS_DROPTOLE, 0.005); }

			HIPS_SetOptionREAL(id, HIPS_DROPTOLE, 0.005);

			if (param_double.N() > 4) {
				if (param_double[4] > 0.0) HIPS_SetOptionREAL(id, HIPS_AMALG, param_double[4]);
				else HIPS_SetOptionREAL(id, HIPS_AMALG, 0.005); }

			HIPS_SetOptionREAL(id, HIPS_AMALG, 0.005);
			...
			if (riord == NULL) {
				if (nproc == 0) printf("%s", "Memory allocation failed in partition stage \n");
