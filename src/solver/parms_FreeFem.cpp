...
public:
	...
	public: parm_param(){
		...
		/* parameters associated with various accelerators (inner/outer)          */
		  ...
		  ipar->ipar[5] = 20;      /* Outer iteration Krylov subs. dimension      */
		  ...
		  ipar->pgfpar[1] = 1.e-05;/* tolerance for outer iteration               */
		/* preconditioning parameters                                             */
		 ...
		 //Integer parameter
		 iov=0; scale=0; unsymm=1; method= 2; VERBOSE=0; /*no statistic infos has print*/
	...
	public: parm_param(const KN<long> &param_int, const KN<double> &param_double)
	{
		...
		if(param_int.N()>0) {if(param_int[0]>=0) iov=param_int[0]; else iov=0;} else iov=0;
		if(param_int.N()>1) {if(param_int[1]>0) scale=param_int[1]; else scale=0;}else scale=0;

		if(param_int.N()>2){if(param_int[2]>0) unsymm=param_int[2]; else unsymm=1;} else unsymm=1;

		if(param_int.N()>3){if((param_int[3]<0)||(param_int[3]>13)) 
		{printf("%s","NO  INDEX FOR PRECONDITIONNER \n"); exit(1);} 
		else if((param_int[3]>=0)||(param_int[3]<=13)) method=param_int[3];}else method=2;

		if(param_int.N()>4){if((param_int[4]<0)||(param_int[4]>2)) 
		{printf("%s","WRONG SOLVER INDEX \n"); exit(1); } else solver=param_int[4];}else solver=0;
		...
		if(param_int.N()>5){if(param_int[5]>0) ipar->ipar[5]=param_int[5]; else ipar->ipar[5]=30;}
		else ipar->ipar[5]=30;
		
		if(param_int.N()>6){if(param_int[6]>0) ipar->ipar[6]=param_int[6]; else ipar->ipar[6]=500;}
		else ipar->ipar[6]=500;

		if(param_int.N()>7){ if(param_int[7]>0) ipar->ipar[3]=param_int[7]; else ipar->ipar[3]=3;}
		else ipar->ipar[3]=3;

		if(param_int.N()>8){if(param_int[8]>0) ipar->ipar[4]=param_int[8]; else ipar->ipar[4]=3;}
		else ipar->ipar[4]=3;

		//if((solver==1)||(solver==2)){ipar->ipar[3]=0; ipar->ipar[4]=0;}

		if(param_int.N()>9){if(param_int[9]>0) ipar->ipar[5]=param_int[9]; else ipar->ipar[5]=3;}
		else ipar->ipar[5]=3;

		if(param_int.N()>10){if(param_int[10]>0) prepar->mc=param_int[10]; else prepar->mc=1; }
		else prepar->mc=1; 

		if(param_int.N()>11){if(param_int[11]>0) ipar->in_iters=param_int[11]; else ipar->in_iters=0; }
		else ipar->in_iters=0;

		if(param_int.N()>12){if(param_int[12]>0) VERBOSE=param_int[12]; else VERBOSE=0; }else VERBOSE=0;
		
		if(param_double.N()>0){if(param_double[0]>0) ipar->pgfpar[1]=param_double[0]; else ipar->pgfpar[1]=1.e-08;}
		ipar->pgfpar[1]=1.e-08;

		if(param_double.N()>1){if(param_double[1]>0) ipar->pgfpar[0]=param_double[1]; else ipar->pgfpar[0]=0.01;}
		else ipar->pgfpar[0]=0.01;

		if(param_double.N()>2) {if(param_double[2]>0) prepar->tolind = param_double[2]; else prepar->tolind=0.1;}
		else prepar->tolind=0.1;

		if(param_double.N()>3){if(param_double[3]>0) prepar->droptol[0]=param_double[3]; else prepar->droptol[0]=.001;}
		else prepar->droptol[0]=.001;
		/* droptol0 (ilut, iluk, and arms) */
		if(param_double.N()>4) {if(param_double[4]>0) prepar->droptol[4]=param_double[4]; else prepar->droptol[4]=.001;}
		else prepar->droptol[4]=.001;

		if(param_double.N()>5){if(param_double[5]>0) prepar->droptol[5]=param_double[5]; else prepar->droptol[5]=.001;}
		else prepar->droptol[5]=.001;
  ...
  while(fgets(buf, BUFLEN, fp) != NULL) {
    ...
    case 10:                             /* tolind  */
      ...
      break;
    case 11:                             /* im (Outer Krylov subs dim) */
      sscanf(buf,"%d",&ipar->ipar[5]);
      break;
    case 12:                             /* max outer gmres steps */
      sscanf(buf,"%d", &ipar->ipar[6]);
      break;
    case 13:                             /* inim (inned Krylov subs dim) */
      sscanf(buf, "%d", &ipar->ipar[3]);
      break;
    case 14:                             /* itsgmr (inner max iterations) */
      sscanf(buf, "%d", &ipar->ipar[4]);
      break;
    case 15:                             /* lfil0 (ilut, iluk, and arms) */
      sscanf(buf, "%d", &prepar->lfil[0]);
      break;
    case 16:                             /* lfil 4 (schur construction) */
      sscanf(buf, "%d", &prepar->lfil[4]);
      break;
    case 17:                             /* lfil 5 (ILUT for Schur) */
...
class dSolvePARMS :   public MatriceMorse<double>::VirtualSolver   {
	...
	mutable int 					dnnz_pre, dgprec_nnz;
...
public:
  dSolvePARMS(const MatriceMorse<double> &AA,string datafile, const KN<long> &param_int, const KN<double> &param_double)
	{
	...
	int *p, *pr;	
	MPI_Comm comm;
	double  dnnz_pre, dgprec_nnz, dgnnz;
	int 	 j,    node;
	...
	int maxmp;
	 ...
	 int * maptmp1, *mapptr1;
   	 ...
   	 free(maptmp); free(mapptr);
         maptmp1=maptmp; mapptr1=mapptr;
  }
	
/*---map from global node lables to local ---*/
	getmap(dm, maptmp, mapptr, &n);
...
/*---prepare for extraction of local submatrix  --*/
	...
	DPERM1(&i1,&i2,AAv,p,pr,vals1,id_rows1,ptr1,maptmp,&tmp0,&tmp);
	
	

	 free(p); free(pr); free(AAv);
...
/*--- set up the local data strucutre for the sparse matrix */
	...
	if((dm->comm->myproc == 0)&&(VERBOSE==1)) {
               if(solver==0) printf("%s","SOLVER IS FGMRES \n");
	       if(solver==1) printf("%s","SOLVER IS DGMRES \n");
	       if(solver==2) printf("%s","SOLVER IS BICGSTAB \n");
...
*/
	...
	/*-------- Statistic in preconditionner contruction*/
	 
	if(VERBOSE==0){
	dnnz_pre = (double)precon->nnz_pre;
         dgnnz = (double)nnz;
	 MPI_Reduce(&dnnz_pre, &dgprec_nnz, 1, MPI_DOUBLE, MPI_SUM, 0, dm->comm->mpi_comm); 
         
	 if(rk == 0) 	{
			printf("Total NNZ(PreCon) / (NNZ*npro) = %e\n",dgprec_nnz / (dgnnz*size));
	...
	for(i=i1;i<=i2;i++)
	{
		  node = maptmp[i-1];poloc[comt]=node-1;
		  rhs->vec[comt]=b[node-1];comt++;
...
/*--------- apply permutations to scaletmp */
	 ...
	 if((dm->comm->myproc == 0)&&(VERBOSE==3)) {
		 ...
		 fprintf(stdout, "THE TILE FOR SOLVING PROCESS is %16.8f\n", t4);
	 ...
	 if((dm->comm->myproc == 0)&&(VERBOSE==1)) {
	  ...
	  x= (double *)malloc(sizeof(double)*n);
	   ...
	   MPI_Gatherv(&(sol->vec[0]), nloc, MPI_DOUBLE, &(xx[0]), iwork, mapptr ,  MPI_DOUBLE, 0,MCW );
	   MPI_Gatherv(&(poloc[0]), nloc, MPI_INT, &(perm[0]), iwork, mapptr ,  MPI_INT, 0,MCW );
	   MPI_Bcast(perm,AA.n,MPI_INT,0, MCW);
	   
	   int *invp=(int *)malloc(sizeof(int)*n);
	   for(i=0;i<n;i++) invp[perm[i]]=i;

	   if(dm->comm->myproc==0){for(i=0;i<n;i++) x[i]=xx[invp[i]];  } 
	   MPI_Bcast(x,AA.n,MPI_DOUBLE,0, MCW);
	  
	   DeleteVec(&sol); DeleteVec(&rhs); free(xx);
	   //This should be in Destructor
	   free(perm); free(displs); free(mapptr);  free(invp);
	  	
	   free(prepar); free(ipar);  DeleteMat(&dm); DeletePrec(precon);
...
BuilddSolvePARMS(DCL_ARG_SPARSE_SOLVER(double,A))
{
    ...
    return new dSolvePARMS(*A,ds.data_filename, ds.lparams, ds.dparams);
}
