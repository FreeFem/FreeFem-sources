		...
		float *eangle = new float[nbe];
		{
			...
			for (i = 0; i < nbe; i++)
				if (edges[i].Cracked()) {
					...
					if (e1[0].The() == e2[0].The() && e1[1].The() == e2[1].The()) {} else if (e1[0].The() == e2[1].The() && e1[1].The() == e2[0].The()) {} else {
						err++;
		...
		for (int i = 0; i < NbOfCurves; i++) {
			...
			if (be->Equi() || be->ReverseEqui()) {
				...
				if (eqbe != bee) {
					...
					if (be != ee)
						sens = -1;
				} else if ((be == ee) && (curves[i].kb != curves[nc].kb)) sens = -1;
...
// if(ordre)     delete [] ordre;
	...
	Real8 GeometricalEdge::R1tg (Real8 theta, R2 &t) const	// 1/R of radius of cuvature
	{
		...
		Real8 sd2 = sqrt(d2);
		t = d;
		if (d2 > 1.0e-20) {t /= sd2; return Abs(Det(d, dd)) / (d2 * sd2);} else return 0;
	}
