 ...
 */
	...
	void Triangles::IntersectConsMetric (const double *s, const Int4 nbsol, const int *typsols,
	                                     ...
	                                     const int DoNormalisation, const double power, const int choice) {	// the array of solution s is store
...
// modif F Hecht 101099
	...
	void Triangles::SmoothMetric (Real8 raisonmax) {
		...
		while (Head0 >= 0 && kk++ < 100) {
			...
			for (i = Head0; i >= 0; i = first_np_or_next_t0[ip = i], first_np_or_next_t0[ip] = -1) {// pour tous les triangles autour du sommet s
				// cout << kk << " i = " << i << " " << ip << endl;
