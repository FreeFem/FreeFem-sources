...
#define srandom init_genrand
	...
	void swap (Triangle *t1, Int1 a1,
	           Triangle *t2, Int1 a2,
	           Vertex *s1, Vertex *s2, Icoor2 det1, Icoor2 det2) {	// swap
		// --------------------------------------------------------------
	...
	Int4 FindTriangle (Triangles &Th, Real8 x, Real8 y, double *a, int &inside) {
		...
		if (tb.link) {	// internal point in a true triangles
...
//
		...
		for (;; edge = dir > 0 ? Next(Adj(Next(edge))) : Previous(Adj(Previous(edge)))) {
			...
			IJ_AJ = (IJ, (J - A));
			if (IJ_AJ < 0) {
				if (dir < 0) {a = 0; b = 1; return edge;} else {
					dir = 1;
					continue;
				}
			}	// go in direction j
	...
	Metric Triangles::MetricAt (const R2 &A) const {// if ((vertices <= &v) && (vertices < v+nbv)) return v.m;
		I2 a = toI2(A);
		Icoor2 deta[3];
	...
	void ListofIntersectionTriangles::SplitEdge (const Triangles &Bh,
	                                             const R2 &A, const R2 &B, int nbegin) {// SplitEdge
...
// if(SHOW) cout << i << " " << j << " " << k << " " << ba[i] << " " << ba[j] << endl;
	...
	int SwapForForcingEdge (Vertex * &pva, Vertex * &pvb,
	                        TriangleAdjacent &tt1, Icoor2 &dets1, Icoor2 &detsa, Icoor2 &detsb, int &NbSwap) {	// l'arete ta coupe l'arete pva pvb
					...
					for (int j = 0; j < 2; j++)
						if (!ei.Mark() && ei[j].Required()) {
							...
							if (ei.Required()) {
								...
								for (int kstep = 0; kstep <= step; kstep++) {	// begin  for ( int kstep=0;kstep<= step;kstep++)
