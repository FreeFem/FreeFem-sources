...
#define MXE__LINE __LINE__ + 1
  ...
  void Geometry::AfterRead( ) {    // -----------------
    ...
    for (i = 0; i < nbv; i++) {
      ...
      if (ord == 2) {    // angulare test to find a corner
        ...
        if (edges[i1].ref != edges[i2].ref) {
          vertices[i].SetRequired( );
          if (verbosity > 17) cout << "     The vertex " << i << " is Required ref" << endl;
        }
      };
    ...
    for (int i = 0; i < NbOfCurves; i++) {
      ...
      if(verbosity> 5) cout <<"  !!!! Curve " << i << "  equi/-equi "<< Number(be) << " " << Number(ee)<<
            " equi "<< Number(eqbe) << " " << Number(eqee) << endl;
     
      if (be->Equi( ) || be->ReverseEqui( )) {
        int beinv = be->ReverseEqui( ) ;// 0 ou 1
        assert(eqbe);
        int nc = eqbe->CurveNumber;
        GeometricalEdge *bee = curves[nc].be;
        GeometricalEdge *eee = curves[nc].ee;
       if(verbosity>10) cout <<"  !!!! Curve " << i << "  equi/-equi "<< Number(curves[i].be) << " " << Number(curves[i].ee)
          << " -> " << Number(curves[i].be->link) << " " << Number(curves[i].ee->link) <<
            " equi "<< Number(curves[nc].be) << " " << Number(curves[nc].ee)
          << " :: "<< curves[i].kb << " " << curves[nc].kb << " // " << curves[i].ke << " " << curves[nc].ke << " " <<  beinv << endl;

        assert(i != nc);
        curves[i].next = curves[nc].next;
        curves[i].master = false;
        curves[nc].next = curves + i;
        //  retourne le courbe si pas meme (debut  et fin)  ..
        if (!( curves[nc].be == curves[i].be->link)) // && curves[i].kb == (beinv ? 1-curves[nc].kb : curves[nc].kb  )))
        { // same start
            if(verbosity>99) cout << "  before reverse curve " << curves[i].kb << " " << curves[nc].kb << " // " << curves[i].ke << " " << curves[nc].ke <<endl;
              curves[i].Reverse( );
            if(verbosity>99) cout << " after reverse " << curves[i].kb << " " << curves[nc].kb << " // " << curves[i].ke << " " << curves[nc].ke <<endl;
        }
          ffassert( curves[nc].ee == curves[i].ee->link) ; // && curves[nc].ke == curves[i].ke );
          ffassert( curves[nc].be == curves[i].be->link) ; // && curves[nc].ke == curves[i].ke );
        ...
        int sens = 1;
        if (eqbe != bee) {
          ffassert(eqee == bee && eqbe == eee);    // eq end == begin
          if (be != ee) sens = -1;
