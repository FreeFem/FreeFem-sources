...
namespace bamg {
  ...
  void Triangles::Draw( ) const {
    ...
    //  for (i=0;i<nbv;i++)
    //    vertices[i].Draw(i);
    for (i = 0; i < nbe; i++) edges[i].Draw( );
    for (i = 0; i < nbt; i++)
      if (triangles[i].link) triangles[i].Draw(-2);
    //  for (i=0;i<nbv;i++)
  ...
  void Edge::Draw(Int4 i) const {
    if (!withrgraphique) return;
    if (InRecScreen(Min(v[0]->r.x, v[1]->r.x), Max(v[0]->r.y, v[1]->r.y), Min(v[0]->r.x, v[1]->r.x),
                    Max(v[0]->r.y, v[1]->r.y))) {
      v[0]->MoveTo( );
      v[1]->LineTo( );
  ...
  void Triangle::Draw(Int4 i) const {
    ...
    if (ns[0] == 0) {
      if (InRecScreen(Min(ns[1]->r.x, ns[2]->r.x), Min(ns[1]->r.y, ns[2]->r.y),
                      Max(ns[1]->r.x, ns[2]->r.x), Max(ns[1]->r.y, ns[2]->r.y))) {
    ...
    } else if (InRecScreen(Min3(ns[0]->r.x, ns[1]->r.x, ns[2]->r.x),
                           ...
                           Max3(ns[0]->r.y, ns[1]->r.y, ns[2]->r.y))) {
      {
        ...
        const int i1 = VerticesOfTriangularEdge[j01][1],
                  j12 = EdgesVertexTriangle[i1][1];    // i1=1,j12=0
        const int i2 = VerticesOfTriangularEdge[j12][1],
                  j20 = EdgesVertexTriangle[i2][1];    // i1=2,j20=1

        rmoveto(ns[i0]->r.x, ns[i0]->r.y);
        if (Hidden(j01))
          rmoveto(ns[i1]->r.x, ns[i1]->r.y);
        else
          rlineto(ns[i1]->r.x, ns[i1]->r.y);
        if (Hidden(j12))
          rmoveto(ns[i2]->r.x, ns[i2]->r.y);
        else
          rlineto(ns[i2]->r.x, ns[i2]->r.y);
        if (Hidden(j20))
          rmoveto(ns[i0]->r.x, ns[i0]->r.y);
        else
          rlineto(ns[i0]->r.x, ns[i0]->r.y);
  ...
  void Triangles::inquire( ) {
    ...
    // cout << "inquire
    // **********************************************************************"<<endl ;
    if (verbosity > 2) cout << "inquire: Nb de Triangle reel = " << ConsRefTriangle(reft) << endl;

    while (Show("Enter ? for help", PS == 0), c = ::Getxyc(x, y),
           (c && c != 'F' && c != 'f') && c < 250) {
      rd = 0;
      couleur(1);
      ...
      if (c == 'D') {
        for (int iter = 0; iter < 50; iter++) {
          ...
          if (k != 0)
            cout << " Bizarre le maillage n'est  pas delaunay  nbswap = " << k << endl;
          else
            break;
        }
        rd = 1;
      ...
      if (c == 'q' && quadtree)
        penthickness(2), couleur(6), quadtree->Draw( ), couleur(1), penthickness(1);
      ...
      if (c == 'm') {
        Int4 i;
        couleur(2);
        for (i = 0; i < nbv; i++) ((Metric)vertices[i]).Draw(vertices[i]);
        couleur(1);
      }
      if (c == 'V') {
        Int4 i;
        for (i = 0; i < nbv; i++) vertices[i].Draw( );
      }
      if (c == 'T') {
        Int4 i;
        for (i = 0; i < nbt; i++)
      ...
      if (c == 'b') {
        Int4 i;
        reffecran( );
        for (i = 0; i < nbe; i++) edges[i].Draw(edges[i].ref);
      }
      if (c == 'k') {
      ...
      if (c == 'v' || c == 'o' || c == 's') {
        ...
        cout << " sommet " << j << "= " << vertices[j] << ", d = " << d << " " << vertices[j].m
             << endl;
        vertices[j].Draw(j);
      ...
      if (c == 't' || c == 'c' || c == 'e') {
        ...
        for (Int4 i = 0; i < nbt; i++) {
          ...
          if (c == 'e' || c == 'E') {
            ...
            cout << " Edge " << Number(tta.EdgeVertex(0)) << " " << Number(tta.EdgeVertex(1))
                 << endl;
        ...
        if (!OnSwap) {
          ...
          while (1)
            if (kopt) {
              ...
              Real8 xb1 = sb->i.x - s1->i.x, x21 = s2->i.x - s1->i.x, yb1 = sb->i.y - s1->i.y,
                    y21 = s2->i.y - s1->i.y, xba = sb->i.x - sa->i.x, x2a = s2->i.x - sa->i.x,
                    yba = sb->i.y - sa->i.y, y2a = s2->i.y - sa->i.y,
                    cosb12 = xb1 * x21 + yb1 * y21, cosba2 = xba * x2a + yba * y2a, sinb12 = det2,
                    sinba2 = t2->det,
                    // zsinba2 = t2->det;
              ...
              DrawMark(sb->r);
              {
                ...
                if (Abs(d) > dd * 1.e-3) {
                  D2 C(MAB + ABo * ((D.x * A1o.y - D.y * A1o.x) / d));
                  cout << "M1 r2 =" << M(C - S2) << " r1 = " << M(C - S1) << "ra = " << M(C - SA)
                       << " rb = " << M(C - SB);
                  som = M(C - S2) / M(C - S1);
                  cout << " r1/r2 = " << som << endl;
                } else {
              ...
              }
              {
                ...
                if (Abs(d) > dd * 1.e-3) {
                  D2 C(MAB + ABo * ((D.x * A1o.y - D.y * A1o.x) / d));
                  cout << "M2 r1 =" << M(C - S2) << " r2 = " << M(C - S1) << "ra = " << M(C - SA)
                       << " rb = " << M(C - SB) << " r1/r2 =  " << M(C - S2) / M(C - S1) << endl;
                  som += M(C - S2) / M(C - S1);
                } else {
                  kopt = 1;
                  continue;
  ...
  void GeometricalEdge::Draw(Int4 i) {
    ...
    if (InPtScreen(v[1]->r.x, v[1]->r.y)) v[1]->LineTo( );
