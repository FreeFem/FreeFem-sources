...
// end of variable
			...
			Triangles (Int4 nbvx, Triangles &BT, int keepBackVertices = 1)
				: Gh(BT.Gh), BTh(BT) {
				try {GeomToTriangles1(nbvx, keepBackVertices);} catch (...) {this->clear(); throw;}}

			Triangles (Int4 nbvx, Geometry &G)
				: Gh(G), BTh(*this) {
				try {GeomToTriangles0(nbvx);} catch (...) {this->clear(); throw;}}
...
// return the other triangle of the quad if a quad or 0 if not a quat
	...
	inline void TriangleAdjacent::SetAdj2 (const TriangleAdjacent &ta, int l) {	// set du triangle adjacent
		if (t) {
			t->at[a] = ta.t;
			t->aa[a] = ta.a | l;
		}
