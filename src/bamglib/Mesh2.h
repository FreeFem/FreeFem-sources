  ...
  class Edge {
   public:
    ...
    int Intersection(const Edge &e) const {
      if (!(adj[0] == &e || adj[1] == &e))
        cerr << "Bug : Intersection " << (void *)&e << "  " << adj[0] << " " << adj[1] << endl;
   ...
   public:
    ...
    void NewSubSeg(GeometricalEdge *e, Real8 s0, Real8 s1) {
      if (NbSeg >= MaxNbSeg) {
        int mneo = MaxNbSeg;
        MaxNbSeg *= 2;
        if (verbosity > 3) cout << " reshape lSegsI from " << mneo << " to " << MaxNbSeg << endl;
    ...
    void ReShape( ) {
      ...
      for (int i = 0; i < MaxSize; i++)    // recopy
        nw[i] = lIntTria[i];
      if (verbosity > 3)
        cout << " ListofIntersectionTriangles  ReShape MaxSize " << MaxSize << " -> " << newsize
             << endl;
      MaxSize = newsize;
