        ...
        for (int phase = 0; phase <= step; phase++) {
          ...
          for (Curve *curve = Gh.curves + icurve; curve; curve = curve->next) {
            ...
            int icurveequi = Gh.Number(curve);
             if(verbosity>5 && phase==0 && icurveequi != icurve ) cout << " do equicurve "<<icurve << " " << icurveequi << endl;
            ...
            if (ongequi->Required( )) {
              ...
              for (;;) {
                ...
                ee.on->SetMark( );
                int ii0=0,ii1=1;
                if(k1 != k1equi) std::swap(ii0,ii1);
                Vertex &v0 = ee[ii0], &v1 = ee[ii1];
                Vertex &v0q = eeequi[ii0], &v1q = eeequi[ii1];
                R2 AB = (R2)v1 - (R2)v0;
                Real8 L0 = L, LAB;
                ...
                if (phase) {    // computation of the new points
                  while ((i != NbCreatePointOnCurve) && sNew <= L) {
                    ...
                    e->v[0] = A0;
                    e->v[1] = A1;
                      if(verbosity > 9 && icurve < 2)
                      {
                          cout << i << " " << icurve << " "
                          <<  icurveequi << " New P " << A1->r << " " << se << " " << seo << " " << ii0<<ii1<< endl;
                      }
                    if (verbosity > 99)
