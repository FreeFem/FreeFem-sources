  ...
  public:
    ...
    public:
      lgNRJ(Stack s, int n, Expression t, Expression JJ, Expression dJJ, Expression hJJ)
        : tNRJ<PARAM, VECT, VMAT, REAL>(n), stack(s), J(JJ), dJ(dJJ), hJ(hJJ), theparame(t), gg(0) {
          if(dJ) gg = new VECT(n);
      }
    ...
    E_LCG(const basicAC_F0 & args,int cc) : cas(cc) {
      ...
      if (opdJ)
        dJ = to<Kn_>(C_F0(opdJ, "(", theparam)); // May2019 (bug?) Modif FH 17102005 (to verify) to<Kn*> ->to<Kn>
      if (ophJ)
