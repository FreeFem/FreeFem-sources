...
template<>
bool v_dfes<MeshS>::buildperiodic(Stack stack, KN<int>& ndfe) {
    return BuildPeriodic2(nbcperiodic, periodic, **ppTh, stack, ndfe);
}

template<>
bool v_dfes<Mesh3>::buildperiodic(Stack stack, KN<int> &ndfe) {
  return BuildPeriodic3(nbcperiodic, periodic, **ppTh, stack, ndfe);
}

template<> 
bool v_dfes<MeshL>::buildperiodic(Stack stack, KN<int>& ndfe) {
    return BuildPeriodic1(nbcperiodic, periodic, **ppTh, stack, ndfe);
}
...
// Accesseurs templatés
template<class Mesh>
long pVhd_ndof(v_dfes<Mesh>** p) {
  throwassert(p && *p);
  GFESpace<Mesh>* fes = **p;
  return fes ? fes->NbOfDF : 0;
}
template<class Mesh>
...
template<class Mesh>
KN<double>* pVhd_D(Stack stack, v_dfes<Mesh>** const& p) {
  throwassert(p);
  v_dfes<Mesh>* f = *p;
  GFESpace<Mesh>* fes = **p; (void)fes;     // déclenche buildDistributedDofData
  KN<double>* r = new KN<double>(f->Ddof);
  Add2StackOfPtr2Free(stack, r); return r;
}
template<class Mesh>
KN<KN<long>>* pVhd_intersection(Stack stack, v_dfes<Mesh>** const& p) {
  throwassert(p);
...
template<class K,class v_fes>
class Op3_pfL2K : public ternary_function<pair<FEbase<K,v_fes> *,int>,R,R,K> { public:
...
void init_lgmesh3() {
  ...
  Add<pfLc>("(","",new OneQuadOperator<Op4_pfL2K<Complex,v_fesL>,Op4_pfL2K<Complex,v_fesL>::Op> );
    Add<pfLr>("(","",new OneTernaryOperator<Op3_pfL2K<R,v_fesL>,Op3_pfL2K<R,v_fesL>::Op> );
    Add<pfLc>("(","",new OneTernaryOperator<Op3_pfL2K<Complex,v_fesL>,Op3_pfL2K<Complex,v_fesL>::Op> );
    Add<pfLr>("(","",new OneBinaryOperatorCode<Op2_pfL2K<R,v_fesL>,Op2_pfL2K<R,v_fesL>::Op> );
    Add<pfLc>("(","",new OneBinaryOperatorCode<Op2_pfL2K<Complex,v_fesL>,Op2_pfL2K<Complex,v_fesL>::Op> );
