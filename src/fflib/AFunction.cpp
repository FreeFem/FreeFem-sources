...
bool Assert (bool b) {
	if (!b) throw (ErrorExec("exec assert", 1));
...
class OneOperator_array: public OneOperator {
	public:
...
class OneOperator_border: public OneOperator {
	public:
		E_F0*code (const basicAC_F0 &a) const {
...
class OneOperator_border_label: public OneOperator {
	public:
...
struct Op2_set_pstring: public binary_function<R, string *, R> {
	static R f (R const &p, string *const &a) {
		*p = new RR(a->c_str());
		if (!*p || !**p) {
...
aType TypeArray (aType b, aType a) {// type of  b[a]
...
aType TypeTemplate (aType b, aType a) {	// type of  b[a]
...
// add frev 2007
class TransE_Array:  public E_F0 {
	public:
		const E_Array *v;
...
template<>
class ForEachType<void *>:  public basicForEachType {
	public:	// correction july 2009..... FH  Hoooo....  (Il y a un bug DUR DUR FH  ...)
...
C_F0 opColumn::code2 (const basicAC_F0 &args) const {
	...
	if ((eb || teb) && (ea || tea)) {
		...
		// if( na1==1 && mb1 ==1)
		return s;
	} else if (ea || tea) {	// modif 2 /08/  2013  FH .. bug in [ a0,a1,... ]'*b
	...
	} else if (eb || teb) {	// modif 2 /08/  2013  FH .. bug in a*[ b0,b1,... ]'
		const E_Array &b = tb ? *teb->v : *eb;
		int nb = b.size();
