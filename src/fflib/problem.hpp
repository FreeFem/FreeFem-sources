...
inline ostream &operator << (ostream &f, const TypeSolveMat &tm) {
	switch (tm.t) {
	case TypeSolveMat::NONESQUARE:  f << "No Square (Sparse Morse)";
		break;
	case TypeSolveMat::LU:  f << "LU (Skyline)";
		break;
	case TypeSolveMat::CROUT:  f << "CROUT (Skyline)";
		break;
	case TypeSolveMat::CHOLESKY:  f << "CHOLESKY (Skyline)";
		break;
	case TypeSolveMat::GC:  f << "CG (Sparse Morse)";
		break;
	case TypeSolveMat::GMRES:  f << "GMRES (Sparse Morse)";
		break;
	case TypeSolveMat::SparseSolver:  f << "SparseSolver (Sparse Morse)";
		break;
...
class C_args_minus: public C_args  {
	public:
...
template<typename F>
class Minus_Form: public E_F0mps    {
	public:
...
class FormBilinear: public E_F0mps {
	public:
...
class FormLinear: public E_F0mps {
	public:
	  ...
	  public OpCall_FormLinear_np
{
	...
	E_F0*code (const basicAC_F0 &args) const {
		...
		Expression p = args[1];
		if (!p->EvaluableWithOutStack()) CompileError("  a(long,Vh) , The long  must be a constant, and  = 0, sorry");

		long pv = GetAny<long>((*p)(NullStack));
		if (pv) CompileError("  a(long,Vh) , The long  must be a constant == 0, sorry");
...
template<class R>
class IntFunction: public E_F0mps {
	public:
		typedef R Result;
		typedef const CDomainOfIntegration *A;
		typedef R B;
...
template<class KA, class KB> class Matrix_Prod {
	public:
		Matrice_Creuse<KA> *A;
		Matrice_Creuse<KB> *B;
		bool ta, tb;
...
template<class K> class Matrice_Creuse_inv {
	public:
		Matrice_Creuse<K> *A;
...
void SetSolver (Stack stack, bool VF, MatriceCreuse<R> &A, Data_Sparse_Solver &ds) {
	...
	if (ds.typemat->profile) {
		...
		switch (ds.typemat->t) {
		case TypeSolveMat::LU: AA.typesolver = FactorizationLU;
			break;
		case TypeSolveMat::CROUT: AA.typesolver = FactorizationCrout;
			break;
		case TypeSolveMat::CHOLESKY:  AA.typesolver = FactorizationCholeski;
			break;
		default:
			cerr << " type resolution " << ds.typemat->t << " sym=" << ds.typemat->profile << endl;
			CompileError("type resolution unknown");
			break;
		}
	} else {
...
// bool factorize=false;
	...
	if (same) {
		...
		if (A.Uh != Uh || A.Vh != Vh) {	// reconstruct all the matrix
			A.A = 0;// to delete  old  matrix ADD FH 16112005
			A.Uh = Uh;
			A.Vh = Vh;
			if (ds.typemat->profile)
			{A.A.master(new MatriceProfile<R>(Vh, VF)); ffassert(&Uh == &Vh);} else if (ds.typemat->sym) {
	...
	} else {// add FH 17 06 2005  int on different meshes.
		map<pair<int, int>, R> AAA;
		bool bc = AssembleVarForm<R, map<pair<int, int>, R>, FESpace>(stack, Th, Uh, Vh, ds.typemat->sym, &AAA, 0, b->largs);
		if (ds.typemat->profile) ExecError(" Sorry, construction of Skyline matrix with different meshes is not implemented! "); else A.A.master(new MatriceMorse<R>(Vh.NbOfDF, Uh.NbOfDF, AAA, ds.typemat->sym));
