...
class classBuildMesh:  public E_F0mps {
	public:
...
class classBuildMeshArray:  public E_F0mps {
	public:
...
// modif aout 2007
class BuildMeshFile:  public E_F0mps {
	public:
...
class SplitMesh:  public E_F0mps {
	public:
...
class Adaptation:   public E_F0mps {
	public:
		typedef pmesh Result;
...
AnyType MoveMesh::operator () (Stack stack) const {
	...
	if (pth)
		for (size_t i = 0; i < sol.size(); i++) {	// ale
...
class CheckMoveMesh:  public E_F0mps {
	public:
	...
	for (int k = 0; k < Th.nt; k++)
		if (!where || (*where)[k]) {
			...
			if (fk[i2] <= 0.) arean += K.area;
				...
				if (fk[i1] < 0) {double y = fk[i2] / (fk[i2] - fk[i1]); c *= y * y;} else {
					double y = fk[i0] / (fk[i0] - fk[i1]);
					c = 1. - (1. - c) * y * y;
				}
...
double Chi (Stack stack, pmesh const &pTh) {// version 2d  oct 2017 FH.
	if (pTh == 0) return 0.;
...
double Chi (Stack stack, pmesh3 const &pTh) {	// version 3d oct 2017 FH.
	if (pTh == 0) return 0.;
