...
long mp_region (MeshPoint *p) {	// throwassert(p && p->Th);
	long nu(p->region);
...
class Convect: public E_F0mps  {
	public:
		...
		Convect (const basicAC_F0 &args): u(0), v(0), w(0), ff(0), dt(0) {
			...
			if (d == 3)
				w = CastTo<double>((*a)[2]);
			else if (d != 2) CompileError("convect vector have only 2 or 3 componant");
...
class Plot:  public E_F0mps	/* [[file:AFunction.hpp::E_F0mps]] */ {
	public:
		...
		{	// FH. change nov 2016  add one  expression  for  colored curve ...
			...
			int EvalandPush (Stack s, int ii, vector<ListWhat> &ll, vector<AnyType> &lat) const {	// add for curve ... and multi curve ...
				// store date in lat..
				long f[4];
			...
			int AEvalandPush (Stack s, int ii, vector<ListWhat> &ll) const {
				...
				for (int i = 0; i < 3; ++i)
					if (e[i]) {
						if (!composant)
						{PA p = GetAny<PA>((*e[i])(s)); cmp[i] = p.second; f[i] = p.first; nn = i;} else
			...
			sol eval (int i, Stack s, int &cmp) const {
				cmp = -1;
				if (e[i]) {
					if (!composant) {pfer p = GetAny<pfer>((*e[i])(s)); cmp = p.second; return p.first;} else return GetAny<pferbase>((*e[i])(s)); } else return 0; }

			sol3 eval3 (int i, Stack s, int &cmp) const {
				cmp = -1;
				if (e[i]) {
					if (!composant) {pf3r p = GetAny<pf3r>((*e[i])(s)); cmp = p.second; return p.first;} else return GetAny<pf3rbase>((*e[i])(s)); } else return 0; }

			// add FH Japon 2010 ..	for complex visu ...  to complex ....  try to uniformize ...
			solc evalc (int i, Stack s, int &cmp) const {
				cmp = -1;
				if (e[i]) {
					if (!composant) {pfec p = GetAny<pfec>((*e[i])(s)); cmp = p.second; return p.first;} else return GetAny<pfecbase>((*e[i])(s)); } else return 0; }

			solc3 evalc3 (int i, Stack s, int &cmp) const {
				cmp = -1;
				if (e[i]) {
					if (!composant) {pf3c p = GetAny<pf3c>((*e[i])(s)); cmp = p.second; return p.first;} else return GetAny<pf3cbase>((*e[i])(s)); } else return 0; }

			asol evala (int i, Stack s, int &cmp) const {
				cmp = -1;
				if (e[i])
				{pferarray p = GetAny<pferarray>((*e[i])(s)); cmp = p.second; return p.first;} else return 0; }

			asol3 evala3 (int i, Stack s, int &cmp) const {
				cmp = -1;
				if (e[i])
				{pf3rarray p = GetAny<pf3rarray>((*e[i])(s)); cmp = p.second; return p.first;} else return 0; }

			asolc evalca (int i, Stack s, int &cmp) const {
				cmp = -1;
				if (e[i])
				{pfecarray p = GetAny<pfecarray>((*e[i])(s)); cmp = p.second; return p.first;} else return 0; }

			asolc3 evalca3 (int i, Stack s, int &cmp) const {
				cmp = -1;
				if (e[i])
				{pf3carray p = GetAny<pf3carray>((*e[i])(s)); cmp = p.second; return p.first;} else return 0; }
		...
		Plot (const basicAC_F0 &args): l(args.size()) {
			...
			for (size_t i = 0; i < l.size(); i++)
				...
				if (args[i].left() == atype<E_Array>()) {
					...
					} else CompileError("plot of array with wrong  number of components (!= 2 or 3) "); } else if (BCastTo<pferbase>(args[i])) {// [[file:problem.hpp::pferbase]] [[file:lgmesh3.hpp::BCastTo]]
 ...
 */
	...
	if (dim == 2) {
				...
				for (int t = 0; t < Th.nt; ++t) {
					if (all || setoflab.find(Th[t].lab) != setoflab.end()) {
						...
						if (umx > 0) {	// coupe ..
							...
							if (f[i1] < 0) {double y = f[i2] / (f[i2] - f[i1]); c *= y * y;} else {
		...
		if (kind == CDomainOfIntegration::int2d)
			...
			if (di->islevelset()) {	// int3d levelset < 0
...
AnyType Plot::operator () (Stack s) const {
	...
	for (size_t i = 0; i < l.size(); i++)
		...
		switch (l[i].what) {
		case 3:  l[i].EvalandPush(s, i, ll, lat);
			break;
		case 0:
		case 5:
		...
		case  100: l[i].MEvalandPush<pmesh>(s, i, ll);
			break;
		case  105: l[i].MEvalandPush<pmesh3>(s, i, ll);
			break;
		case  101: l[i].AEvalandPush<asol, sol>(s, i, ll);
			break;
		case  106: l[i].AEvalandPush<asol3, sol3>(s, i, ll);
			break;
		case  111:  l[i].AEvalandPush<asolc, solc>(s, i, ll);
			break;
		case  116:  l[i].AEvalandPush<asolc3, solc3>(s, i, ll);
			break;
		case  103: l[i].AEvalandPush(s, i, ll, lat);
			break;
...
#define SEND_VTK_PARAM(index, type)                                      \
	...
	if (clean) {
		...
		for (size_t ii = 0; ii < ll.size(); ii++) {
			...
			if (first)
			{first = false; Pmin = P1; Pmax = P2;} else {
...
AnyType Convect::eval3 (Stack s) const {// nouvelle version de convect 3d Feb 2015  version 3.44-01
	MeshPoint *mp(MeshPointStack(s)), mpc(*mp);
...
template<class T> T*resizeandclean2 (const Resize<T> &t, const long &n) {	// resizeandclean1
...
aType typeFESpace (const basicAC_F0 &args) {
	...
	for (int i = 0; i < args.size(); i++)
		...
		{ffassert(d == 0 || d == 2); d = 2; break;} else if (args[i].left() == t_tfe3 || args[i].left() == t_m3)
