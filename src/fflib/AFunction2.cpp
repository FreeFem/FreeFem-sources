...
class vectorOfInst: public E_F0mps {
	public:
		int n;
		Expression *v;
		vectorOfInst (int k): n(k), v(new Expression[k]) {
			ffassert(v);
...
AnyType ListOfInst::operator () (Stack s) const {
	...
	try {	// modif FH oct 2006
		...
		if (atclose) {
			if (verbosity > 99999) cout << " ListOfInst::operator()  " << n << " " << atclose->n << " // " << lsldel[n - 1] << endl;

			atclose->eval(s, atclose->n);
		}	// Add for sep 2016  FH
	} catch (E_exception &e) {
