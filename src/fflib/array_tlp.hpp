...
template<class T>
void HeapSort (T *c, long n, long o) {	// trie un tableau c de n valeur avec un decalage de o.
...
template<class K> long get_imax (KN<K> *p) {
	long i = 0;

	for (long k = 1; k < p->N(); ++k) if ((*p)[k] > (*p)[i]) i = k;

	return i;
}

template<class K> long get_imin (KN<K> *p) {
	long i = 0;
...
template<class K>
class OneOperator_2KN_: public OneOperator {
	public:
