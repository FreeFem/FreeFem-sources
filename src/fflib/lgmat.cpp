...
template<class pfes>
class RestrictArray: public OneOperator {
	public:
...
// Fin Add FH Houston --------
template<class pfes>
class MatrixInterpolation: public OneOperator {
	public:
...
template<class R>
class SetMatrix_Op: public E_F0mps {
	public:
		Expression a;
		static aType btype;
...
template<class R>
class SetMatrix: public OneOperator {
	public:
 ...
 */
	...
	if (ds.factorize) {
		...
		switch (ds.typemat->t) {
		case TypeSolveMat::LU: pf->LU(Abs(ds.epsilon));
			break;
		case TypeSolveMat::CROUT: pf->crout(Abs(ds.epsilon));
			break;
		case TypeSolveMat::CHOLESKY: pf->cholesky(Abs(ds.epsilon));
			break;
...
void buildInterpolationMatrix (MatriceMorse<R> *m, const FESpace &Uh, const FESpace &Vh, void *data) {	// Uh = Vh
...
void buildInterpolationMatrix (MatriceMorse<R> *m, const FESpace3 &Uh, const FESpace3 &Vh, void *data) {// Uh = Vh
...
MatriceMorse<R>*buildInterpolationMatrix1 (const FESpace &Uh, const KN_<double> &xx, const KN_<double> &yy, int *data) {// Uh = Vh
	int op = op_id;	// value of the function
	int icomp = 0;
	bool transpose = false;
	bool inside = false;
...
MatriceMorse<R>*buildInterpolationMatrix1 (const FESpace3 &Uh, const KN_<double> &xx, const KN_<double> &yy, const KN_<double> &zz, int *data) {// Uh = Vh
	typedef FESpace3::Mesh Mesh;
	typedef FESpace3::FElement FElement;
	typedef Mesh::Element Element;
	typedef FESpace3::Rd Rd;
...
AnyType SetMatrixInterpolation1 (Stack stack, Expression emat, Expression einter, int init) {
	...
	data[1] = mi->arg(1, stack, (long)op_id);
	;	// get just value
	data[2] = mi->arg(2, stack, false);
	;	// get just value
	data[3] = mi->arg(3, stack, 0L);
	;	// get just value
	KN<long> U2Vc;
...
R*get_elementp2mc (Matrice_Creuse<R> *const &ac, const long &b, const long &c) {
	...
	R *p = a->pij(b, c);
	if (!p) {
		if (verbosity) cerr << "Error: the coef a(" << b << "," << c << ")  do'nt exist in sparse matrix "
			                << " Matrix  type = " << typeid(ac).name() << endl;
...
template<class K>
class OneBinaryOperatorA_inv: public OneOperator {
	public:
...
struct TheDiagMat {
	...
	void get_mat_daig (KN_<R> &x) {
		ffassert(A && A->A && x.N() == A->A->n && A->A->n == A->A->m);
		A->A->getdiag(x);
	}
	...
	void set_mat_daig (const KN_<R> &x) {
		ffassert(A && A->A && x.N() == A->A->n && A->A->n == A->A->m);
		A->A->setdiag(x);
	}
};
...
struct TheCoefMat {
	...
	void get_mat_coef (KN_<R> &x) {
		ffassert(A && A->A && x.N() == A->A->NbCoef());
		A->A->getcoef(x);
	}

	void set_mat_coef (const KN_<R> &x) {
		ffassert(A && A->A && x.N() == A->A->NbCoef());
		A->A->setcoef(x);
	}
...
template<typename R>
class BlockMatrix1:  public BlockMatrix<R> {
	public:
		...
		for (int j = 0; j < M; j++) {
			...
			if (rij == atype<long>() && eij->EvaluableWithOutStack()) {
				...
				if (contm == 0) {
					e_Mij[i][j] = 0;
					t_Mij[i][j] = 0;
				} else if (atype<R>()->CastingFrom(rij)) {	// frev 2007
				...
				e_Mij[i][j] = eij;
				t_Mij[i][j] = 2;
			} else if (atype<KNM<R> *>()->CastingFrom(rij)) {	// before KN_ because KNM can be cast in KN_
	...
	public:
		...
		class CODE: public E_F0 {
			public:
				...
				operator aType () const {return atype<R>();}};	// end sub class CODE
	...
	for (int i = 0; i < N; ++i)
		for (int j = 0; j < M; ++j) {
			...
			if (eij) {
				...
				else if (tij == 3) {KN_<R> x = GetAny<KN_<R>>(e); Fij(i, j) = new KNM_<R>(x, x.N(), 1);} else if (tij == 4) {KN_<R> x = GetAny<Transpose<KN_<R>>>(e).t; Fij(i, j) = new KNM_<R>(x, 1, x.N());} else if (tij == 5) {KNM<R> *m = GetAny<KNM<R> *>(e); Fij(i, j) = new KNM_<R>(*m);} else if (tij == 6) {KNM<R> *m = GetAny<Transpose<KNM<R> *>>(e).t; Fij(i, j) = new KNM_<R>(m->t());} else if (tij == 7) {Rij(i, j) = GetAny<R>(e); Fij(i, j) = new KNM_<R>(&(Rij(i, j)), 1, 1);}
				// else if  (tij==3) {}
				else {
					cout << " Bug " << tij << endl;
...
struct  VirtualMatCR: public VirtualMatrice<Complex> {
	public:
...
class PrintErrorCompileIM:  public E_F0info {
	public:
		typedef double Result;
		static E_F0*f (const basicAC_F0 &args) {
