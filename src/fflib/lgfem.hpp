...
template<class Mesh> class v_dfes : public generic_v_fes {
 public:
  ...
  const int N;
  const pmeshT *ppTh;    // adr du maillage
  ...
  operator FESpace *( ) {
    ...
    if (!pVh || *ppTh != &pVh->Th) { pVh = CountPointer< FESpace >(update( ), true); dofDataBuilt = false; }
    if (DTh && !dofDataBuilt) { buildDistributedDofData(*pVh); dofDataBuilt = true; }
    return pVh;
  }
  
  FESpace *update( ){
    assert(dHat == Mesh::RdHat::d);
...
template<class Mesh, class TypeOfFE>
class pdfes_tef : public v_dfes<Mesh> {
 public:
  typedef const Mesh* pmeshT;
  typedef GFESpace<Mesh> FESpace;
  const TypeOfFE *tef;

  pdfes_tef(const pmeshT *t, const TypeOfFE *tt, const DistributedMesh<Mesh>* dth = nullptr, Stack s = NullStack, int n = 0, Expression *p = nullptr)
    : v_dfes<Mesh>(tt->N, t, dth, s, n, p), tef(tt) {
    this->operator FESpace *( );
  }
  FESpace *buildupdate(KN< int > &ndfe) {
    return *this->ppTh ? new FESpace(**this->ppTh, *tef, ndfe.size( ) / 2, ndfe) : nullptr;
  }
  FESpace *buildupdate( ) { return *this->ppTh ? new FESpace(**this->ppTh, *tef) : nullptr; }
};

template<class Mesh, class TypeOfFE>
class pdfes_tefk : public v_dfes<Mesh> {
 public:
  typedef const Mesh* pmeshT;
  typedef GFESpace<Mesh> FESpace;
  const TypeOfFE **tef;
  const int k;
  KN< GTypeOfFE< Mesh > const * > atef;
  GTypeOfFESum< Mesh > tefs;

  static int sum(const TypeOfFE **l, int const TypeOfFE::*p, int n) {
    int r = 0;
    for (int i = 0; i < n; i++) r += l[i]->*p;
    return r;
  }

  pdfes_tefk(const pmeshT *t, const TypeOfFE **tt, int kk, const DistributedMesh<Mesh>* dth = nullptr, Stack s = NullStack, int n = 0,
             Expression *p = nullptr)
    : v_dfes<Mesh>(sum(tt, &TypeOfFE::N, kk), t, dth, s, n, p), tef(tt),
      k(kk), atef(kk, tt), tefs(atef)
  ...
  FESpace *buildupdate( ) {
    return *this->ppTh ? new FESpace(**this->ppTh, tefs) : nullptr;
  }
  virtual ~pdfes_tefk( ) { delete[] tef; }
  FESpace *buildupdate(KN< int > &ndfe) {
    return *this->ppTh ? new FESpace(**this->ppTh, tefs, ndfe.size( ) / 2, ndfe) : nullptr;
  }
};

typedef v_dfes<MeshS>                  v_dfesS;
typedef v_dfes<MeshS>*                 pdfesS;
typedef pdfes_tef<MeshS, TypeOfFES>    pdfesS_tef;
typedef pdfes_tefk<MeshS, TypeOfFES>   pdfesS_tefk;
typedef v_dfes<Mesh3> v_dfes3;
typedef v_dfes<Mesh3>* pdfes3;
typedef pdfes_tef<Mesh3, TypeOfFE3> pdfes3_tef;
typedef pdfes_tefk<Mesh3, TypeOfFE3> pdfes3_tefk;
typedef v_dfes<MeshL>                 v_dfesL;
typedef v_dfes<MeshL>*                pdfesL;
typedef pdfes_tef<MeshL, TypeOfFEL>   pdfesL_tef;
typedef pdfes_tefk<MeshL, TypeOfFEL>  pdfesL_tefk;

template<> bool v_dfes<MeshS>::buildperiodic(Stack, KN<int>&);
template<> bool v_dfes<Mesh3>::buildperiodic(Stack, KN<int>&);
template<> bool v_dfes<MeshL>::buildperiodic(Stack, KN<int>&);
