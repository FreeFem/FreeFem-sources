...
namespace Fem2D {
	...
	void Check (const Opera &Op, int N, int M) {
		...
		for (BilinearOperator::const_iterator l = Op.v.begin(); l != Op.v.end(); l++) {	// attention la fonction test donne la ligne
			...
			for (npi = 0; npi < FI.n; npi++) {	// loop on the integration point
				...
				for (i = 0; i < n; i++) {
					...
					for (j = 0; j < m; j++) {
						...
						int jg(Ku(j));
						if (!sym || ig <= jg)
							for (BilinearOperator::const_iterator l = Op.v.begin(); l != Op.v.end(); l++, il++) {	// attention la fonction test donne la ligne
								// et la fonction test est en second
								BilinearOperator::K ll(*l);
								pair<int, int> jj(ll.first.first), ii(ll.first.second);
...
// assert(maxd<last_operatortype);
	...
	void Element_rhs (const FElement3 &Kv, const LOperaD &Op, double *p, void *vstack, KN_<R> &B,
		...
		for (int iii = 0; iii + 1 < np; iii += 2) {	// 0,1,, a and 2,3,0.
	...
	bool AssembleVarForm (Stack stack, const typename FESpace::Mesh &Th, const FESpace &Uh, const FESpace &Vh, bool sym,
	                      MC *A, KN_<R> *B, const list<C_F0> &largs) {	// return true if BC
		...
		for (int ib = 0; ib < Th.neb; ib++) {
			...
			if (on.find(r) != on.end()) {
				...
				for (int k = 0; k < kk; k++) {
					...
					while (comp + 1 < Nbcomp && Uh.dim_which_sub_fem[comp + 1] == Uh.dim_which_sub_fem[comp]) {	// the right
						k++;// NEXT COMP
						comp++;
					...
					for (int df = 0; df < nbdf; df++)
						...
						if (K.FromASubFE(df) == Uh.dim_which_sub_fem[xx.first] && onWhatIsEdge[ie][K.DFOnWhat(df)]) {
							...
							;	// A->SetBC(ddf, tgv);// change 21 dec 2010 FH (Hack of ILU)
 ...
 *    cout << endl;*/
	...
	void AssembleLinearForm (Stack stack, const Mesh3 &Th, const FESpace3 &Vh, KN_<R> *B, const FormLinear *l) {
		...
		if (kind == CDomainOfIntegration::int2d) {	// AFAIRE("3D Elment RHS CDomainOfIntegration::int2d");
			double ss = 0;
			...
			if (di.islevelset()) {	// init on level set (of RHS)
				...
				for (int t = 0; t < ThI.nt; ++t) {
					...
					if (umn <= 0 && umx >= 0) {
						int np = IsoLineK(f, Q, 1e-10);	// ca code ...
						if (np == 3 || np == 4) {	// if(np==3) Q[3]=Q[0]; // same 0 == 3 bofbof ??? FH
							// cout << " Q[0]" << Q[0] << endl;
							if (verbosity > 99) {
								R3 PP[4];
								const Tet &K(ThI[t]);
		...
		} else if (kind == CDomainOfIntegration::intalledges) AFAIRE("3D Elment RHS CDomainOfIntegration::intalledges");
		/*
		 * if(VF)
		 * {
			...
			*X = R();
			if (verbosity > 1)
				cout << "   -- Change of Mesh " << (u_Vh ? &(*(u_h[0])).Vh->Th : 0)
				     << "  " << &Th << endl;
		} else {// copy the previous soluton to initialize CG, GMRES, etc ...
			if (Nb == 1) {	// modif  FH 0701/2005 + april 2006
				if (u_h[0]->x()->N() != X->N())
					cout << " bug ???? " << endl;
...
void DefSolver (Stack stack, MatriceCreuse<R> &A, Data_Sparse_Solver &ds) {
	...
	if (ds.typemat->profile) {
		...
		switch (ds.typemat->t) {
		case TypeSolveMat::LU: AA.LU(tol_pivot1);
			break;
		case TypeSolveMat::CROUT: AA.crout(tol_pivot1);
			break;
		case TypeSolveMat::CHOLESKY: AA.cholesky(tol_pivot1);
			break;
		default:
			cerr << " type resolution " << ds.typemat->t << endl;
			CompileError("type resolution profile inconnue");
			break;
		}
	} else {
...
// #endif
		default:
			cerr << " type resolution " << ds.typemat->t << endl;
			CompileError("type resolution inconnue");
			break;
		}
	}
}
...
bool FieldOfForm (list<C_F0> &largs, bool complextype) {// true => complex problem
	...
	for (ii = ib; ii != ie; ii++) {
		...
		} else if (r == atype<const BC_set *>()) {	// modif FH  mai 2007  A FAIRE il y a un bug ici XXXXXXXXXXXXX
...
void SetArgsFormLinear (const ListOfId *lid, int ordre) {
	...
	} else if (nbarray == 0) {	// a supprimer  to remove   in case of bilinear
