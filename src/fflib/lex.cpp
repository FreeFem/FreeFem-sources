...
bool mylex::InMotClef (const char *b, aType &t, int &r) const {
	...
	if (i == MotClef.end()) {t = 0; r = ID; return false;} else {
		r = i->second.first;
		t = i->second.second;
		ffassert(r);
		return true;
...
debut:
	...
	// <<found_a_number>>
		...
		buf[i] = 0;
		if (nc == 'i') buf[i++] = source().get(), buf[i] = 0, plglval->dnum = atof(buf), ret = CNUM; else if (real)
...
bool mylex::SetMacro (int &ret) {
	...
	if (strncmp(buf, "macro", 6) == 0 || (oldmacro = strncmp(buf, newmacro, 9)) == 0) {
		...
		do {
			...
			int ii = source().peek();
			if (isNLCR(source(), i)) {linenumber++; nl = 1;} else if (isalpha(i) && isalpha(ii)) {	// Modif F.H
				// def +=char(i);
				item = char(i);
				i = source().get();
...
bool mylex::IFMacro (int &ret) {// A faire !!!! F.H
	bool rt = false;
...
void mylex::input (const string &str, const string *name, int lg) {
	ffassert(level < 99 && level >= -1);
	if (level >= 0) pilesource[level].l = linenumber;
...
mylex::~mylex () {
	...
	while (!strdata.empty()) {	// commente july 2005 FH ???. a test plus finement.
