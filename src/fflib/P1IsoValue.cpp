...
static inline int signep4 (int i0, int i1, int i2, int i3) {// calcul du signe dans la permutation
	int s = 1;
...
int UnderIso (double *f, R2 Q[2][3], double area2[2], double eps) {
	...
	if (km == 0) {
		area2[0] = 0;
		return 0;
	} else if (km == 1) {	// 1 tet  j, j+1, j+2, j+3
		int j0 = nm[0];
...
// count number
	...
	if (km == 0) {
		...
		ntet = 1;
	} else if (km == 2) {	// 1 prisme i0,i1, j0,j1, k0, k0
		ntet = 3;
		int i0 = nm[0];
		int i1 = nm[1];
		int k0 = np[0];
