...
class E_F0: public CodeAlloc
{
	public:
		...
		virtual int compare (const E_F0 *t) const {
			int r = (t == this) ? 0 : ((this < t) ? -1 : 1);
...
inline ostream &operator << (ostream &f, const E_F0 &e) {
	if (!e.Empty()) e.dump(f);
	else f << " --0-- ";
...
class E_F1: public CodeAlloc {
	public: virtual AnyType operator () (Stack, AnyType &)  const = 0;
};

class E_F2: public CodeAlloc {
	public: virtual AnyType operator () (Stack, AnyType &, AnyType &)  const = 0;
};

class E_FN: public CodeAlloc {
	public: virtual AnyType operator () (Stack, size_t N, ...)  const = 0;
};
...
// by default Empty and do nothing
		...
		void Add (const char *op, OneOperator *p0, OneOperator *p1 = 0, OneOperator *p2 = 0,
		          ...
		          OneOperator *pc = 0, OneOperator *pd = 0, OneOperator *pe = 0) const
...
// warning ------------------
class ForTypeVoid:  public basicForEachType {
	public:
		ForTypeVoid (): basicForEachType(typeid(void), 0, 0, 0, 0, 0) {}
};

template<class T>
class ForEachType:  public basicForEachType {
	public:
...
class ForEachTypePtr:  public basicForEachType {
	public:
		ForEachTypePtr ();
...
template<class T>
class ForEachTypePtr<T *, T **>:  public basicForEachType {
	public:
...
class ForEachTypePtrfspace:  public ForEachTypePtr<T> {
	public:
...
class ForTypeAnyType:  public basicForEachType {
	public:
...
class E_F0_CFunc2: public E_F0mps {
	public:
		CFunction2 f2;
		E_F0 *a, *b;
...
class E_F0_CFunc4: public E_F0mps {
	public:
		CFunction4 f4;
		E_F0 *a, *b, *c, *d;
...
// modif for xlc++ FH
template<class R, class TA0, bool RO = true>
class E_F_F0_Opt: public E_F_F0<R, TA0, RO>  {
	public:
		size_t ia;
...
// modif for xlc++
template<class R, class TA0, class TA1>
class E_F_F0F0_Opt: public E_F_F0F0<R, TA0, TA1>  {
	public:
		size_t ia, ib;
...
template<class R, class A0>
class E_F_F0_Add2RC: public E_F0 {
	public:
		typedef  R (*func)(const A0 &);
		func f;
		Expression a;
...
template<class R, class A0, class A1, class E = E_F0>
class E_F_F0F0_Add2RC: public E {
	public:
		typedef  R (*func)(const A0 &, const A1 &);
		func f;
		Expression a0, a1;
...
template<class R, class A0, class A1, class A2, class E = E_F0>
class E_F_stackF0F0F0_: public E_F0mps {
	public:
...
class E_F_StackF0F0: public E_F0mps {
	public:
...
// return PtrtoAny(static_cast<void *>(static_cast<char *>(s)+offset),t);}
		...
		LocalVariable (size_t o, aType tt): offset(o), t(tt) {
			throwassert(tt);
...
template<class U>
class LocalVariablePlus: public LocalVariable {
	public:
		U data;
...
class basicAC_F0 {
	...
	public:
		...
		C_F0 find (const char *k) const {
			assert(k);
			if (named_parameter) {
				const_iterator i = named_parameter->find(k);
				if (i == named_parameter->end()) return C_F0();
				else return i->second; } else return C_F0(); }
...
class basicAC_F0_wa: public basicAC_F0 {
	public:
...
class PlotStream;
class E_Border;
class E_BorderN: public E_F0mps {
	public:
		const E_Border *b;
		...
		int size () const {
			...
			for (const E_BorderN *pp = this; pp; pp = pp->next) k++;
		...
		long Nbseg (Stack stack, int index) const {
			if (cas == 0) {assert(index == 0); return GetAny<long>((*n)(stack));} else if (cas == 1) return (GetAny<KN_<long>>((*n)(stack)))(index);
			else if (cas == 2) {E_Array &a = *dynamic_cast<E_Array *>((Expression)n); assert(a); Expression nn = a[index]; return GetAny<long>((*nn)(stack));} else return 0;
		}
...
class OneOperator_borderN: public OneOperator {
	public:
...
class E_Border: public Polymorphic  {
	public:
...
inline C_F0 TableOfIdentifier::Find (const char *name) const {
	...
	if (i == m.end()) return C_F0(); else return C_F0(i->second); }

inline C_F0 TableOfIdentifier::Find (const char *name, const basicAC_F0 &args) const {
	const_iterator i = m.find(name);

	if (i == m.end()) {
		cerr << "No operator " << name << endl;
		cerr << *this << endl;
		CompileError("TableOfIdentifier::Find");
		return C_F0();
...
template<class T>
inline C_F0 TableOfIdentifier::NewVar (Key k, aType t, size_t &top) {	// if( t-> ExistDestroy()) NbNewVarWithDel++;
...
inline C_F0::C_F0 (const C_F0 &a, const C_F0 &b) {	// the concatenation
	if (a.Empty()) r = b.r, f = b.f; else if (b.Empty()) r = a.r, f = b.f; else {
		r = b.r;
...
class OneBinaryOperator_st: public OneOperator {
	...
	public:
		E_F0*code (const basicAC_F0 &args) const {	// cout << "A op B \n" ;
...
class E_F1_funcT_Type: public OneOperator {
	public:
...
template<class R, class A>
class E_F1_funcT: public E_F1_funcT_Type {
	public:
...
inline Type_Expr basicForEachType::SetParam (const C_F0 &, const ListOfId *, size_t &) const {
	...
	InternalError("basicForEachType::SetParam non defined");
}	// return make_pair<aType,const E_F0  *>(c.left(),c.LeftValue());}
...
class E_exception: public exception {
	public:
 ...
 */
class Routine;
class E_Routine:  public E_F0mps {
	public:
		ListOfInst *code;
		// Expression clean;
		aType rt;
...
class Routine: public OneOperator {
	public:
		size_t offset;
		aType tfunc, tret;
		const char *name;
...
class ForAllLoopOpBase:  public E_F0mps {
	public:
		Expression et, ecode;
...
template<class F>
class ForAllLoop: public OneOperator {
	public:
		typedef typename F::Tab T;
		class ForAllLoopOp:  public ForAllLoopOpBase {
			public:
				F f;
