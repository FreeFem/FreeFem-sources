...
void getline_popen_vertex(int ddim, double *c, int *ref) {
  ...
  for (i = 1; i < ddim; i++) c[i] = strtod(tictac, &tictac);
...
int loadScaVecTen(pMesh mesh, int numsol, int dim, int ver, int nel, int type, int size,
                  int *typtab, int key, char *natureread) {
  pSolution sol;
