...
AnyType MetricPk:: operator()(Stack stack) const 
{
    ...
    //le bord
    //Th.ElementAdj(k,ie); renvoie k', écrase ie par ie';
    // is frontiere k' <0 ou k'==k
    //Seg
    //BoundaryElement (+récent)
    //Th.nbe ou Th.neb
...
//  for(int i=0; i<Th.neb; ++i) nextv[Th.be(i)[0]]=Th.be(i)[1]; //ne convient pas : les numéros sont oubliés, seuls les points sont conservés.
    ...
    for(int i=0; i<Th.nt; ++i){
        ...
        switch(m_deg){
            case 2:{    //accolades nécessaires pour déclarer des variables dans un case
                double f[2]; tk.getDerivatives(DOFt,invHauteur,f); //f={fx,fy}
                
                for(int j=0; j<3; ++j){
                    const int s= Th(i,j); //le sommet j du triangle i
                    Deriv[m_dim*s+0]+= aire* f[0]*invHauteur[j].x; //contribution à l'estimation des dérivées secondes.
            ...
            case 3:{
                ...
                for(int j=0; j<3; ++j){
                    const int s= Th(i,j);
                    Deriv[m_dim*s+0]+= aire*f[0]*invHauteur[j].x; //contribution à l'estimation des dérivées troisièmes.
            ...
            case 4:{
                ...
                for(int j=0; j<3; ++j){
                    const int s= Th(i,j);
                    Deriv[m_dim*s+0]+= f[0]*invHauteur[j].x*aire; //contribution à l'estimation des dérivées quatrièmes.
