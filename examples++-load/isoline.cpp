...
struct R2_I2 {
	...
	int count (int k0, const multimap<int, int> &L) const {
		if (nx >= 0) {return 1;} else {return L.count(k0);}
	}
};
...
AnyType FINDLOCALMIN_P1_Op::operator () (Stack stack)  const {
	...
	for (int k = 0; k < nbt; ++k) {
		...
		for (int i = 0; i < 3; ++i) {
			...
			int i0 = Th(K[(i + 1) % 3]);
			int i1 = Th(K[(i + 2) % 3]);
			if (U[i0] > U[i1]) {rs[i0] = 0;} else if (U[i1] > U[i0]) {rs[i1] = 0;}
		}
	}
...
AnyType ISOLINE_P1_Op::operator () (Stack stack)  const {
	...
	vector<R2_I2> P;
	multimap<int, int> L;
	if (verbosity >= 1000) {debug = verbosity / 1000;} else {debug = 0;}
