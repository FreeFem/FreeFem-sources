// ORIG-DATE: april 2016
//  Compute function signe distance function
// -*- Mode : c++ -*-
//
// SUMMARY  :
// USAGE    : LGPL
// ORG      : LJLL Universite Pierre et Marie Curie, Paris,  FRANCE
// AUTHOR   :  F. Hecht Dec. 2016
// E-MAIL   :frederic.hecht@umpc.fr
//
...
template <class Rd>
Rd zero(double a, const Rd &A, double b,const  Rd &B)
{  // zero affine
...
double distmin(const Rd & A,const Rd & B,const Rd & Q)
{
    ...
    if (lc<0) d= Norme2(AQ);
    else if(lc>1.) d= Norme2(Rd(B,Q));
    else  d= Norme2(CQ);
    if(verbosity>9999)
        cout << " distmin: d =" << d << " /" << lc << " :: " << A << " " << B << " " << Q << " C" << C << endl;
    return d;
}
...
double distmin(const Rd & A,const Rd & B,const Rd & Q,double aq,double bq)
{
    ...
    if(verbosity>9999)
        cout << " distmin:AB/Q: d =" << d << " /" << lp << " :: A " << A << " B " << B << " Q " << Q << "  P " << (A + lp*AB) << endl;
    return d;
}
...
int DistanceIso0(const Tet &K,double *f,double *fK)
{
    ...
    R3 P[10];
    int np =0;
    if( abs(f[0]) <eps) {f[0]=0.;P[np++]=K[0];}
    if( abs(f[1]) <eps) {f[1]=0.;P[np++]=K[1];}
    if( abs(f[2]) <eps) {f[2]=0.;P[np++]=K[2];}
    if( abs(f[3]) <eps) {f[3]=0.;P[np++]=K[3];}
    for (int e=0; e<6; ++e)
    {
...
int DistanceIso0(const Triangle &K,double *f,double *fK)
{
    ...
    R2 P[6];
    int np =0;
    if( abs(f[0]) <eps) f[0]=0.;
    if( abs(f[1]) <eps) f[1]=0.;
    if( abs(f[2]) <eps) f[2]=0.;
    int ke[6];
    for (int e=0; e<3; ++e)
    {
        int i1= (e+1)%3;
...
basicAC_F0::name_and_type Distance2d_Op::name_param[]= {
    {  "distmax", &typeid(double)}
};
...
pair<double,long > Add(const Mesh3 & Th,int kk,int ee,double *fv)
{
    ...
    if(debug)  cout << " ** add " << kk << " " << ee << " ; " <<  fq << " :: "
                    << fv[ia] << " " << fv[ib] << " "<< fv[ic]<< " || " << fv[iq] << endl;
...
//#define mmm min
    ...
    if( verbosity >2) cout << "      distance max = "<< dmax << " nt ="<<nbt <<  endl;
   ...
   // Pk[3],Gk[3];
    ...
    //
    if(verbosity>3)
        cout << "    Distance: nb elemets in queue after init"<< pqs.size() << " / nb set " << nt0<< endl;
    double fm;
    while (!pqs.empty())
    {
        KEY t=pqs.top();
    ...
    for(int i=0; i< nbv; ++i)
    {
        if(fv[i] == distinf ) {fv[i]=dmax;err++;}
        fv[i] = copysign(fv[i],f[i]);
    }
    if ( err && fm <dmax)
    {
        if(verbosity )cout << " Distance 2d : we miss some point " << err << endl;
    }
    return err;
}
...
basicAC_F0::name_and_type Distance3d_Op::name_param[]= {
    {  "distmax", &typeid(double)}
};
...
class  Distance2d_P1: public OneOperator { public:
    typedef const Mesh *pmesh;
    Distance2d_P1() :
    OneOperator(atype<long>(),atype<pmesh>(),atype<double>(), atype<KN<double>*>() ){}
    
    E_F0 * code(const basicAC_F0 & args) const
    {
        return  new Distance2d_Op( args,
                                  t[0]->CastTo(args[0]),
                                  t[1]->CastTo(args[1]),
                                  t[2]->CastTo(args[2])
                                  );
...
class  Distance3d_P1: public OneOperator { public:
    typedef const Mesh3 *pmesh3;
    Distance3d_P1() :
    OneOperator(atype<long>(),atype<pmesh3>(),atype<double>(), atype<KN<double>*>() ){}
    
    E_F0 * code(const basicAC_F0 & args) const
    {
        return  new Distance3d_Op( args,
                                  t[0]->CastTo(args[0]),
                                  t[1]->CastTo(args[1]),
                                  t[2]->CastTo(args[2])
                                  );
