...
#include "ExampleMetrics.h"
	...
	Edge *Edge::refine (Tab<Edge> &EdgeAllocator, Tab<Vertex> &VertexAllocator, const Metric2 &metric, refinement_priority priority) {
		...
		if (wf != this) {wf->refine(EdgeAllocator, VertexAllocator, metric, priority);}

		if (sister != NULL) {
			Edge *const swf = sister->which_first(priority);
			if (swf != sister) {swf->refine(EdgeAllocator, VertexAllocator, metric, priority);}
		}
	...
	Edge *Edge::which_first (refinement_priority priority) {
		if (priority == selected_edge_first) {return this;} else if (priority == newest_vertex_first) {
	...
	bool Edge::cut (Vertex const *start, Vertex const *end, Tab<Edge> &EdgeAllocator, Tab<Vertex> &VertexAllocator,
		...
		if (start == v) {
			return next->cut(start, end, EdgeAllocator, VertexAllocator, metric, aligned);	// looking for the starting edge, in the wrong direction
		}
		...
		if (start == u) {	// looking for the starting edge, in the good direction
			...
			do {
				...
				if (currentEdge == this) {return false;}
	...
	void Triangulation::hRefineQA (double h, unsigned int flag, Edge::refinement_priority priority) {
		...
		for (int i = 0; i < ne_iso; i++) {
			...
			for (int j = 0; j <= 1; j++) {
				...
				if (currentDet < minDet[index]) {minDet[index] = currentDet; minEdge[index] = e;} else if (currentDet > maxDet[index]) {maxDet[index] = currentDet; maxEdge[index] = e;}
			}
		}
		...
		for (int i = 0; i < ne_iso; i++) {
			...
			if (finalRefine) {
				if (extru && !extrv) {
					endVertex[indexw] = indexv + 1;	// "+1" required to distinguish from values initialized to zero
				}
