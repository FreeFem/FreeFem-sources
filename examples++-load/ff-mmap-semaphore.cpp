...
{  typedef FF_P_mmap T;
    ...
    void destroy(){ if(map) ffmmap_destroy(map); delete map;map=0; }
};
...
struct ff_pointeur_sem
{
    ...
    void destroy(){ if(sem) ffsem_destroy(sem); delete sem;sem=0; }
};
...
long ff_msync(pFF_p_mmap const & map)
{
    return  ffmmap_msync(map->map,0,0);
}
